...
void buildOpenCLKernels_poisson_kernel_populate(int xdim3, int xdim4,
                                                int xdim5) {
  ...
  if (!isbuilt_poisson_kernel_populate) {
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim3_poisson_kernel_populate=%d  "
                           "-Dxdim4_poisson_kernel_populate=%d  "
                           "-Dxdim5_poisson_kernel_populate=%d ",
                pPath, 32, xdim3, xdim4, xdim5);
      else
    ...
    OPS_opencl_core.kernel[0] = clCreateKernel(
        OPS_opencl_core.program, "ops_poisson_kernel_populate", &ret);
    clSafeCall(ret);
...
void ops_par_loop_poisson_kernel_populate(char const *name, ops_block block,
  ...
  buildOpenCLKernels_poisson_kernel_populate(xdim3, xdim4, xdim5);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[0], 0, sizeof(cl_int),
                            (void *)arg0.data));
