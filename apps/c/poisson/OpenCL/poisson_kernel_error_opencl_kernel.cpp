...
void buildOpenCLKernels_poisson_kernel_error(OPS_instance *instance, int xdim0,
                                             int xdim1) {
  ...
  if (!isbuilt_poisson_kernel_error) {
    ...
    instance->ostream() << "Compiling poisson_kernel_error " << OCL_FMA
                        << " source -- start \n";
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/include "
                           "-DOPS_WARPSIZE=%d  -Dxdim0_poisson_kernel_error=%d "
                           " -Dxdim1_poisson_kernel_error=%d ",
                pPath, 32, xdim0, xdim1);
      else
        sprintf(buildOpts, "-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  "
                           "-Dxdim0_poisson_kernel_error=%d  "
                           "-Dxdim1_poisson_kernel_error=%d ",
                pPath, 32, xdim0, xdim1);
    else {
    ...
    instance->opencl_instance->OPS_opencl_core.kernel[4] =
        clCreateKernel(instance->opencl_instance->OPS_opencl_core.program,
                       "ops_poisson_kernel_error", &ret);
    clSafeCall(ret);
