...
void poisson_kernel_error(const ptr_double u, const ptr_double ref,
                          double *err) {
  *err = *err +
         (OPS_ACCS(u, 0, 0) - OPS_ACCS(ref, 0, 0)) *
             (OPS_ACCS(u, 0, 0) - OPS_ACCS(ref, 0, 0));
}

__kernel void ops_poisson_kernel_error(__global const double *restrict arg0,
  ...
  if (idx_x < size0 && idx_y < size1) {
    const ptr_double ptr0 = {&arg0[base0 + idx_x * 1 * 1 +
                                   idx_y * 1 * 1 * xdim0_poisson_kernel_error],
                             xdim0_poisson_kernel_error};
    const ptr_double ptr1 = {&arg1[base1 + idx_x * 1 * 1 +
                                   idx_y * 1 * 1 * xdim1_poisson_kernel_error],
                             xdim1_poisson_kernel_error};
    poisson_kernel_error(ptr0, ptr1, arg2_l);
  }
