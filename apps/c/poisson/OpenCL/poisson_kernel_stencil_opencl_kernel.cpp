...
void buildOpenCLKernels_poisson_kernel_stencil(int xdim0, int xdim1,
  ...
  if (!isbuilt_poisson_kernel_stencil) {
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_poisson_kernel_stencil=%d  "
                           "-Dxdim1_poisson_kernel_stencil=%d  "
                           "-Dxdim2_poisson_kernel_stencil=%d ",
    ...
    OPS_opencl_core.kernel[2] = clCreateKernel(
        OPS_opencl_core.program, "ops_poisson_kernel_stencil", &ret);
    clSafeCall(ret);
...
void ops_par_loop_poisson_kernel_stencil(char const *name, ops_block block,
  ...
  buildOpenCLKernels_poisson_kernel_stencil(xdim0, xdim1, xdim2);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[2], 2, sizeof(cl_mem),
                            (void *)&arg2.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[2], 3, sizeof(cl_double),
                            (void *)&dx));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[2], 4, sizeof(cl_double),
                            (void *)&dy));
