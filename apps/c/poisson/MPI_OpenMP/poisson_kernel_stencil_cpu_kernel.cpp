...
void ops_par_loop_poisson_kernel_stencil_execute(const char *name, ops_block block, int blockidx_start, int blockidx_end, int dim, int *range, int nargs, ops_arg* args) {
  ...
  const int xdim0_poisson_kernel_stencil = OPS_BATCHED == 0 ? block->count : args[0].dat->size[0];
  const int ydim0_poisson_kernel_stencil = OPS_BATCHED == 1 ? block->count : args[0].dat->size[1];
  const int xdim1_poisson_kernel_stencil = OPS_BATCHED == 0 ? block->count : args[1].dat->size[0];
  const int ydim1_poisson_kernel_stencil = OPS_BATCHED == 1 ? block->count : args[1].dat->size[1];
  ...
  #else
  const int xdim0_poisson_kernel_stencil = args[0].dat->size[0];
  const int ydim0_poisson_kernel_stencil = args[0].dat->size[1];
  const int xdim1_poisson_kernel_stencil = args[1].dat->size[0];
  const int ydim1_poisson_kernel_stencil = args[1].dat->size[1];
  ...
  double * __restrict__ u2_p = (double *)(args[1].data + args[1].dat->base_offset + blockidx_start * args[1].dat->batch_offset);
  ...
  for ( int n_2=bounds_2_l; n_2<bounds_2_u; n_2++ ){
    ...
    for ( int n_1=bounds_1_l; n_1<bounds_1_u; n_1++ ){
      ...
      for ( int n_0=bounds_0_l; n_0<bounds_0_u; n_0++ ){
        const ACC<double> u(xdim0_poisson_kernel_stencil, ydim0_poisson_kernel_stencil, u_p + n_0 + n_1 * xdim0_poisson_kernel_stencil + n_2 * xdim0_poisson_kernel_stencil * ydim0_poisson_kernel_stencil);
        ACC<double> u2(xdim1_poisson_kernel_stencil, ydim1_poisson_kernel_stencil, u2_p + n_0 + n_1 * xdim1_poisson_kernel_stencil + n_2 * xdim1_poisson_kernel_stencil * ydim1_poisson_kernel_stencil);
