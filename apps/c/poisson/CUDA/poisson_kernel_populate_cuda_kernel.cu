...
#else
#ifdef OPS_HYBRID
void ops_par_loop_poisson_kernel_populate_execute_gpu(ops_kernel_descriptor *desc) {
#else
...
#ifdef OPS_HYBRID
void ops_par_loop_poisson_kernel_populate_execute_cpu(ops_kernel_descriptor *desc);
void ops_par_loop_poisson_kernel_populate_execute(ops_kernel_descriptor *desc) {
  if (desc->device == 1) {
    ops_par_loop_poisson_kernel_populate_execute_gpu(desc);
  }
  else {
    ops_par_loop_poisson_kernel_populate_execute_cpu(desc);
  }
}
#endif
