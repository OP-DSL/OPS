...
void buildOpenCLKernels_multidim_reduce_kernel(int xdim0, int ydim0, int zdim0) {
  ...
  if(!isbuilt_multidim_reduce_kernel) {
    ...
    printf("Compiling multidim_reduce_kernel %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_multidim_reduce_kernel=%d  -Dydim0_multidim_reduce_kernel=%d  -Dzdim0_multidim_reduce_kernel=%d ", pPath, 32,xdim0,ydim0,zdim0);
        else
          sprintf(buildOpts,"-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_multidim_reduce_kernel=%d  -Dydim0_multidim_reduce_kernel=%d  -Dzdim0_multidim_reduce_kernel=%d ", pPath, 32,xdim0,ydim0,zdim0);
      else {
    ...
    OPS_opencl_core.kernel[2] = clCreateKernel(OPS_opencl_core.program, "ops_multidim_reduce_kernel", &ret);
    clSafeCall( ret );
...
void ops_par_loop_multidim_reduce_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[2], 2, nthread*sizeof(double), NULL));
    clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[2], 3, sizeof(cl_int), (void*) &r_bytes1 ));
    clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[2], 4, sizeof(cl_int), (void*) &base0 ));
    clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[2], 5, sizeof(cl_int), (void*) &x_size ));
    clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[2], 6, sizeof(cl_int), (void*) &y_size ));
    clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[2], 7, sizeof(cl_int), (void*) &z_size ));
