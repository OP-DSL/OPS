...
void buildOpenCLKernels_multidim_reduce_kernel(OPS_instance *instance,
                                               int xdim0, int ydim0,
                                               int zdim0) {
  ...
  if (!isbuilt_multidim_reduce_kernel) {
    ...
    instance->ostream() << "Compiling multidim_reduce_kernel " << OCL_FMA
                        << " source -- start \n";
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts,
                ...
                "-Dxdim0_multidim_reduce_kernel=%d  "
                "-Dydim0_multidim_reduce_kernel=%d  "
                "-Dzdim0_multidim_reduce_kernel=%d ",
    ...
    instance->opencl_instance->OPS_opencl_core.kernel[2] =
        clCreateKernel(instance->opencl_instance->OPS_opencl_core.program,
                       "ops_multidim_reduce_kernel", &ret);
    clSafeCall(ret);
