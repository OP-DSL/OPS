...
void buildOpenCLKernels_multidim_copy_kernel(int xdim0, int ydim0, int zdim0,
                                             int xdim1, int ydim1, int zdim1) {
  ...
  if (!isbuilt_multidim_copy_kernel) {
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(
            ...
            "-Dxdim0_multidim_copy_kernel=%d  -Dydim0_multidim_copy_kernel=%d  "
            "-Dzdim0_multidim_copy_kernel=%d  -Dxdim1_multidim_copy_kernel=%d  "
            "-Dydim1_multidim_copy_kernel=%d  -Dzdim1_multidim_copy_kernel=%d ",
            pPath, 32, xdim0, ydim0, zdim0, xdim1, ydim1, zdim1);
      else
        sprintf(
            buildOpts,
    ...
    OPS_opencl_core.kernel[1] = clCreateKernel(
        OPS_opencl_core.program, "ops_multidim_copy_kernel", &ret);
    clSafeCall(ret);
...
void ops_par_loop_multidim_copy_kernel(char const *name, ops_block block,
  ...
  buildOpenCLKernels_multidim_copy_kernel(xdim0, ydim0, zdim0, xdim1, ydim1,
                                          zdim1);
  ...
  base1 = base1 +
          args[1].dat->size[0] * args[1].dat->size[1] *
