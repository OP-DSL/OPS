...
//
#define OPS_ACC_MD0(d,x,y,z) ((x)+(xdim0*(y))+(xdim0*ydim0*(z))+(d)*xdim0*ydim0*zdim0)
#define OPS_ACC_MD1(d,x,y,z) ((x)+(xdim1*(y))+(xdim1*ydim1*(z))+(d)*xdim1*ydim1*zdim1)
...
void ops_par_loop_multidim_copy_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  zdim0 = args[0].dat->size[1];
  xdim1 = args[1].dat->size[0];
  ydim1 = args[1].dat->size[1];
  zdim1 = args[1].dat->size[1];
    ...
    for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
      ...
      for( n_x=start[0]; n_x<start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x+=SIMD_VEC ) {
        ...
        for ( int i=0; i<SIMD_VEC; i++ ){
          multidim_copy_kernel(  (double *)p_a[0]+ i*1, (double *)p_a[1]+ i*1 );
      ...
      for ( int n_x=start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x<end[0]; n_x++ ){
        ...
        multidim_copy_kernel(  (double *)p_a[0], (double *)p_a[1] );
