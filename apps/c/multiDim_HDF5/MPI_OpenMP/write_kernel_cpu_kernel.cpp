...
#define OPS_ACC1(x, y, z)                                                      \
  (n_x * 1 + x + (n_y * 1 + (y)) * xdim1_write_kernel +                        \
   (n_z * 1 + (z)) * xdim1_write_kernel * ydim1_write_kernel)
#define OPS_ACC2(x, y, z)                                                      \
  (n_x * 1 + x + (n_y * 1 + (y)) * xdim2_write_kernel +                        \
   (n_z * 1 + (z)) * xdim2_write_kernel * ydim2_write_kernel)
#define OPS_ACC_MD0(d, x, y, z)                                                \
  (n_x * 1 + x + (n_y * 1 + (y)) * xdim2_write_kernel + (n_z * 1 + (z)) * 2 +  \
   (d))
...
void ops_par_loop_write_kernel_execute(ops_kernel_descriptor *desc) {
...
#ifdef OPS_MPI
  sub_dat_list sd = OPS_sub_dat_list[args[2].dat->index];
  ...
  double *__restrict__ mult = (double *)(args[0].data + base0);

  int base1 = args[1].dat->base_offset;
  double *__restrict__ single = (double *)(args[1].data + base1);

  int base2 = args[2].dat->base_offset;
  int *__restrict__ digit = (int *)(args[2].data + base2);
...
#pragma omp simd aligned(mult, single, digit)
#else
#pragma simd
#endif
