...
//
#define OPS_ACC_MD0(d,x,y,z) ((x)*2+(d)+(xdim0*(y)*2)+(xdim0*ydim0*(z)*2))

//user function
inline void write_kernel(double *mult, double *single, int *digit, const int *idx) {
    ...
    for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
      ...
      for( n_x=start[0]; n_x<start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x+=SIMD_VEC ) {
        ...
        for ( int i=0; i<SIMD_VEC; i++ ){
          write_kernel(  (double *)p_a[0]+ i*1*2, (double *)p_a[1]+ i*1*1, (int *)p_a[2]+ i*1*1,
           (int *)p_a[3] );
      ...
      for ( int n_x=start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x<end[0]; n_x++ ){
        ...
        write_kernel(  (double *)p_a[0], (double *)p_a[1], (int *)p_a[2],
           (int *)p_a[3] );
