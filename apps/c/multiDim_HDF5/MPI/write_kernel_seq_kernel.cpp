...
#define OPS_ACC_MD0(d, x, y, z)                                                \
  ((x)*2 + (d) + (xdim0 * (y)*2) + (xdim0 * ydim0 * (z)*2))

// user function
inline void write_kernel(double *arr, double *arr_single, int *arr_integ,
                         const int *idx) {
...
#pragma novector
      for (n_x = start[0];
        ...
        for (int i = 0; i < SIMD_VEC; i++) {
          write_kernel((double *)p_a[0] + i * 1 * 2,
                       (double *)p_a[1] + i * 1 * 1, (int *)p_a[2] + i * 1 * 1,
                       (int *)p_a[3]);
