...
void flux_calc_kernely_c_wrapper(double *restrict vol_flux_y_p,
                                 double *restrict yarea_p,
                                 double *restrict yvel0_p,
                                 double *restrict yvel1_p, int x_size,
                                 int y_size, int z_size) {
#pragma omp parallel for
  for (int n_z = 0; n_z < z_size; n_z++) {
    for (int n_y = 0; n_y < y_size; n_y++) {
      for (int n_x = 0; n_x < x_size; n_x++) {
        ptr_double vol_flux_y = {
            vol_flux_y_p + n_x * 1 + n_y * xdim0_flux_calc_kernely * 1 +
        ...
        const ptr_double yarea = {
            yarea_p + n_x * 1 + n_y * xdim1_flux_calc_kernely * 1 +
        ...
        const ptr_double yvel0 = {
            yvel0_p + n_x * 1 + n_y * xdim2_flux_calc_kernely * 1 +
        ...
        const ptr_double yvel1 = {
            yvel1_p + n_x * 1 + n_y * xdim3_flux_calc_kernely * 1 +
