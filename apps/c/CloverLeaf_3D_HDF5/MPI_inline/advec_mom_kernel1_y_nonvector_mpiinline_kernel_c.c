...
void advec_mom_kernel1_y_nonvector_c_wrapper(
    ...
    double *restrict mom_flux_p, double *restrict celldy_p,
    ...
    for (int n_y = 0; n_y < y_size; n_y++) {
      for (int n_x = 0; n_x < x_size; n_x++) {
        const ptr_double node_flux = {
            node_flux_p + n_x * 1 +
                n_y * xdim0_advec_mom_kernel1_y_nonvector * 1 +
        ...
        const ptr_double node_mass_pre = {
            node_mass_pre_p + n_x * 1 +
                n_y * xdim1_advec_mom_kernel1_y_nonvector * 1 +
        ...
        ptr_double mom_flux = {mom_flux_p + n_x * 1 +
                                   n_y * xdim2_advec_mom_kernel1_y_nonvector *
                                       1 +
        ...
        const ptr_double celldy = {
            celldy_p + n_x * 0 + n_y * xdim3_advec_mom_kernel1_y_nonvector * 1 +
        ...
        const ptr_double vel1 = {
            vel1_p + n_x * 1 + n_y * xdim4_advec_mom_kernel1_y_nonvector * 1 +
        ...
        sigma = fabs(OPS_ACC(node_flux, 0, 0, 0)) /
                OPS_ACC(node_mass_pre, 0, donor, 0);
        ...
        if (vdiffuw * vdiffdw > 0.0) {
          ...
          limiter =
              ...
              MIN(width * ((2.0 - sigma) * adw / width +
                           (1.0 + sigma) * auw / OPS_ACC(celldy, 0, dif, 0)) /
                      6.0,
                  MIN(auw, adw));
        }
