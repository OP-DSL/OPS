...
void buildOpenCLKernels_advec_cell_kernel2_xdir(int xdim0, int ydim0, int xdim1,
  ...
  if (!isbuilt_advec_cell_kernel2_xdir) {
      ...
      if (OCL_FMA)
        sprintf(
            buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                       "-DOPS_WARPSIZE=%d  -Dxdim0_advec_cell_kernel2_xdir=%d  "
                       "-Dydim0_advec_cell_kernel2_xdir=%d  "
                       "-Dxdim1_advec_cell_kernel2_xdir=%d  "
                       "-Dydim1_advec_cell_kernel2_xdir=%d  "
                       "-Dxdim2_advec_cell_kernel2_xdir=%d  "
                       "-Dydim2_advec_cell_kernel2_xdir=%d  "
                       "-Dxdim3_advec_cell_kernel2_xdir=%d  "
                       "-Dydim3_advec_cell_kernel2_xdir=%d ",
            pPath, 32, xdim0, ydim0, xdim1, ydim1, xdim2, ydim2, xdim3, ydim3);
      else
        sprintf(buildOpts, "-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  "
                           "-Dxdim0_advec_cell_kernel2_xdir=%d  "
                           "-Dydim0_advec_cell_kernel2_xdir=%d  "
                           "-Dxdim1_advec_cell_kernel2_xdir=%d  "
                           "-Dydim1_advec_cell_kernel2_xdir=%d  "
                           "-Dxdim2_advec_cell_kernel2_xdir=%d  "
    ...
    OPS_opencl_core.kernel[109] = clCreateKernel(
        OPS_opencl_core.program, "ops_advec_cell_kernel2_xdir", &ret);
    clSafeCall(ret);
...
void ops_par_loop_advec_cell_kernel2_xdir(char const *name, ops_block block,
  ...
  buildOpenCLKernels_advec_cell_kernel2_xdir(xdim0, ydim0, xdim1, ydim1, xdim2,
                                             ydim2, xdim3, ydim3);
