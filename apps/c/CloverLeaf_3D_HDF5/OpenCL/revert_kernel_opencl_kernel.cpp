...
void buildOpenCLKernels_revert_kernel(int xdim0, int ydim0, int xdim1,
  ...
  if (!isbuilt_revert_kernel) {
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        ...
        sprintf(
            buildOpts, "-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  "
                       "-Dxdim0_revert_kernel=%d  -Dydim0_revert_kernel=%d  "
                       "-Dxdim1_revert_kernel=%d  -Dydim1_revert_kernel=%d  "
                       "-Dxdim2_revert_kernel=%d  -Dydim2_revert_kernel=%d  "
                       "-Dxdim3_revert_kernel=%d  -Dydim3_revert_kernel=%d ",
    ...
    OPS_opencl_core.kernel[103] =
        clCreateKernel(OPS_opencl_core.program, "ops_revert_kernel", &ret);
    clSafeCall(ret);
...
void ops_par_loop_revert_kernel(char const *name, ops_block block, int dim,
  ...
  buildOpenCLKernels_revert_kernel(xdim0, ydim0, xdim1, ydim1, xdim2, ydim2,
                                   xdim3, ydim3);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[103], 7, sizeof(cl_int),
                            (void *)&base3));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[103], 8, sizeof(cl_int),
                            (void *)&x_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[103], 9, sizeof(cl_int),
                            (void *)&y_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[103], 10, sizeof(cl_int),
                            (void *)&z_size));
