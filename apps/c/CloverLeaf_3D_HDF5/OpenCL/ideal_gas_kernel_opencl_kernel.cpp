...
void buildOpenCLKernels_ideal_gas_kernel(int xdim0, int ydim0, int xdim1,
  ...
  if (!isbuilt_ideal_gas_kernel) {
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts,
                ...
                "-Dxdim0_ideal_gas_kernel=%d  -Dydim0_ideal_gas_kernel=%d  "
                "-Dxdim1_ideal_gas_kernel=%d  -Dydim1_ideal_gas_kernel=%d  "
                "-Dxdim2_ideal_gas_kernel=%d  -Dydim2_ideal_gas_kernel=%d  "
                "-Dxdim3_ideal_gas_kernel=%d  -Dydim3_ideal_gas_kernel=%d ",
    ...
    OPS_opencl_core.kernel[10] =
        clCreateKernel(OPS_opencl_core.program, "ops_ideal_gas_kernel", &ret);
    clSafeCall(ret);
...
void ops_par_loop_ideal_gas_kernel(char const *name, ops_block block, int dim,
...
#ifdef CHECKPOINTING
  if (!ops_checkpointing_before(args, 4, range, 10))
    return;
#endif
  ...
  buildOpenCLKernels_ideal_gas_kernel(xdim0, ydim0, xdim1, ydim1, xdim2, ydim2,
                                      xdim3, ydim3);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[10], 3, sizeof(cl_mem),
                            (void *)&arg3.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[10], 4, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[10], 5, sizeof(cl_int),
                            (void *)&base1));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[10], 6, sizeof(cl_int),
                            (void *)&base2));
