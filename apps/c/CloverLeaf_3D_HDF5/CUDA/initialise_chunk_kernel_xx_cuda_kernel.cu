...
#define OPS_ACC0(x, y, z)                                                      \
  (x + dims_initialise_chunk_kernel_xx[0][0] * (y) +                           \
   dims_initialise_chunk_kernel_xx[0][0] *                                     \
       dims_initialise_chunk_kernel_xx[0][1] * (z))

// user function
__device__

    void
    initialise_chunk_kernel_xx_gpu(int *xx, int *idx) {
  xx[OPS_ACC0(0, 0, 0)] = idx[0] - 2;
}

#undef OPS_ACC0

__global__ void ops_initialise_chunk_kernel_xx(int *__restrict arg0,
  ...
  arg0 += idx_x * 1 * 1 +
          idx_y * 0 * 1 * dims_initialise_chunk_kernel_xx[0][0] +
          idx_z * 0 * 1 * dims_initialise_chunk_kernel_xx[0][0] *
              dims_initialise_chunk_kernel_xx[0][1];
...
void ops_par_loop_initialise_chunk_kernel_xx(char const *name, ops_block block,
                                             ...
                                             ops_arg arg1) {
#else
void ops_par_loop_initialise_chunk_kernel_xx_execute(
  ...
  if (xdim0 != dims_initialise_chunk_kernel_xx_h[0][0] ||
      ydim0 != dims_initialise_chunk_kernel_xx_h[0][1]) {
    dims_initialise_chunk_kernel_xx_h[0][0] = xdim0;
    dims_initialise_chunk_kernel_xx_h[0][1] = ydim0;
    cutilSafeCall(cudaMemcpyToSymbol(dims_initialise_chunk_kernel_xx,
                                     dims_initialise_chunk_kernel_xx_h,
                                     sizeof(dims_initialise_chunk_kernel_xx)));
  }
