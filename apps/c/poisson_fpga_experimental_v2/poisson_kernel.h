...
void poisson_kernel_populate(const int *idx, ACC<float> &u, ACC<float> &ref) {
...
void poisson_kernel_stencil(const int *idx, const ACC<float> &u,
                            ACC<float> &u2) {

  float x = dx * (float)(idx[0]);
  float y = dy * (float)(idx[1]);
  float f = -5.0*M_PI*M_PI*sin(M_PI*x)*cos(2.0*M_PI*y);
