...
#ifdef OPS_SOA
      const ACC<double> src(
          2, xdim0_multidim_copy_kernel, ydim0_multidim_copy_kernel,
          src_p + n_x * 1 + n_y * xdim0_multidim_copy_kernel * 1);
#else
      const ACC<double> src(
          2, xdim0_multidim_copy_kernel, ydim0_multidim_copy_kernel,
          src_p + 2 * (n_x * 1 + n_y * xdim0_multidim_copy_kernel * 1));
#endif
#ifdef OPS_SOA
      ACC<double> dest(2, xdim1_multidim_copy_kernel,
                       ydim1_multidim_copy_kernel,
                       dest_p + n_x * 1 + n_y * xdim1_multidim_copy_kernel * 1);
#else
      ACC<double> dest(
          2, xdim1_multidim_copy_kernel, ydim1_multidim_copy_kernel,
          dest_p + 2 * (n_x * 1 + n_y * xdim1_multidim_copy_kernel * 1));
#endif
