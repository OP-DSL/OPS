...
void buildOpenCLKernels_multidim_reduce_kernel(int xdim0) {
  ...
  if(!isbuilt_multidim_reduce_kernel) {
    ...
    printf("Compiling multidim_reduce_kernel %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_multidim_reduce_kernel=%d ", pPath, 32,xdim0);
        else
