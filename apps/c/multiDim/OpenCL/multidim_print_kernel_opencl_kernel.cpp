...
static bool isbuilt_multidim_print_kernel = false;

void buildOpenCLKernels_multidim_print_kernel(int xdim0) {
  ...
  if(!isbuilt_multidim_print_kernel) {
    ...
    cl_int ret;
    char* source_filename[1] = {"./OpenCL/multidim_print_kernel.cl"};
    ...
    printf("Compiling multidim_print_kernel %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_multidim_print_kernel=%d ", pPath, 32,xdim0);
        else
          sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_multidim_print_kernel=%d ", pPath, 32,xdim0);
      else {
      ...
      printf("compiling multidim_print_kernel -- done\n");

    // Create the OpenCL kernel
    OPS_opencl_core.kernel[2] = clCreateKernel(OPS_opencl_core.program, "ops_multidim_print_kernel", &ret);
    clSafeCall( ret );

    isbuilt_multidim_print_kernel = true;
  }
...
void ops_par_loop_multidim_print_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0) {
  ...
  buildOpenCLKernels_multidim_print_kernel(
  xdim0);
