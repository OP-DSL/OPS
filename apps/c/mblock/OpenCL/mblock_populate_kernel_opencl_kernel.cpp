...
void buildOpenCLKernels_mblock_populate_kernel(OPS_instance *instance,
                                               int xdim0) {
  ...
  if (!isbuilt_mblock_populate_kernel) {
    ...
    instance->ostream() << "Compiling mblock_populate_kernel " << OCL_FMA
                        << " source -- start \n";
    ...
    instance->opencl_instance->OPS_opencl_core.kernel[0] =
        clCreateKernel(instance->opencl_instance->OPS_opencl_core.program,
                       "ops_mblock_populate_kernel", &ret);
    clSafeCall(ret);
