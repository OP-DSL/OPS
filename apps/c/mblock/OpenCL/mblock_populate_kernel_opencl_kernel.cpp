...
void buildOpenCLKernels_mblock_populate_kernel(int xdim0) {
  ...
  if(!isbuilt_mblock_populate_kernel) {
    ...
    printf("Compiling mblock_populate_kernel %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_mblock_populate_kernel=%d ", pPath, 32,xdim0);
        else
      ...
      printf("compiling mblock_populate_kernel -- done\n");

    // Create the OpenCL kernel
    OPS_opencl_core.kernel[0] = clCreateKernel(OPS_opencl_core.program, "ops_mblock_populate_kernel", &ret);
    clSafeCall( ret );
...
void ops_par_loop_mblock_populate_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  buildOpenCLKernels_mblock_populate_kernel(
  xdim0);
  ...
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[0], 5, sizeof(cl_int), (void*) &y_size ));
