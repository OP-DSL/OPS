...
if [[ "$target" == "dev_seq" ]]; then
    ...
    echo "mpirun -np 2  $exe OPS_TILING OPS_TILING_MAXDEPTH=10"
elif [[ "$target" == "cuda" ]]; then
    echo "$exe OPS_BLOCK_SIZE_X=64 OPS_BLOCK_SIZE_Y=4"
elif [[ "$target" == "mpi_cuda" ]]; then
    echo "mpirun -np $gpu_number  $exe OPS_BLOCK_SIZE_X=64 OPS_BLOCK_SIZE_Y=4"
elif [[ "$target" == "mpi_cuda_tiled" ]]; then
    echo "mpirun -np $gpu_number  $exe OPS_BLOCK_SIZE_X=64 OPS_BLOCK_SIZE_Y=4 OPS_TILING OPS_TILING_MAXDEPTH=10"
