...
void timeloop_eqB(ACC<double> &N, const ACC<double> &workArray, const int* cx, const int* cy) {

    for (int f = 1; f < 9; f++) {
        N(f,0,0) = workArray(f,-cx[f],-cy[f]);
    }
}
...
void timeloop_eqM(const ACC<double> &ux, const ACC<double> &uy, double* energy) {

    *energy = *energy + (ux(0,0)*ux(0,0) + uy(0,0)*uy(0,0));
}
