...
// host stub function
void ops_par_loop_mgrid_populate_kernel_2(char const *name, ops_block block,
    ...
    for (int n_y = start_i; n_y < finish_i; n_y++) {
      for (int n_x = start[0]; n_x < start[0] + (end[0] - start[0]) / SIMD_VEC;
        ...
        for (int i = 0; i < SIMD_VEC; i++) {
          mgrid_populate_kernel_2((double *)p_a[0] + i * 1 * 1, arg_idx);
      ...
      for (int n_x = start[0] + ((end[0] - start[0]) / SIMD_VEC) * SIMD_VEC;
        ...
        mgrid_populate_kernel_2((double *)p_a[0], arg_idx);
