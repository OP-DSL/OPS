...
//
#define OPS_ACC0(x,y) ((n_x+arg_idx[0]%args[0].stencil->mgrid_stride[0])/args[0].stencil->mgrid_stride[0] + x + ((n_y+arg_idx[1]%args[0].stencil->mgrid_stride[1])/args[0].stencil->mgrid_stride[1]+(y))*xdim0_mgrid_prolong_kernel)
#define OPS_ACC1(x,y) (n_x*1 + x + (n_y*1+(y))*xdim1_mgrid_prolong_kernel)
...
void ops_par_loop_mgrid_prolong_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  const double * __restrict__ coarse = (double *)(args[0].data + base0);
  #ifdef OPS_MPI
  sub_dat_list sd0 = OPS_sub_dat_list[args[0].dat->index];
  coarse += arg_idx[0]/args[0].stencil->mgrid_stride[0] - sd0->decomp_disp[0] + args[0].dat->d_m[0];
  coarse += (arg_idx[1]/args[0].stencil->mgrid_stride[1] - sd0->decomp_disp[1] + args[0].dat->d_m[1])*xdim0_mgrid_prolong_kernel;
  #endif

  int base1 = args[1].dat->base_offset;
  double * __restrict__ fine = (double *)(args[1].data + base1);
  ...
  for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
    ...
    #pragma omp simd aligned(coarse,fine)
    #else
    #pragma simd
    #endif
