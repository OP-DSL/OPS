...
void ops_par_loop_mgrid_prolong_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  coarse_p += arg_idx[0] / args[0].stencil->mgrid_stride[0] -
              sd0->decomp_disp[0] + args[0].dat->d_m[0];
  coarse_p += (arg_idx[1] / args[0].stencil->mgrid_stride[1] -
...
#pragma simd
    ...
    for ( int n_x=start[0]; n_x<end[0]; n_x++ ){
      ...
      const ACC<double> coarse(
          xdim0_mgrid_prolong_kernel,
          coarse_p +
              (n_x + arg_idx[0] % args[0].stencil->mgrid_stride[0]) /
                  args[0].stencil->mgrid_stride[0] * 1 +
              (n_y + arg_idx[1] % args[0].stencil->mgrid_stride[1]) /
                  args[0].stencil->mgrid_stride[1] *
                  xdim0_mgrid_prolong_kernel * 1);
      ACC<double> fine(xdim1_mgrid_prolong_kernel,
                       fine_p + n_x * 1 + n_y * xdim1_mgrid_prolong_kernel * 1);
