...
void ops_par_loop_mgrid_prolong_kernel(char const *name, ops_block block,
  ...
  offs[0][1] = off2D(1, &start_0[0], &end_0[0], args[0].dat->size,
                     args[0].stencil->stride) -
               offs[0][0];
  ...
  for (int n_y = 0; n_y < end[1] - start[1]; n_y++) {
    for (int n_x = 0; n_x < end[0] - start[0]; n_x++) {
      ...
      mgrid_prolong_kernel(
          (double *)p_a[0] +
              (n_x + global_idx[0] % stride_0[0]) / stride_0[0] * 1 * 1 +
              (n_y + global_idx[1] % stride_0[1]) / stride_0[1] * xdim0 * 1 * 1,
