...
//
__constant__ int xdim0_mgrid_populate_kernel_3;
...
__device__
    ...
    void
    mgrid_populate_kernel_3_gpu(double *val, int *idx) {
  val[OPS_ACC0(0, 0)] = (double)(idx[0] + 24 * idx[1]);
}

#undef OPS_ACC0

__global__ void ops_mgrid_populate_kernel_3(double *__restrict arg0,
  ...
  arg0 += idx_x * 1 * 1 + idx_y * 1 * 1 * xdim0_mgrid_populate_kernel_3;
...
// host stub function
#ifndef OPS_LAZY
void ops_par_loop_mgrid_populate_kernel_3(char const *name, ops_block block,
...
void ops_par_loop_mgrid_populate_kernel_3_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != xdim0_mgrid_populate_kernel_3_h) {
    cudaMemcpyToSymbol(xdim0_mgrid_populate_kernel_3, &xdim0, sizeof(int));
  ...
  ops_mgrid_populate_kernel_3<<<grid, tblock>>>((double *)p_a[0], arg_idx[0],
