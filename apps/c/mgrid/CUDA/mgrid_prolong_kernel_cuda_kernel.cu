...
__device__
    ...
    void
    mgrid_prolong_kernel_gpu(const double *coarse, double *fine, int *idx) {
...
__global__ void ops_mgrid_prolong_kernel(const double *__restrict arg0,
                                         int stride_00, int stride_01,
                                         double *__restrict arg1, int arg_idx0,
                                         int arg_idx1, int global_idx0,
                                         int global_idx1, int size0,
                                         int size1) {
  ...
  arg0 += (idx_x + global_idx0 % stride_00) / stride_00 * 1 * 1 +
          (idx_y + global_idx1 % stride_01) / stride_01 * 1 * 1 *
...
// host stub function
#ifndef OPS_LAZY
void ops_par_loop_mgrid_prolong_kernel(char const *name, ops_block block,
...
void ops_par_loop_mgrid_prolong_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != xdim0_mgrid_prolong_kernel_h ||
      xdim1 != xdim1_mgrid_prolong_kernel_h) {
    cudaMemcpyToSymbol(xdim0_mgrid_prolong_kernel, &xdim0, sizeof(int));
    xdim0_mgrid_prolong_kernel_h = xdim0;
    cudaMemcpyToSymbol(xdim1_mgrid_prolong_kernel, &xdim1, sizeof(int));
  ...
  base0 =
      base0 +
      dat0 * args[0].dat->size[0] * (start_0[1] * args[0].stencil->stride[1]);
  ...
  ops_mgrid_prolong_kernel<<<grid, tblock>>>(
      (double *)p_a[0], stride_0[0], stride_0[1], (double *)p_a[1], arg_idx[0],
      arg_idx[1], global_idx[0], global_idx[1], x_size, y_size);
