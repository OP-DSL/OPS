...
#pragma omp distribute parallel for simd collapse(2) schedule(static, 1)
  ...
  for (int n_y = 0; n_y < y_size; n_y++) {
    ...
    for (int n_x = 0; n_x < x_size; n_x++) {
      ...
      const double *vertexy = p_a1 + base1 + n_x * 0 * 1 +
                              n_y * xdim1_generate_chunk_kernel * 1 * 1;
      double *energy0 = p_a2 + base2 + n_x * 1 * 1 +
                        n_y * xdim2_generate_chunk_kernel * 1 * 1;

      double *density0 = p_a3 + base3 + n_x * 1 * 1 +
                         n_y * xdim3_generate_chunk_kernel * 1 * 1;
      double *xvel0 = p_a4 + base4 + n_x * 1 * 1 +
                      n_y * xdim4_generate_chunk_kernel * 1 * 1;

      double *yvel0 = p_a5 + base5 + n_x * 1 * 1 +
                      n_y * xdim5_generate_chunk_kernel * 1 * 1;
      const double *cellx = p_a6 + base6 + n_x * 1 * 1 +
                            n_y * xdim6_generate_chunk_kernel * 0 * 1;

      const double *celly = p_a7 + base7 + n_x * 0 * 1 +
