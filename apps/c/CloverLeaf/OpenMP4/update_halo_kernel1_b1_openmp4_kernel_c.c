...
#pragma omp distribute parallel for simd schedule(static, 1)
  ...
  for (int i = 0; i < y_size * x_size; i++) {
    ...
    int n_y = i / x_size;
    double *density0 =
        p_a0 + base0 + n_x * 1 * 1 + n_y * xdim0_update_halo_kernel1_b1 * 1 * 1;

    double *density1 =
        p_a1 + base1 + n_x * 1 * 1 + n_y * xdim1_update_halo_kernel1_b1 * 1 * 1;
    double *energy0 =
        p_a2 + base2 + n_x * 1 * 1 + n_y * xdim2_update_halo_kernel1_b1 * 1 * 1;

    double *energy1 =
        p_a3 + base3 + n_x * 1 * 1 + n_y * xdim3_update_halo_kernel1_b1 * 1 * 1;
    double *pressure =
        p_a4 + base4 + n_x * 1 * 1 + n_y * xdim4_update_halo_kernel1_b1 * 1 * 1;

    double *viscosity =
        p_a5 + base5 + n_x * 1 * 1 + n_y * xdim5_update_halo_kernel1_b1 * 1 * 1;
    double *soundspeed =
        p_a6 + base6 + n_x * 1 * 1 + n_y * xdim6_update_halo_kernel1_b1 * 1 * 1;
