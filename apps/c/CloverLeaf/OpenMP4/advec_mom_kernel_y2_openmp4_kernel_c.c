//
// auto-generated by ops.py
//
#include "./OpenMP4/clover_leaf_common.h"

#define OPS_GPU

extern int xdim0_advec_mom_kernel_y2;
extern int xdim1_advec_mom_kernel_y2;
extern int xdim2_advec_mom_kernel_y2;
extern int xdim3_advec_mom_kernel_y2;

#undef OPS_ACC0
#undef OPS_ACC1
#undef OPS_ACC2
#undef OPS_ACC3

#define OPS_ACC0(x, y) (x + xdim0_advec_mom_kernel_y2 * (y))
#define OPS_ACC1(x, y) (x + xdim1_advec_mom_kernel_y2 * (y))
#define OPS_ACC2(x, y) (x + xdim2_advec_mom_kernel_y2 * (y))
#define OPS_ACC3(x, y) (x + xdim3_advec_mom_kernel_y2 * (y))

// user function

void advec_mom_kernel_y2_c_wrapper(double *p_a0, int base0, int tot0,
                                   double *p_a1, int base1, int tot1,
                                   double *p_a2, int base2, int tot2,
                                   double *p_a3, int base3, int tot3,
                                   int x_size, int y_size) {
#ifdef OPS_GPU

#pragma omp target teams distribute parallel for num_teams(OPS_threads)        \
    thread_limit(OPS_threads_for_block) schedule(static, 1)
#endif
  for (int i = 0; i < y_size * x_size; i++) {
    int n_x = i % x_size;
    int n_y = i / x_size;
    double *pre_vol =
        p_a0 + base0 + n_x * 1 * 1 + n_y * xdim0_advec_mom_kernel_y2 * 1 * 1;

    double *post_vol =
        p_a1 + base1 + n_x * 1 * 1 + n_y * xdim1_advec_mom_kernel_y2 * 1 * 1;
    const double *volume =
        p_a2 + base2 + n_x * 1 * 1 + n_y * xdim2_advec_mom_kernel_y2 * 1 * 1;

    const double *vol_flux_x =
        p_a3 + base3 + n_x * 1 * 1 + n_y * xdim3_advec_mom_kernel_y2 * 1 * 1;

    post_vol[OPS_ACC1(0, 0)] = volume[OPS_ACC2(0, 0)];
    pre_vol[OPS_ACC0(0, 0)] = post_vol[OPS_ACC1(0, 0)] +
                              vol_flux_x[OPS_ACC3(1, 0)] -
                              vol_flux_x[OPS_ACC3(0, 0)];
  }
}
#undef OPS_ACC0
#undef OPS_ACC1
#undef OPS_ACC2
#undef OPS_ACC3
