...
#ifdef CHECKPOINTING
  if (!ops_checkpointing_before(args, 8, range, 36))
    return;
#endif

  if (OPS_diags > 1) {
    ops_timing_realloc(36, "update_halo_kernel1_b1");
...
// ops_set_dirtybit_host(args, 8);
  ...
  if (OPS_diags > 1) {
    ...
    OPS_kernels[36].transfer += ops_compute_transfer(dim, start, end, &arg5);
    OPS_kernels[36].transfer += ops_compute_transfer(dim, start, end, &arg6);
  }
}
