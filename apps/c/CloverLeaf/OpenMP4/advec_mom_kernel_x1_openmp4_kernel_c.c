...
#pragma omp distribute parallel for simd collapse(2) schedule(static, 1)
  ...
  for (int n_y = 0; n_y < y_size; n_y++) {
    ...
    for (int n_x = 0; n_x < x_size; n_x++) {
      double *pre_vol =
          p_a0 + base0 + n_x * 1 * 1 + n_y * xdim0_advec_mom_kernel_x1 * 1 * 1;

      double *post_vol =
          p_a1 + base1 + n_x * 1 * 1 + n_y * xdim1_advec_mom_kernel_x1 * 1 * 1;
      const double *volume =
          p_a2 + base2 + n_x * 1 * 1 + n_y * xdim2_advec_mom_kernel_x1 * 1 * 1;

      const double *vol_flux_x =
          p_a3 + base3 + n_x * 1 * 1 + n_y * xdim3_advec_mom_kernel_x1 * 1 * 1;
      const double *vol_flux_y =
          p_a4 + base4 + n_x * 1 * 1 + n_y * xdim4_advec_mom_kernel_x1 * 1 * 1;
