...
#ifdef CHECKPOINTING
  if (!ops_checkpointing_before(args, 8, range, 39))
    return;
#endif

  if (OPS_diags > 1) {
    ops_timing_realloc(39, "update_halo_kernel1_l2");
...
// ops_set_dirtybit_host(args, 8);
  ...
  if (OPS_diags > 1) {
    ...
    OPS_kernels[39].transfer += ops_compute_transfer(dim, start, end, &arg3);
    OPS_kernels[39].transfer += ops_compute_transfer(dim, start, end, &arg4);
    OPS_kernels[39].transfer += ops_compute_transfer(dim, start, end, &arg5);
    OPS_kernels[39].transfer += ops_compute_transfer(dim, start, end, &arg6);
  }
}
