...
#ifdef CHECKPOINTING
  if (!ops_checkpointing_before(args, 8, range, 75))
    return;
#endif
...
// ops_set_dirtybit_host(args, 8);
  ...
  if (OPS_diags > 1) {
    ...
    OPS_kernels[75].transfer += ops_compute_transfer(dim, start, end, &arg5);
    OPS_kernels[75].transfer += ops_compute_transfer(dim, start, end, &arg6);
    OPS_kernels[75].transfer += ops_compute_transfer(dim, start, end, &arg7);
  }
}
