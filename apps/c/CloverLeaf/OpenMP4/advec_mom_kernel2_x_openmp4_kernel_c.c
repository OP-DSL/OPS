...
#pragma omp target teams distribute parallel for num_teams(OPS_threads)        \
  ...
  for (int i = 0; i < y_size * x_size; i++) {
    ...
    int n_y = i / x_size;
    double *vel1 =
        p_a0 + base0 + n_x * 1 * 1 + n_y * xdim0_advec_mom_kernel2_x * 1 * 1;

    const double *node_mass_post =
        p_a1 + base1 + n_x * 1 * 1 + n_y * xdim1_advec_mom_kernel2_x * 1 * 1;
    const double *node_mass_pre =
        p_a2 + base2 + n_x * 1 * 1 + n_y * xdim2_advec_mom_kernel2_x * 1 * 1;

    const double *mom_flux =
        p_a3 + base3 + n_x * 1 * 1 + n_y * xdim3_advec_mom_kernel2_x * 1 * 1;
