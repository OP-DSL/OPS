...
void buildOpenCLKernels_advec_mom_kernel2_y(int xdim0, int xdim1, int xdim2,
                                            int xdim3) {
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  -Dxdim0_advec_mom_kernel2_y=%d  "
                           "-Dxdim1_advec_mom_kernel2_y=%d  "
                           "-Dxdim2_advec_mom_kernel2_y=%d  "
                           "-Dxdim3_advec_mom_kernel2_y=%d ",
                pPath, 32, xdim0, xdim1, xdim2, xdim3);
      else
        sprintf(buildOpts, "-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  "
                           "-Dxdim0_advec_mom_kernel2_y=%d  "
                           "-Dxdim1_advec_mom_kernel2_y=%d  "
                           "-Dxdim2_advec_mom_kernel2_y=%d  "
                           "-Dxdim3_advec_mom_kernel2_y=%d ",
                pPath, 32, xdim0, xdim1, xdim2, xdim3);
...
void ops_par_loop_advec_mom_kernel2_y(char const *name, ops_block block,
  ...
  buildOpenCLKernels_advec_mom_kernel2_y(xdim0, xdim1, xdim2, xdim3);
