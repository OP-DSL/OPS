...
advec_cell_kernel3_ydir(const ptr_double vol_flux_y, const ptr_double pre_vol,
  ...
  sigma3 =
      (1.0 + sigmat) * (OPS_ACCS(vertexdy, 0, 0) / OPS_ACCS(vertexdy, 0, dif));
  sigma4 = 2.0 - sigmat;
  ...
  OPS_ACCS(mass_flux_y, 0, 0) =
      (OPS_ACCS(vol_flux_y, 0, 0)) * (OPS_ACCS(density1, 0, donor) + limiter);

  sigmam = fabs(OPS_ACCS(mass_flux_y, 0, 0)) /
           (OPS_ACCS(density1, 0, donor) * OPS_ACCS(pre_vol, 0, donor));
  ...
  OPS_ACCS(ener_flux, 0, 0) =
      OPS_ACCS(mass_flux_y, 0, 0) * (OPS_ACCS(energy1, 0, donor) + limiter);
}
