...
void buildOpenCLKernels_initialise_chunk_kernel_celly(int xdim0, int xdim1,
                                                      int xdim2) {
  ...
  if (!isbuilt_initialise_chunk_kernel_celly) {
    ...
    if (pPath != NULL)
      sprintf(buildOpts, "-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  "
                         "-Dxdim0_initialise_chunk_kernel_celly=%d "
                         "-Dxdim1_initialise_chunk_kernel_celly=%d "
                         "-Dxdim2_initialise_chunk_kernel_celly=%d",
    ...
    OPS_opencl_core.kernel[75] = clCreateKernel(
        OPS_opencl_core.program, "ops_initialise_chunk_kernel_celly", &ret);
    clSafeCall(ret);
...
void ops_par_loop_initialise_chunk_kernel_celly(char const *name,
  ...
  buildOpenCLKernels_initialise_chunk_kernel_celly(xdim0, xdim1, xdim2);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[75], 2, sizeof(cl_mem),
                            (void *)&arg2.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[75], 3, sizeof(cl_mem),
                            (void *)&OPS_opencl_core.constant[6]));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[75], 4, sizeof(cl_mem),
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[75], 7, sizeof(cl_int),
                            (void *)&base2));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[75], 8, sizeof(cl_int),
                            (void *)&x_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[75], 9, sizeof(cl_int),
                            (void *)&y_size));
