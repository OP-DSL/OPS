...
void buildOpenCLKernels_calc_dt_kernel_min(int xdim0) {
  ...
  if(!isbuilt_calc_dt_kernel_min) {
    ...
    printf("Compiling calc_dt_kernel_min %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_calc_dt_kernel_min=%d ", pPath, 32,xdim0);
        else
    ...
    OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[52] = clCreateKernel(OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.program, "ops_calc_dt_kernel_min", &ret);
    clSafeCall( ret );
...
void ops_par_loop_calc_dt_kernel_min(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[52], 2, nthread*sizeof(double), NULL));
    clSafeCall( clSetKernelArg(OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[52], 3, sizeof(cl_int), (void*) &r_bytes1 ));
    clSafeCall( clSetKernelArg(OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[52], 4, sizeof(cl_int), (void*) &base0 ));
