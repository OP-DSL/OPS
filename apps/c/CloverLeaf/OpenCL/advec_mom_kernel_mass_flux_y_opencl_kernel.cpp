...
void buildOpenCLKernels_advec_mom_kernel_mass_flux_y(int xdim0, int xdim1) {
  ...
  if(!isbuilt_advec_mom_kernel_mass_flux_y) {
    ...
    printf("Compiling advec_mom_kernel_mass_flux_y %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_advec_mom_kernel_mass_flux_y=%d  -Dxdim1_advec_mom_kernel_mass_flux_y=%d ", pPath, 32,xdim0,xdim1);
        else
    ...
    OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[77] = clCreateKernel(OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.program, "ops_advec_mom_kernel_mass_flux_y", &ret);
    clSafeCall( ret );
...
void ops_par_loop_advec_mom_kernel_mass_flux_y(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[77], 2, sizeof(cl_int), (void*) &base0 ));
    clSafeCall( clSetKernelArg(OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[77], 3, sizeof(cl_int), (void*) &base1 ));
    clSafeCall( clSetKernelArg(OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[77], 4, sizeof(cl_int), (void*) &x_size ));
    clSafeCall( clSetKernelArg(OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[77], 5, sizeof(cl_int), (void*) &y_size ));
