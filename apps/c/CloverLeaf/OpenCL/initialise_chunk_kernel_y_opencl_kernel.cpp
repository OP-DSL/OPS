...
void buildOpenCLKernels_initialise_chunk_kernel_y(int xdim0, int xdim1,
                                                  int xdim2) {
  ...
  if (!isbuilt_initialise_chunk_kernel_y) {
    ...
    if (pPath != NULL)
      sprintf(buildOpts, "-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  "
                         "-Dxdim0_initialise_chunk_kernel_y=%d "
                         "-Dxdim1_initialise_chunk_kernel_y=%d "
                         "-Dxdim2_initialise_chunk_kernel_y=%d",
    ...
    OPS_opencl_core.kernel[73] = clCreateKernel(
        OPS_opencl_core.program, "ops_initialise_chunk_kernel_y", &ret);
    clSafeCall(ret);
...
void ops_par_loop_initialise_chunk_kernel_y(char const *name, ops_block block,
  ...
  buildOpenCLKernels_initialise_chunk_kernel_y(xdim0, xdim1, xdim2);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[73], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[73], 2, sizeof(cl_mem),
                            (void *)&arg2.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[73], 3, sizeof(cl_mem),
                            (void *)&OPS_opencl_core.constant[6]));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[73], 4, sizeof(cl_mem),
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[73], 7, sizeof(cl_int),
                            (void *)&base2));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[73], 8, sizeof(cl_int),
                            (void *)&x_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[73], 9, sizeof(cl_int),
                            (void *)&y_size));
