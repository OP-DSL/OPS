...
void calc_dt_kernel(const ptr_double celldx, const ptr_double celldy,
                    ...
                    ptr_double dt_min, const double g_small, const double g_big,
  ...
  dtut = dtu_safe * 2.0 * OPS_ACCS(volume, 0, 0) /
         MAX(MAX(fabs(dv1), fabs(dv2)), g_small * OPS_ACCS(volume, 0, 0));
  ...
  dtvt = dtv_safe * 2.0 * OPS_ACCS(volume, 0, 0) /
         MAX(MAX(fabs(dv1), fabs(dv2)), g_small * OPS_ACCS(volume, 0, 0));
...
__kernel void ops_calc_dt_kernel(
  ...
  if (idx_x < size0 && idx_y < size1) {
    ...
    calc_dt_kernel(ptr0, ptr1, ptr2, ptr3, ptr4, ptr5, ptr6, ptr7, ptr8, ptr9,
                   ptr10, g_small, g_big, dtc_safe, dtu_safe, dtv_safe,
                   dtdiv_safe);
  }
}
