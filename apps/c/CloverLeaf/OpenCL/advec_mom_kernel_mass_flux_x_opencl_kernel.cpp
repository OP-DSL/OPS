...
void buildOpenCLKernels_advec_mom_kernel_mass_flux_x(int xdim0, int xdim1) {
  ...
  if (!isbuilt_advec_mom_kernel_mass_flux_x) {
    ...
    char *source_filename[1] = {
        (char *)"./OpenCL/advec_mom_kernel_mass_flux_x.cl"};
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_advec_mom_kernel_mass_flux_x=%d  "
                           "-Dxdim1_advec_mom_kernel_mass_flux_x=%d ",
...
void ops_par_loop_advec_mom_kernel_mass_flux_x(char const *name,
  ...
  buildOpenCLKernels_advec_mom_kernel_mass_flux_x(xdim0, xdim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[19], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[19], 2, sizeof(cl_int),
                            (void *)&base0));
