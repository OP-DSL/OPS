...
void buildOpenCLKernels_advec_mom_kernel_mass_flux_x(int xdim0, int xdim1) {
  ...
  if(!isbuilt_advec_mom_kernel_mass_flux_x) {
    ...
    printf("Compiling advec_mom_kernel_mass_flux_x %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_advec_mom_kernel_mass_flux_x=%d  -Dxdim1_advec_mom_kernel_mass_flux_x=%d ", pPath, 32,xdim0,xdim1);
        else
    ...
    OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[73] = clCreateKernel(OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.program, "ops_advec_mom_kernel_mass_flux_x", &ret);
    clSafeCall( ret );
...
void ops_par_loop_advec_mom_kernel_mass_flux_x(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[73], 2, sizeof(cl_int), (void*) &base0 ));
    clSafeCall( clSetKernelArg(OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[73], 3, sizeof(cl_int), (void*) &base1 ));
    clSafeCall( clSetKernelArg(OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[73], 4, sizeof(cl_int), (void*) &x_size ));
    clSafeCall( clSetKernelArg(OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[73], 5, sizeof(cl_int), (void*) &y_size ));
