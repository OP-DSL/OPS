...
#pragma simd
    ...
    for (int n_x = start[0]; n_x < end[0]; n_x++) {
      ACC<double> density0(xdim0_update_halo_kernel1_t2,
                           density0_p + n_x * 1 +
                               n_y * xdim0_update_halo_kernel1_t2 * 1);
      ACC<double> density1(xdim1_update_halo_kernel1_t2,
                           density1_p + n_x * 1 +
                               n_y * xdim1_update_halo_kernel1_t2 * 1);
      ACC<double> energy0(xdim2_update_halo_kernel1_t2,
                          energy0_p + n_x * 1 +
                              n_y * xdim2_update_halo_kernel1_t2 * 1);
      ACC<double> energy1(xdim3_update_halo_kernel1_t2,
                          energy1_p + n_x * 1 +
                              n_y * xdim3_update_halo_kernel1_t2 * 1);
      ACC<double> pressure(xdim4_update_halo_kernel1_t2,
                           pressure_p + n_x * 1 +
                               n_y * xdim4_update_halo_kernel1_t2 * 1);
      ACC<double> viscosity(xdim5_update_halo_kernel1_t2,
                            viscosity_p + n_x * 1 +
                                n_y * xdim5_update_halo_kernel1_t2 * 1);
      ACC<double> soundspeed(xdim6_update_halo_kernel1_t2,
                             soundspeed_p + n_x * 1 +
                                 n_y * xdim6_update_halo_kernel1_t2 * 1);

      if (fields[FIELD_DENSITY0] == 1)
        density0(0, 0) = density0(0, -3);
      if (fields[FIELD_DENSITY1] == 1)
        density1(0, 0) = density1(0, -3);
      if (fields[FIELD_ENERGY0] == 1)
        energy0(0, 0) = energy0(0, -3);
      if (fields[FIELD_ENERGY1] == 1)
        energy1(0, 0) = energy1(0, -3);
      if (fields[FIELD_PRESSURE] == 1)
        pressure(0, 0) = pressure(0, -3);
      if (fields[FIELD_VISCOSITY] == 1)
        viscosity(0, 0) = viscosity(0, -3);
      if (fields[FIELD_SOUNDSPEED] == 1)
        soundspeed(0, 0) = soundspeed(0, -3);
    }
  }
  if (OPS_diags > 1) {
    ops_timers_core(&__c2, &__t2);
