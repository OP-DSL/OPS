...
void ops_par_loop_ideal_gas_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
    ...
    #pragma omp simd aligned(density_p,energy_p,pressure_p,soundspeed_p)
    ...
    for ( int n_x=start[0]; n_x<end[0]; n_x++ ){
      const ACC<double> density(xdim0_ideal_gas_kernel, density_p + n_x*1 + n_y * xdim0_ideal_gas_kernel*1);
      const ACC<double> energy(xdim1_ideal_gas_kernel, energy_p + n_x*1 + n_y * xdim1_ideal_gas_kernel*1);
      ACC<double> pressure(xdim2_ideal_gas_kernel, pressure_p + n_x*1 + n_y * xdim2_ideal_gas_kernel*1);
      ACC<double> soundspeed(xdim3_ideal_gas_kernel, soundspeed_p + n_x*1 + n_y * xdim3_ideal_gas_kernel*1);
