...
#pragma simd
    ...
    for (int n_x = start[0]; n_x < end[0]; n_x++) {
      const ACC<double> xvel0(xdim0_calc_dt_kernel_print,
                              xvel0_p + n_x * 1 +
                                  n_y * xdim0_calc_dt_kernel_print * 1);
      const ACC<double> yvel0(xdim1_calc_dt_kernel_print,
                              yvel0_p + n_x * 1 +
                                  n_y * xdim1_calc_dt_kernel_print * 1);
      const ACC<double> density0(xdim2_calc_dt_kernel_print,
                                 density0_p + n_x * 1 +
                                     n_y * xdim2_calc_dt_kernel_print * 1);
      const ACC<double> energy0(xdim3_calc_dt_kernel_print,
                                energy0_p + n_x * 1 +
                                    n_y * xdim3_calc_dt_kernel_print * 1);
      const ACC<double> pressure(xdim4_calc_dt_kernel_print,
                                 pressure_p + n_x * 1 +
                                     n_y * xdim4_calc_dt_kernel_print * 1);
      const ACC<double> soundspeed(xdim5_calc_dt_kernel_print,
                                   soundspeed_p + n_x * 1 +
                                       n_y * xdim5_calc_dt_kernel_print * 1);
      double output[12];
