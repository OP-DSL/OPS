...
#pragma omp simd aligned(vol_flux_y_p, mass_flux_y_p)
...
#pragma simd
    ...
    for (int n_x = start[0]; n_x < end[0]; n_x++) {
      ACC<double> vol_flux_y(xdim0_update_halo_kernel4_plus_2_b,
                             vol_flux_y_p + n_x * 1 +
                                 n_y * xdim0_update_halo_kernel4_plus_2_b * 1);
      ACC<double> mass_flux_y(xdim1_update_halo_kernel4_plus_2_b,
                              mass_flux_y_p + n_x * 1 +
                                  n_y * xdim1_update_halo_kernel4_plus_2_b * 1);

      if (fields[FIELD_VOL_FLUX_Y] == 1)
        vol_flux_y(0, 0) = vol_flux_y(-2, 0);
      if (fields[FIELD_MASS_FLUX_Y] == 1)
        mass_flux_y(0, 0) = mass_flux_y(-2, 0);
    }
  }
  if (OPS_diags > 1) {
    ops_timers_core(&__c2, &__t2);
