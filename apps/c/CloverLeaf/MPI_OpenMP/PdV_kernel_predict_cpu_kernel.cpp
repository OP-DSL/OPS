...
#pragma omp simd aligned(xarea_p, xvel0_p, yarea_p, yvel0_p, volume_change_p,  \
                         volume_p, pressure_p, density0_p, density1_p,         \
                         viscosity_p, energy0_p, energy1_p)
...
#pragma simd
    ...
    for (int n_x = start[0]; n_x < end[0]; n_x++) {
      const ACC<double> xarea(xdim0_PdV_kernel_predict,
                              xarea_p + n_x * 1 +
                                  n_y * xdim0_PdV_kernel_predict * 1);
      const ACC<double> xvel0(xdim1_PdV_kernel_predict,
                              xvel0_p + n_x * 1 +
                                  n_y * xdim1_PdV_kernel_predict * 1);
      const ACC<double> yarea(xdim2_PdV_kernel_predict,
                              yarea_p + n_x * 1 +
                                  n_y * xdim2_PdV_kernel_predict * 1);
      const ACC<double> yvel0(xdim3_PdV_kernel_predict,
                              yvel0_p + n_x * 1 +
                                  n_y * xdim3_PdV_kernel_predict * 1);
      ACC<double> volume_change(xdim4_PdV_kernel_predict,
                                volume_change_p + n_x * 1 +
                                    n_y * xdim4_PdV_kernel_predict * 1);
      const ACC<double> volume(xdim5_PdV_kernel_predict,
                               volume_p + n_x * 1 +
                                   n_y * xdim5_PdV_kernel_predict * 1);
      const ACC<double> pressure(xdim6_PdV_kernel_predict,
                                 pressure_p + n_x * 1 +
                                     n_y * xdim6_PdV_kernel_predict * 1);
      const ACC<double> density0(xdim7_PdV_kernel_predict,
                                 density0_p + n_x * 1 +
                                     n_y * xdim7_PdV_kernel_predict * 1);
      ACC<double> density1(xdim8_PdV_kernel_predict,
                           density1_p + n_x * 1 +
                               n_y * xdim8_PdV_kernel_predict * 1);
      const ACC<double> viscosity(xdim9_PdV_kernel_predict,
                                  viscosity_p + n_x * 1 +
                                      n_y * xdim9_PdV_kernel_predict * 1);
      const ACC<double> energy0(xdim10_PdV_kernel_predict,
                                energy0_p + n_x * 1 +
                                    n_y * xdim10_PdV_kernel_predict * 1);
      ACC<double> energy1(xdim11_PdV_kernel_predict,
                          energy1_p + n_x * 1 +
                              n_y * xdim11_PdV_kernel_predict * 1);

      double recip_volume, energy_change;
      double right_flux, left_flux, top_flux, bottom_flux, total_flux;

      left_flux = (xarea(0, 0) *
                   (xvel0(0, 0) + xvel0(0, 1) + xvel0(0, 0) + xvel0(0, 1))) *
                  0.25 * dt * 0.5;
      right_flux = (xarea(1, 0) *
                    (xvel0(1, 0) + xvel0(1, 1) + xvel0(1, 0) + xvel0(1, 1))) *
                   0.25 * dt * 0.5;

      bottom_flux = (yarea(0, 0) *
                     (yvel0(0, 0) + yvel0(1, 0) + yvel0(0, 0) + yvel0(1, 0))) *
                    0.25 * dt * 0.5;
      top_flux = (yarea(0, 1) *
                  (yvel0(0, 1) + yvel0(1, 1) + yvel0(0, 1) + yvel0(1, 1))) *
                 0.25 * dt * 0.5;
