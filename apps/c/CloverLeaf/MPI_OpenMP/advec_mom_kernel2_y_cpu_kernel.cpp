...
#pragma simd
    ...
    for (int n_x = start[0]; n_x < end[0]; n_x++) {
      ACC<double> vel1(xdim0_advec_mom_kernel2_y,
                       vel1_p + n_x * 1 + n_y * xdim0_advec_mom_kernel2_y * 1);
      const ACC<double> node_mass_post(xdim1_advec_mom_kernel2_y,
                                       node_mass_post_p + n_x * 1 +
                                           n_y * xdim1_advec_mom_kernel2_y * 1);
      const ACC<double> node_mass_pre(xdim2_advec_mom_kernel2_y,
                                      node_mass_pre_p + n_x * 1 +
                                          n_y * xdim2_advec_mom_kernel2_y * 1);
      const ACC<double> mom_flux(xdim3_advec_mom_kernel2_y,
                                 mom_flux_p + n_x * 1 +
                                     n_y * xdim3_advec_mom_kernel2_y * 1);

      vel1(0, 0) = (vel1(0, 0) * node_mass_pre(0, 0) + mom_flux(0, -1) -
                    mom_flux(0, 0)) /
                   node_mass_post(0, 0);
    }
  }
