...
#pragma simd
    ...
    for (int n_x = start[0]; n_x < end[0]; n_x++) {
      ACC<double> vol_flux_x(xdim0_update_halo_kernel3_minus_4_b,
                             vol_flux_x_p + n_x * 1 +
                                 n_y * xdim0_update_halo_kernel3_minus_4_b * 1);
      ACC<double> mass_flux_x(xdim1_update_halo_kernel3_minus_4_b,
                              mass_flux_x_p + n_x * 1 +
                                  n_y * xdim1_update_halo_kernel3_minus_4_b *
                                      1);

      if (fields[FIELD_VOL_FLUX_X] == 1)
        vol_flux_x(0, 0) = -(vol_flux_x(-4, 0));
      if (fields[FIELD_MASS_FLUX_X] == 1)
        mass_flux_x(0, 0) = -(mass_flux_x(-4, 0));
    }
  }
  if (OPS_diags > 1) {
    ops_timers_core(&__c2, &__t2);
