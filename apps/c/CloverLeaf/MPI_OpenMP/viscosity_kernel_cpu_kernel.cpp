...
#pragma omp simd aligned(xvel0_p, yvel0_p, celldx_p, celldy_p, pressure_p,     \
                         density0_p, viscosity_p)
...
#pragma simd
    ...
    for (int n_x = start[0]; n_x < end[0]; n_x++) {
      const ACC<double> xvel0(xdim0_viscosity_kernel,
                              xvel0_p + n_x * 1 +
                                  n_y * xdim0_viscosity_kernel * 1);
      const ACC<double> yvel0(xdim1_viscosity_kernel,
                              yvel0_p + n_x * 1 +
                                  n_y * xdim1_viscosity_kernel * 1);
      const ACC<double> celldx(xdim2_viscosity_kernel,
                               celldx_p + n_x * 1 +
                                   n_y * xdim2_viscosity_kernel * 0);
      const ACC<double> celldy(xdim3_viscosity_kernel,
                               celldy_p + n_x * 0 +
                                   n_y * xdim3_viscosity_kernel * 1);
      const ACC<double> pressure(xdim4_viscosity_kernel,
                                 pressure_p + n_x * 1 +
                                     n_y * xdim4_viscosity_kernel * 1);
      const ACC<double> density0(xdim5_viscosity_kernel,
                                 density0_p + n_x * 1 +
                                     n_y * xdim5_viscosity_kernel * 1);
      ACC<double> viscosity(xdim6_viscosity_kernel,
                            viscosity_p + n_x * 1 +
      ...
      pgradx =
          (pressure(1, 0) - pressure(-1, 0)) / (celldx(0, 0) + celldx(1, 0));
      pgrady =
          (pressure(0, 1) - pressure(0, -1)) / (celldy(0, 0) + celldy(0, 1));
