...
#pragma omp simd aligned(vol_flux_y_p, yarea_p, yvel0_p, yvel1_p)
...
#pragma simd
    ...
    for (int n_x = start[0]; n_x < end[0]; n_x++) {
      ACC<double> vol_flux_y(xdim0_flux_calc_kernely,
                             vol_flux_y_p + n_x * 1 +
                                 n_y * xdim0_flux_calc_kernely * 1);
      const ACC<double> yarea(xdim1_flux_calc_kernely,
                              yarea_p + n_x * 1 +
                                  n_y * xdim1_flux_calc_kernely * 1);
      const ACC<double> yvel0(xdim2_flux_calc_kernely,
                              yvel0_p + n_x * 1 +
                                  n_y * xdim2_flux_calc_kernely * 1);
      const ACC<double> yvel1(xdim3_flux_calc_kernely,
                              yvel1_p + n_x * 1 +
                                  n_y * xdim3_flux_calc_kernely * 1);
