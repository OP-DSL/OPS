...
#pragma omp simd reduction(+:p_a2_0) reduction(+:p_a3_0) aligned(cellx_p,celly_p)
...
#pragma simd
    ...
    for (int n_x = start[0]; n_x < end[0]; n_x++) {
      const ACC<double> cellx(xdim0_calc_dt_kernel_get,
                              cellx_p + n_x * 1 +
                                  n_y * xdim0_calc_dt_kernel_get * 0);
      const ACC<double> celly(xdim1_calc_dt_kernel_get,
                              celly_p + n_x * 0 +
                                  n_y * xdim1_calc_dt_kernel_get * 1);
      double xl_pos[1];
