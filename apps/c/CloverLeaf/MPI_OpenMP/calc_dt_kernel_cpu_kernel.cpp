...
#pragma omp simd aligned(celldx_p, celldy_p, soundspeed_p, viscosity_p,        \
                         density0_p, xvel0_p, xarea_p, volume_p, yvel0_p,      \
                         yarea_p, dt_min_p)
...
#pragma simd
    ...
    for (int n_x = start[0]; n_x < end[0]; n_x++) {
      const ACC<double> celldx(xdim0_calc_dt_kernel,
                               celldx_p + n_x * 1 +
                                   n_y * xdim0_calc_dt_kernel * 0);
      const ACC<double> celldy(xdim1_calc_dt_kernel,
                               celldy_p + n_x * 0 +
                                   n_y * xdim1_calc_dt_kernel * 1);
      const ACC<double> soundspeed(xdim2_calc_dt_kernel,
                                   soundspeed_p + n_x * 1 +
                                       n_y * xdim2_calc_dt_kernel * 1);
      const ACC<double> viscosity(xdim3_calc_dt_kernel,
                                  viscosity_p + n_x * 1 +
                                      n_y * xdim3_calc_dt_kernel * 1);
      const ACC<double> density0(xdim4_calc_dt_kernel,
                                 density0_p + n_x * 1 +
                                     n_y * xdim4_calc_dt_kernel * 1);
      const ACC<double> xvel0(xdim5_calc_dt_kernel,
                              xvel0_p + n_x * 1 +
                                  n_y * xdim5_calc_dt_kernel * 1);
      const ACC<double> xarea(xdim6_calc_dt_kernel,
                              xarea_p + n_x * 1 +
                                  n_y * xdim6_calc_dt_kernel * 1);
      const ACC<double> volume(xdim7_calc_dt_kernel,
                               volume_p + n_x * 1 +
                                   n_y * xdim7_calc_dt_kernel * 1);
      const ACC<double> yvel0(xdim8_calc_dt_kernel,
                              yvel0_p + n_x * 1 +
                                  n_y * xdim8_calc_dt_kernel * 1);
      const ACC<double> yarea(xdim9_calc_dt_kernel,
                              yarea_p + n_x * 1 +
                                  n_y * xdim9_calc_dt_kernel * 1);
      ACC<double> dt_min(xdim10_calc_dt_kernel,
      ...
      dtut = dtu_safe * 2.0 * volume(0, 0) /
             MAX(MAX(fabs(dv1), fabs(dv2)), g_small * volume(0, 0));
