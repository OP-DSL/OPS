...
#pragma omp simd reduction(+:p_a6_0) reduction(+:p_a7_0) reduction(+:p_a8_0) reduction(+:p_a9_0) reduction(+:p_a10_0) aligned(volume_p,density0_p,energy0_p,pressure_p,xvel0_p,yvel0_p)
...
#pragma simd
    ...
    for (int n_x = start[0]; n_x < end[0]; n_x++) {
      ...
      const ACC<double> density0(xdim1_field_summary_kernel,
                                 density0_p + n_x * 1 +
                                     n_y * xdim1_field_summary_kernel * 1);
      const ACC<double> energy0(xdim2_field_summary_kernel,
                                energy0_p + n_x * 1 +
                                    n_y * xdim2_field_summary_kernel * 1);
      const ACC<double> pressure(xdim3_field_summary_kernel,
                                 pressure_p + n_x * 1 +
                                     n_y * xdim3_field_summary_kernel * 1);
      const ACC<double> xvel0(xdim4_field_summary_kernel,
                              xvel0_p + n_x * 1 +
                                  n_y * xdim4_field_summary_kernel * 1);
      const ACC<double> yvel0(xdim5_field_summary_kernel,
                              yvel0_p + n_x * 1 +
                                  n_y * xdim5_field_summary_kernel * 1);
      double vol[1];
      vol[0] = ZERO_double;
      double mass[1];
      mass[0] = ZERO_double;
      ...
      vsqrd = 0.0;
      vsqrd = vsqrd +
              0.25 * (xvel0(0, 0) * xvel0(0, 0) + yvel0(0, 0) * yvel0(0, 0));
      vsqrd = vsqrd +
              0.25 * (xvel0(1, 0) * xvel0(1, 0) + yvel0(1, 0) * yvel0(1, 0));
      vsqrd = vsqrd +
              0.25 * (xvel0(0, 1) * xvel0(0, 1) + yvel0(0, 1) * yvel0(0, 1));
      vsqrd = vsqrd +
              0.25 * (xvel0(1, 1) * xvel0(1, 1) + yvel0(1, 1) * yvel0(1, 1));
