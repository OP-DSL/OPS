...
void ops_par_loop_advec_mom_kernel1_x_nonvector_execute(ops_kernel_descriptor *desc) {
  ...
  for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
    for ( int n_x=start[0]; n_x<end[0]; n_x++ ){
      advec_mom_kernel1_x_nonvector(  p_a0 + n_x*1*1 + n_y*xdim0*1*1,
           p_a1 + n_x*1*1 + n_y*xdim1*1*1, p_a2 + n_x*1*1 + n_y*xdim2*1*1, p_a3 + n_x*1*1 + n_y*xdim3*0*1, p_a4 + n_x*1*1 + n_y*xdim4*1*1 );
