...
// user function
inline void advec_mom_kernel_post_pre_advec_x(ACC<double> &node_mass_post,
  ...
  node_mass_post(0, 0) =
      0.25 *
      (density1(0, -1) * post_vol(0, -1) + density1(0, 0) * post_vol(0, 0) +
       density1(-1, -1) * post_vol(-1, -1) + density1(-1, 0) * post_vol(-1, 0));
...
#pragma simd
    ...
    for (int n_x = start[0] + ((end[0] - start[0]) / SIMD_VEC) * SIMD_VEC;
      ...
      advec_mom_kernel_post_pre_advec_x((double *)p_a[0], (double *)p_a[1],
