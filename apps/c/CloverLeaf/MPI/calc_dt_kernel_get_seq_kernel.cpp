...
// user function
inline void calc_dt_kernel_get(const ACC<double> &cellx,
                               const ACC<double> &celly, double *xl_pos,
                               double *yl_pos) {
  *xl_pos = cellx(0, 0);
  *yl_pos = celly(0, 0);
}
...
#pragma novector
    for (n_x = start[0];
      ...
      for (int i = 0; i < SIMD_VEC; i++) {
        calc_dt_kernel_get((double *)p_a[0] + i * 1 * 1,
                           (double *)p_a[1] + i * 0 * 1, (double *)p_a[2],
                           (double *)p_a[3]);
      }
