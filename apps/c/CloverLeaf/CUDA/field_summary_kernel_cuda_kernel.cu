...
#define OPS_ACC4(x, y) (x + dims_field_summary_kernel[4][0] * (y))
#define OPS_ACC5(x, y) (x + dims_field_summary_kernel[5][0] * (y))
...
__device__
    ...
    field_summary_kernel_gpu(const double *volume, const double *density0,
                             ...
                             double *vol, double *mass, double *ie, double *ke,
                             double *press) {
...
__global__ void ops_field_summary_kernel(
    ...
    double *__restrict arg9, double *__restrict arg10, int size0, int size1) {
  ...
  if (idx_x < size0 && idx_y < size1) {
    field_summary_kernel_gpu(arg0, arg1, arg2, arg3, arg4, arg5, arg6_l, arg7_l,
                             arg8_l, arg9_l, arg10_l);
  }
  for (int d = 0; d < 1; d++)
  ...
  for (int d = 0; d < 1; d++)
    ops_reduction_cuda<OPS_INC>(
        &arg8[d + (blockIdx.x + blockIdx.y * gridDim.x) * 1], arg8_l[d]);
  for (int d = 0; d < 1; d++)
    ops_reduction_cuda<OPS_INC>(
        &arg9[d + (blockIdx.x + blockIdx.y * gridDim.x) * 1], arg9_l[d]);
  for (int d = 0; d < 1; d++)
    ops_reduction_cuda<OPS_INC>(
        &arg10[d + (blockIdx.x + blockIdx.y * gridDim.x) * 1], arg10_l[d]);
}
...
void ops_par_loop_field_summary_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (x_size > 0 && y_size > 0)
    ops_field_summary_kernel<<<grid, tblock, nshared>>>(
        ...
        (double *)p_a[4], (double *)p_a[5], (double *)arg6.data_d,
