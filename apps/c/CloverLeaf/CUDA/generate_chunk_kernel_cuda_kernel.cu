...
void ops_par_loop_generate_chunk_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (OPS_instance::getOPSInstance()->OPS_diags > 1) {
    ...
    OPS_instance::getOPSInstance()->OPS_kernels[7].transfer += ops_compute_transfer(dim, start, end, &arg6);
    OPS_instance::getOPSInstance()->OPS_kernels[7].transfer += ops_compute_transfer(dim, start, end, &arg7);
  }
}
