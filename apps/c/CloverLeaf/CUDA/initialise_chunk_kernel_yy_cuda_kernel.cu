...
#else
#ifdef OPS_HYBRID
void ops_par_loop_initialise_chunk_kernel_yy_execute_gpu(ops_kernel_descriptor *desc) {
#else
void ops_par_loop_initialise_chunk_kernel_yy_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != xdim0_initialise_chunk_kernel_yy_h) {
    cudaMemcpyToSymbolAsync( xdim0_initialise_chunk_kernel_yy, &xdim0, sizeof(int),0 );
...
#ifdef OPS_HYBRID
void ops_par_loop_initialise_chunk_kernel_yy_execute_cpu(ops_kernel_descriptor *desc);
void ops_par_loop_initialise_chunk_kernel_yy_execute(ops_kernel_descriptor *desc) {
  if (desc->device == 1) {
    ops_par_loop_initialise_chunk_kernel_yy_execute_gpu(desc);
  }
  else {
    ops_par_loop_initialise_chunk_kernel_yy_execute_cpu(desc);
  }
}
#endif
