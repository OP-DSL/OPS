...
cd -
cd $OPS_INSTALL_PATH/../apps/c/adi_burger
make clean
...
mv adi_burger2D.h5 adi_burger2D_dev_seq.h5
rm perf_out
...
mv adi_burger2D.h5 adi_burger2D_seq.h5
grep "Total Wall time" perf_out
$HDF5_INSTALL_PATH/bin/h5diff -p $TOL adi_burger2D_dev_seq.h5 adi_burger2D_seq.h5 > diff_out
if [ -s ./diff_out ]
then
...
mv adi_burger2D.h5 adi_burger2D_omp.h5
grep "Total Wall time" perf_out
$HDF5_INSTALL_PATH/bin/h5diff -p $TOL adi_burger2D_dev_seq.h5 adi_burger2D_omp.h5 > diff_out
if [ -s ./diff_out ]
then
    echo "File not empty - Solution Not Valid";exit 1;
else
...
mv adi_burger2D.h5 adi_burger2D_cuda.h5
grep "Total Wall time" perf_out
$HDF5_INSTALL_PATH/bin/h5diff -p $TOL adi_burger2D_dev_seq.h5 adi_burger2D_cuda.h5 > diff_out
if [ -s ./diff_out ]
then
    echo "File not empty - Solution Not Valid";exit 1;
else
...
mv adi_burger2D.h5 adi_burger2D_mpi.h5
grep "Total Wall time" perf_out
$HDF5_INSTALL_PATH/bin/h5diff -p $TOL adi_burger2D_dev_seq.h5 adi_burger2D_mpi.h5 > diff_out
if [ -s ./diff_out ]
then
    echo "File not empty - Solution Not Valid";exit 1;
else
...
mv adi_burger2D.h5 adi_burger2D_mpi_cuda.h5
grep "Total Wall time" perf_out
$HDF5_INSTALL_PATH/bin/h5diff -p $TOL adi_burger2D_dev_seq.h5 adi_burger2D_mpi_cuda.h5 > diff_out
if [ -s ./diff_out ]
then
    echo "File not empty - Solution Not Valid";exit 1;
else
...
mv adi_burger2D.h5 adi_burger2D_mpi_cuda.h5
grep "Total Wall time" perf_out
$HDF5_INSTALL_PATH/bin/h5diff -p $TOL ./adi_burger2D_seq.h5 adi_burger2D_mpi_cuda.h5 > diff_out
if [ -s ./diff_out ]
then
    echo "File not empty - Solution Not Valid";exit 1;
else
