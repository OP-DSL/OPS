...
int main(int argc, char **argv)
{
  ...
  for (int i = 0; i < argc; i++) {
    if (strcmp(argv[i], "--max_cp") == 0) {
      num_max_cp = atoi(argv[i+1]);
    }
  }
  ...
  // set number of used checkpoints for adjoints
  if (num_max_cp == 0) {
    num_max_cp = end_step/3;
  }
  ops_ad_set_checkpoint_count(num_max_cp, end_step+1);

  start();
  printf("OPS_AD num_max_cp = %d\n", num_max_cp);
  /***************************************************************************
  **-----------------------------hydro loop---------------------------------**
  ***************************************************************************/
  double ct0, ct1, et0, et1;
  ops_timers(&ct0, &et0);
  ops_ad_manual_checkpoint_datlist(density0, energy0, xvel0, yvel0, xvel1, yvel1);
  ...
  while(1) {
    ...
    ops_ad_manual_checkpoint_datlist(density1, energy1, xvel1, yvel1);
  ...
  // TODO output some result of derivatives
  ops_device_sync(OPS_instance::getOPSInstance());
  double adjoint_end;
  ops_timers(&ct1, &adjoint_end);

  ops_timing_output(std::cout); // print output to STDOUT
  //ops_timing_output(g_out);
  process_profile();

  ops_printf("\nTotal Wall time %lf\n", et1-et0 + adjoint_end - et1);
