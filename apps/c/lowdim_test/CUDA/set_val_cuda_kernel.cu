...
#define OPS_ACC0(x, y, z)                                                      \
  (x + dims_set_val[0][0] * (y) + dims_set_val[0][0] * dims_set_val[0][1] * (z))
...
__global__ void ops_set_val(double *__restrict arg0, const double arg1,
  ...
  arg0 += idx_x * 1 * 1 + idx_y * 1 * 1 * dims_set_val[0][0] +
          idx_z * 1 * 1 * dims_set_val[0][0] * dims_set_val[0][1];
...
void ops_par_loop_set_val_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_set_val_h[0][0] || ydim0 != dims_set_val_h[0][1]) {
    ...
    cutilSafeCall(
        cudaMemcpyToSymbol(dims_set_val, dims_set_val_h, sizeof(dims_set_val)));
  }
