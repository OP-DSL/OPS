...
__device__
    ...
    void
    tea_leaf_norm2_kernel_gpu(const double *x, double *norm) {
  *norm = *norm + x[OPS_ACC0(0, 0)] * x[OPS_ACC0(0, 0)];
}

#undef OPS_ACC0

__global__ void ops_tea_leaf_norm2_kernel(const double *__restrict arg0,
...
void ops_par_loop_tea_leaf_norm2_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_tea_leaf_norm2_kernel_h[0][0]) {
    ...
    cutilSafeCall(cudaMemcpyToSymbol(dims_tea_leaf_norm2_kernel,
                                     dims_tea_leaf_norm2_kernel_h,
                                     sizeof(dims_tea_leaf_norm2_kernel)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    ops_tea_leaf_norm2_kernel<<<grid, tblock, nshared>>>(
