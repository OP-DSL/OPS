...
#define OPS_ACC0(x, y) (x + dims_initialise_chunk_kernel_celly[0][0] * (y))
#define OPS_ACC1(x, y) (x + dims_initialise_chunk_kernel_celly[1][0] * (y))
#define OPS_ACC2(x, y) (x + dims_initialise_chunk_kernel_celly[2][0] * (y))
...
__global__ void ops_initialise_chunk_kernel_celly(const double *__restrict arg0,
  ...
  arg0 +=
      idx_x * 0 * 1 + idx_y * 1 * 1 * dims_initialise_chunk_kernel_celly[0][0];
  arg1 +=
      idx_x * 0 * 1 + idx_y * 1 * 1 * dims_initialise_chunk_kernel_celly[1][0];
  arg2 +=
      idx_x * 0 * 1 + idx_y * 1 * 1 * dims_initialise_chunk_kernel_celly[2][0];
...
void ops_par_loop_initialise_chunk_kernel_celly_execute(
  ...
  if (!ops_checkpointing_before(args, 3, range, 13))
    return;
#endif
