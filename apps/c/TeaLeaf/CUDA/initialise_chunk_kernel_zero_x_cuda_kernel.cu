...
#define OPS_ACC0(x, y) (x + dims_initialise_chunk_kernel_zero_x[0][0] * (y))

// user function
__device__

    void
    initialise_chunk_kernel_zero_x_gpu(double *var) {
  *var = 0.0;
}

#undef OPS_ACC0

__global__ void ops_initialise_chunk_kernel_zero_x(double *__restrict arg0,
                                                   int size0, int size1) {
  ...
  arg0 +=
      idx_x * 1 * 1 + idx_y * 0 * 1 * dims_initialise_chunk_kernel_zero_x[0][0];

  if (idx_x < size0 && idx_y < size1) {
    initialise_chunk_kernel_zero_x_gpu(arg0);
  }
}
...
#else
void ops_par_loop_initialise_chunk_kernel_zero_x_execute(
  ...
  if (xdim0 != dims_initialise_chunk_kernel_zero_x_h[0][0]) {
    ...
    cutilSafeCall(
        cudaMemcpyToSymbol(dims_initialise_chunk_kernel_zero_x,
                           dims_initialise_chunk_kernel_zero_x_h,
                           sizeof(dims_initialise_chunk_kernel_zero_x)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    ops_initialise_chunk_kernel_zero_x<<<grid, tblock>>>((double *)p_a[0],
                                                         x_size, y_size);
