//
// auto-generated by ops.py
//

#define OPS_GPU

int xdim0_tea_leaf_jacobi_kernel;
int xdim1_tea_leaf_jacobi_kernel;
int xdim2_tea_leaf_jacobi_kernel;
int xdim3_tea_leaf_jacobi_kernel;
int xdim4_tea_leaf_jacobi_kernel;


#undef OPS_ACC0
#undef OPS_ACC1
#undef OPS_ACC2
#undef OPS_ACC3
#undef OPS_ACC4


#define OPS_ACC0(x,y) (x+xdim0_tea_leaf_jacobi_kernel*(y))
#define OPS_ACC1(x,y) (x+xdim1_tea_leaf_jacobi_kernel*(y))
#define OPS_ACC2(x,y) (x+xdim2_tea_leaf_jacobi_kernel*(y))
#define OPS_ACC3(x,y) (x+xdim3_tea_leaf_jacobi_kernel*(y))
#define OPS_ACC4(x,y) (x+xdim4_tea_leaf_jacobi_kernel*(y))

//user function
inline 
void tea_leaf_jacobi_kernel(double *u1, const double *Kx, const double *Ky,
		const double *un,const double *u0,const double *rx,const double *ry, double *error) {
	u1[OPS_ACC0(0,0)] = (u0[OPS_ACC4(0,0)]
		+ (*rx)*(Kx[OPS_ACC1(1, 0)] *un[OPS_ACC3(1, 0)] + Kx[OPS_ACC1(0,0)]*un[OPS_ACC3(-1, 0)])
		+ (*ry)*(Ky[OPS_ACC2(0, 1)] *un[OPS_ACC3(0, 1)] + Ky[OPS_ACC2(0,0)]*un[OPS_ACC3(0, -1)]))
			/(1.0
				+ (*rx)*(Kx[OPS_ACC1(1, 0)] + Kx[OPS_ACC1(0,0)])
				+ (*ry)*(Ky[OPS_ACC2(0, 1)] + Ky[OPS_ACC2(0,0)]));

    *error = *error + fabs(u1[OPS_ACC0(0,0)] - un[OPS_ACC3(0,0)]);
}


#undef OPS_ACC0
#undef OPS_ACC1
#undef OPS_ACC2
#undef OPS_ACC3
#undef OPS_ACC4



void tea_leaf_jacobi_kernel_c_wrapper(
  double *p_a0,
  double *p_a1,
  double *p_a2,
  double *p_a3,
  double *p_a4,
  double p_a5,
  double p_a6,
  double *p_a7,
  int x_size, int y_size) {
  double p_a7_0 = p_a7[0];
  #ifdef OPS_GPU
  #pragma acc parallel deviceptr(p_a0,p_a1,p_a2,p_a3,p_a4) reduction(+:p_a7_0)
  #pragma acc loop reduction(+:p_a7_0)
  #endif
  for ( int n_y=0; n_y<y_size; n_y++ ){
    #ifdef OPS_GPU
    #pragma acc loop reduction(+:p_a7_0)
    #endif
    for ( int n_x=0; n_x<x_size; n_x++ ){
      tea_leaf_jacobi_kernel(  p_a0 + n_x*1*1 + n_y*xdim0_tea_leaf_jacobi_kernel*1*1,
           p_a1 + n_x*1*1 + n_y*xdim1_tea_leaf_jacobi_kernel*1*1, p_a2 + n_x*1*1 + n_y*xdim2_tea_leaf_jacobi_kernel*1*1,
           p_a3 + n_x*1*1 + n_y*xdim3_tea_leaf_jacobi_kernel*1*1, p_a4 + n_x*1*1 + n_y*xdim4_tea_leaf_jacobi_kernel*1*1,
           &p_a5, &p_a6,
           &p_a7_0 );

    }
  }
  p_a7[0] = p_a7_0;
}
