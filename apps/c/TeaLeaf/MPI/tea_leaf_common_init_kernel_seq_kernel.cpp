...
//user function
inline void tea_leaf_common_init_kernel(double *w, double *r, const double *Kx, const double *Ky,
...
void ops_par_loop_tea_leaf_common_init_kernel(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4, ops_arg arg5, ops_arg arg6) {
  ...
  #ifdef CHECKPOINTING
  if (!ops_checkpointing_before(args,7,range,29)) return;
  #endif

  if (OPS_diags > 1) {
    ops_timing_realloc(29,"tea_leaf_common_init_kernel");
  ...
  for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
    ...
    for( n_x=start[0]; n_x<start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x+=SIMD_VEC ) {
      ...
      for ( int i=0; i<SIMD_VEC; i++ ){
        tea_leaf_common_init_kernel(  (double *)p_a[0]+ i*1*1, (double *)p_a[1]+ i*1*1, (double *)p_a[2]+ i*1*1,
           (double *)p_a[3]+ i*1*1, (double *)p_a[4]+ i*1*1, (double *)p_a[5], (double *)p_a[6] );
    ...
    for ( int n_x=start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x<end[0]; n_x++ ){
      ...
      tea_leaf_common_init_kernel(  (double *)p_a[0], (double *)p_a[1], (double *)p_a[2],
  ...
  if (OPS_diags > 1) {
    ...
    OPS_kernels[29].transfer += ops_compute_transfer(dim, start, end, &arg3);
    OPS_kernels[29].transfer += ops_compute_transfer(dim, start, end, &arg4);
  }
}
