...
//user function
inline void tea_leaf_common_init_diag_init_kernel(double *Mi, const double *Kx, const double *Ky,
...
void ops_par_loop_tea_leaf_common_init_diag_init_kernel(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4) {
  ...
  #ifdef CHECKPOINTING
  if (!ops_checkpointing_before(args,5,range,38)) return;
  #endif

  if (OPS_diags > 1) {
    ops_timing_realloc(38,"tea_leaf_common_init_diag_init_kernel");
  ...
  for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
    ...
    for( n_x=start[0]; n_x<start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x+=SIMD_VEC ) {
      ...
      for ( int i=0; i<SIMD_VEC; i++ ){
        tea_leaf_common_init_diag_init_kernel(  (double *)p_a[0]+ i*1*1, (double *)p_a[1]+ i*1*1, (double *)p_a[2]+ i*1*1,
    ...
    for ( int n_x=start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x<end[0]; n_x++ ){
      ...
      tea_leaf_common_init_diag_init_kernel(  (double *)p_a[0], (double *)p_a[1], (double *)p_a[2],
           (double *)p_a[3], (double *)p_a[4] );
