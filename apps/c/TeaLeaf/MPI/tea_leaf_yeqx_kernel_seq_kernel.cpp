...
//user function
inline void tea_leaf_yeqx_kernel (double * p, const double * x) {
...
void ops_par_loop_tea_leaf_yeqx_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  if (OPS_diags > 1) {
    ops_timing_realloc(31,"tea_leaf_yeqx_kernel");
  ...
  for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
    ...
    for( n_x=start[0]; n_x<start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x+=SIMD_VEC ) {
      ...
      for ( int i=0; i<SIMD_VEC; i++ ){
        tea_leaf_yeqx_kernel(  (double *)p_a[0]+ i*1*1, (double *)p_a[1]+ i*1*1 );
    ...
    for ( int n_x=start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x<end[0]; n_x++ ){
      ...
      tea_leaf_yeqx_kernel(  (double *)p_a[0], (double *)p_a[1] );
