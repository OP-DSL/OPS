...
#pragma simd
    ...
    for ( int n_x=start[0]; n_x<end[0]; n_x++ ){
      ACC<double> r(xdim0_tea_leaf_common_residual_kernel,
                    r_p + n_x * 1 +
                        n_y * xdim0_tea_leaf_common_residual_kernel * 1);
      const ACC<double> Kx(xdim1_tea_leaf_common_residual_kernel,
                           Kx_p + n_x * 1 +
                               n_y * xdim1_tea_leaf_common_residual_kernel * 1);
      const ACC<double> Ky(xdim2_tea_leaf_common_residual_kernel,
                           Ky_p + n_x * 1 +
                               n_y * xdim2_tea_leaf_common_residual_kernel * 1);
      const ACC<double> u(xdim3_tea_leaf_common_residual_kernel,
                          u_p + n_x * 1 +
                              n_y * xdim3_tea_leaf_common_residual_kernel * 1);
      const ACC<double> u0(xdim4_tea_leaf_common_residual_kernel,
                           u0_p + n_x * 1 +
                               n_y * xdim4_tea_leaf_common_residual_kernel * 1);

      double smvp = 0.0;
      smvp = (1.0 + (*ry) * (Ky(0, 1) + Ky(0, 0)) +
              (*rx) * (Kx(1, 0) + Kx(0, 0))) *
                 u(0, 0) -
