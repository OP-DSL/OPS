...
void tea_leaf_dot_kernel_c_wrapper(
  ...
  for ( int n_y=0; n_y<y_size; n_y++ ){
    for ( int n_x=0; n_x<x_size; n_x++ ){
      double rro[1];
      rro[0] = ZERO_double;
      const ptr_double r = { r_p + n_x*1 + n_y * xdim0_tea_leaf_dot_kernel*1, xdim0_tea_leaf_dot_kernel};
      const ptr_double p = { p_p + n_x*1 + n_y * xdim1_tea_leaf_dot_kernel*1, xdim1_tea_leaf_dot_kernel};
