...
void tea_leaf_xpy_kernel_c_wrapper(double *p_a0, int base0, int tot0,
...
#pragma omp distribute parallel for simd collapse(2) schedule(static, 1)
  ...
  for (int n_y = 0; n_y < y_size; n_y++) {
    ...
    for (int n_x = 0; n_x < x_size; n_x++) {
      double *u =
          p_a0 + base0 + n_x * 1 * 1 + n_y * xdim0_tea_leaf_xpy_kernel * 1 * 1;

      const double *p =
          p_a1 + base1 + n_x * 1 * 1 + n_y * xdim1_tea_leaf_xpy_kernel * 1 * 1;
