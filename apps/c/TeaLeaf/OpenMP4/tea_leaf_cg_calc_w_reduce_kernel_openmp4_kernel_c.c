...
void tea_leaf_cg_calc_w_reduce_kernel_c_wrapper(
    ...
    double p_a4, double p_a5, double *p_a6, int x_size, int y_size) {
...
#pragma omp target map(                                                        \
    to : p_a0[0 : tot0], p_a1[0 : tot1], p_a2[0 : tot2], p_a3[0 : tot3])       \
                                             map(tofrom : p_a6_0)
#pragma omp teams num_teams(num_blocks) thread_limit(128) reduction(+ : p_a6_0)
#pragma omp distribute parallel for simd collapse(2)                           \
    schedule(static, 1) reduction(+ : p_a6_0)
#endif
  for (int n_y = 0; n_y < y_size; n_y++) {
    ...
    for (int n_x = 0; n_x < x_size; n_x++) {
      ...
      const double *p = p_a3 + base3 + n_x * 1 * 1 +
