...
#pragma omp target teams distribute parallel for num_teams(OPS_threads)        \
    thread_limit(OPS_threads_for_block)                                        \
        schedule(static, 1) map(tofrom : p_a7_0) reduction(+ : p_a7_0)
#endif
  for (int i = 0; i < y_size * x_size; i++) {
    int n_x = i % x_size;
    int n_y = i / x_size;
    double *u1 =
        p_a0 + base0 + n_x * 1 * 1 + n_y * xdim0_tea_leaf_jacobi_kernel * 1 * 1;

    const double *Kx =
        p_a1 + base1 + n_x * 1 * 1 + n_y * xdim1_tea_leaf_jacobi_kernel * 1 * 1;
    const double *Ky =
        p_a2 + base2 + n_x * 1 * 1 + n_y * xdim2_tea_leaf_jacobi_kernel * 1 * 1;

    const double *un =
        p_a3 + base3 + n_x * 1 * 1 + n_y * xdim3_tea_leaf_jacobi_kernel * 1 * 1;
    const double *u0 =
        p_a4 + base4 + n_x * 1 * 1 + n_y * xdim4_tea_leaf_jacobi_kernel * 1 * 1;
