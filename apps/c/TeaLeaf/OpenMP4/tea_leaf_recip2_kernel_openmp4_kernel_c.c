...
void tea_leaf_recip2_kernel_c_wrapper(double *p_a0, int base0, int tot0,
...
#pragma omp target teams distribute parallel for num_teams(OPS_threads)        \
  ...
  for (int i = 0; i < y_size * x_size; i++) {
    ...
    int n_y = i / x_size;
    double *z =
        p_a0 + base0 + n_x * 1 * 1 + n_y * xdim0_tea_leaf_recip2_kernel * 1 * 1;

    const double *x =
        p_a1 + base1 + n_x * 1 * 1 + n_y * xdim1_tea_leaf_recip2_kernel * 1 * 1;
    const double *y =
        p_a2 + base2 + n_x * 1 * 1 + n_y * xdim2_tea_leaf_recip2_kernel * 1 * 1;
