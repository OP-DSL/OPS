...
void buildOpenCLKernels_tea_leaf_yeqx_kernel(int xdim0, int xdim1) {
  ...
  if (!isbuilt_tea_leaf_yeqx_kernel) {
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  -Dxdim0_tea_leaf_yeqx_kernel=%d "
                           " -Dxdim1_tea_leaf_yeqx_kernel=%d ",
                pPath, 32, xdim0, xdim1);
      else
        sprintf(buildOpts, "-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  "
                           "-Dxdim0_tea_leaf_yeqx_kernel=%d  "
                           "-Dxdim1_tea_leaf_yeqx_kernel=%d ",
                pPath, 32, xdim0, xdim1);
    else {
    ...
    OPS_opencl_core.kernel[30] = clCreateKernel(
        OPS_opencl_core.program, "ops_tea_leaf_yeqx_kernel", &ret);
    clSafeCall(ret);
...
void ops_par_loop_tea_leaf_yeqx_kernel(char const *name, ops_block block,
  ...
  buildOpenCLKernels_tea_leaf_yeqx_kernel(xdim0, xdim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[30], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[30], 2, sizeof(cl_int),
                            (void *)&base0));
