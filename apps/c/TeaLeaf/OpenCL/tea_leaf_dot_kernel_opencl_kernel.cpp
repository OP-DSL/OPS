...
void buildOpenCLKernels_tea_leaf_dot_kernel(int xdim0, int xdim1) {
  ...
  if (!isbuilt_tea_leaf_dot_kernel) {
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  -Dxdim0_tea_leaf_dot_kernel=%d  "
                           "-Dxdim1_tea_leaf_dot_kernel=%d ",
                pPath, 32, xdim0, xdim1);
      else
        sprintf(buildOpts, "-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  "
                           "-Dxdim0_tea_leaf_dot_kernel=%d  "
                           "-Dxdim1_tea_leaf_dot_kernel=%d ",
                pPath, 32, xdim0, xdim1);
    else {
    ...
    OPS_opencl_core.kernel[18] = clCreateKernel(
        OPS_opencl_core.program, "ops_tea_leaf_dot_kernel", &ret);
    clSafeCall(ret);
...
// host stub function
void ops_par_loop_tea_leaf_dot_kernel(char const *name, ops_block block,
...
#ifdef CHECKPOINTING
  if (!ops_checkpointing_before(args, 3, range, 18))
    return;
#endif
  ...
  buildOpenCLKernels_tea_leaf_dot_kernel(xdim0, xdim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[18], 2, sizeof(cl_mem),
                            (void *)&arg2.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[18], 3,
                            nthread * sizeof(double), NULL));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[18], 4, sizeof(cl_int),
                            (void *)&r_bytes2));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[18], 5, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[18], 6, sizeof(cl_int),
                            (void *)&base1));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[18], 7, sizeof(cl_int),
                            (void *)&x_size));
