...
void buildOpenCLKernels_tea_leaf_recip3_kernel(int xdim0, int xdim1) {
  ...
  if (!isbuilt_tea_leaf_recip3_kernel) {
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_tea_leaf_recip3_kernel=%d  "
                           "-Dxdim1_tea_leaf_recip3_kernel=%d ",
    ...
    OPS_opencl_core.kernel[24] = clCreateKernel(
        OPS_opencl_core.program, "ops_tea_leaf_recip3_kernel", &ret);
    clSafeCall(ret);
...
void ops_par_loop_tea_leaf_recip3_kernel(char const *name, ops_block block,
  ...
  buildOpenCLKernels_tea_leaf_recip3_kernel(xdim0, xdim1);
