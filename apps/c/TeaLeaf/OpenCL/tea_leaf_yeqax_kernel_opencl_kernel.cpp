...
void buildOpenCLKernels_tea_leaf_yeqax_kernel(int xdim0, int xdim1) {
  ...
  if (!isbuilt_tea_leaf_yeqax_kernel) {
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_tea_leaf_yeqax_kernel=%d  "
                           "-Dxdim1_tea_leaf_yeqax_kernel=%d ",
    ...
    OPS_opencl_core.kernel[24] = clCreateKernel(
        OPS_opencl_core.program, "ops_tea_leaf_yeqax_kernel", &ret);
    clSafeCall(ret);
...
void ops_par_loop_tea_leaf_yeqax_kernel(char const *name, ops_block block,
  ...
  buildOpenCLKernels_tea_leaf_yeqax_kernel(xdim0, xdim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[24], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[24], 2, sizeof(cl_double),
                            (void *)arg2.data));
