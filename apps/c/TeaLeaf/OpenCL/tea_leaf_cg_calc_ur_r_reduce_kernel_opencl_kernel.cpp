...
void buildOpenCLKernels_tea_leaf_cg_calc_ur_r_reduce_kernel(int xdim0,
                                                            int xdim1) {
  ...
  if (!isbuilt_tea_leaf_cg_calc_ur_r_reduce_kernel) {
    ...
    char *source_filename[1] = {
        (char *)"./OpenCL/tea_leaf_cg_calc_ur_r_reduce_kernel.cl"};
    ...
    printf(
        "Compiling tea_leaf_cg_calc_ur_r_reduce_kernel %d source -- start \n",
        OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_tea_leaf_cg_calc_ur_r_reduce_kernel=%d  "
                           "-Dxdim1_tea_leaf_cg_calc_ur_r_reduce_kernel=%d ",
    ...
    OPS_opencl_core.kernel[21] =
        clCreateKernel(OPS_opencl_core.program,
                       "ops_tea_leaf_cg_calc_ur_r_reduce_kernel", &ret);
    clSafeCall(ret);
...
void ops_par_loop_tea_leaf_cg_calc_ur_r_reduce_kernel(
  ...
  buildOpenCLKernels_tea_leaf_cg_calc_ur_r_reduce_kernel(xdim0, xdim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[21], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[21], 2, sizeof(cl_double),
                            (void *)arg2.data));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[21], 3, sizeof(cl_mem),
                            (void *)&arg3.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[21], 4,
