...
void buildOpenCLKernels_tea_leaf_common_init_diag_init_kernel(int xdim0,
                                                              int xdim1,
                                                              int xdim2) {
  ...
  if (!isbuilt_tea_leaf_common_init_diag_init_kernel) {
    ...
    char *source_filename[1] = {
        (char *)"./OpenCL/tea_leaf_common_init_diag_init_kernel.cl"};
    ...
    printf(
        "Compiling tea_leaf_common_init_diag_init_kernel %d source -- start \n",
        OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_tea_leaf_common_init_diag_init_kernel=%d  "
                           "-Dxdim1_tea_leaf_common_init_diag_init_kernel=%d  "
                           "-Dxdim2_tea_leaf_common_init_diag_init_kernel=%d ",
    ...
    OPS_opencl_core.kernel[40] =
        clCreateKernel(OPS_opencl_core.program,
                       "ops_tea_leaf_common_init_diag_init_kernel", &ret);
    clSafeCall(ret);
...
void ops_par_loop_tea_leaf_common_init_diag_init_kernel(
  ...
  buildOpenCLKernels_tea_leaf_common_init_diag_init_kernel(xdim0, xdim1, xdim2);
  ...
  if (globalWorkSize[0] > 0 && globalWorkSize[1] > 0 && globalWorkSize[2] > 0) {
    ...
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[40], 2, sizeof(cl_mem),
                              (void *)&arg2.data_d));
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[40], 3, sizeof(cl_double),
                              (void *)arg3.data));
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[40], 4, sizeof(cl_double),
                              (void *)arg4.data));
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[40], 5, sizeof(cl_int),
                              (void *)&base0));
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[40], 6, sizeof(cl_int),
                              (void *)&base1));
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[40], 7, sizeof(cl_int),
                              (void *)&base2));
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[40], 8, sizeof(cl_int),
                              (void *)&x_size));
