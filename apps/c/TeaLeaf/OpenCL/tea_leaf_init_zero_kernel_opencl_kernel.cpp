...
void buildOpenCLKernels_tea_leaf_init_zero_kernel(int xdim0) {
  ...
  if (!isbuilt_tea_leaf_init_zero_kernel) {
    ...
    char *source_filename[1] = {
        (char *)"./OpenCL/tea_leaf_init_zero_kernel.cl"};
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_tea_leaf_init_zero_kernel=%d ",
                pPath, 32, xdim0);
      else
    ...
    OPS_opencl_core.kernel[45] = clCreateKernel(
        OPS_opencl_core.program, "ops_tea_leaf_init_zero_kernel", &ret);
    clSafeCall(ret);
...
void ops_par_loop_tea_leaf_init_zero_kernel(char const *name, ops_block block,
  ...
  buildOpenCLKernels_tea_leaf_init_zero_kernel(xdim0);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[45], 0, sizeof(cl_mem),
                            (void *)&arg0.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[45], 1, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[45], 2, sizeof(cl_int),
                            (void *)&x_size));
