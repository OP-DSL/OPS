...
void buildOpenCLKernels_tea_leaf_ppcg_reduce_kernel(int xdim0, int xdim1,
                                                    int xdim2) {
  ...
  if (!isbuilt_tea_leaf_ppcg_reduce_kernel) {
    ...
    char *source_filename[1] = {
        (char *)"./OpenCL/tea_leaf_ppcg_reduce_kernel.cl"};
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_tea_leaf_ppcg_reduce_kernel=%d  "
                           "-Dxdim1_tea_leaf_ppcg_reduce_kernel=%d  "
                           "-Dxdim2_tea_leaf_ppcg_reduce_kernel=%d ",
    ...
    OPS_opencl_core.kernel[48] = clCreateKernel(
        OPS_opencl_core.program, "ops_tea_leaf_ppcg_reduce_kernel", &ret);
    clSafeCall(ret);
...
void ops_par_loop_tea_leaf_ppcg_reduce_kernel(char const *name, ops_block block,
  ...
  buildOpenCLKernels_tea_leaf_ppcg_reduce_kernel(xdim0, xdim1, xdim2);
  ...
  if (globalWorkSize[0] > 0 && globalWorkSize[1] > 0 && globalWorkSize[2] > 0) {
    ...
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[48], 3, sizeof(cl_mem),
                              (void *)&arg3.data_d));
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[48], 4,
