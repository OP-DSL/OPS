...
void ops_par_loop_tea_leaf_cheby_init_kernel(char const *, ops_block, int,
...
void tea_leaf_cheby_init(ops_dat u, ops_dat u0, ops_dat p, ops_dat r,
                         ops_dat Mi, ops_dat w, ops_dat z, ops_dat Kx,
                         ops_dat Ky, ops_dat cp, ops_dat bfp, double rx,
                         double ry, double theta, int preconditioner_type) {
  ...
  ops_par_loop_tea_leaf_cheby_init_kernel(
      "tea_leaf_cheby_init_kernel", tea_grid, 2, rangexy,
  ...
  if (preconditioner_type != TL_PREC_NONE) {
    ...
    ops_par_loop_tea_leaf_recip3_kernel(
        "tea_leaf_recip3_kernel", tea_grid, 2, rangexy,
  ...
  double one = 1.0;
  ops_par_loop_tea_leaf_xpy_kernel(
      "tea_leaf_xpy_kernel", tea_grid, 2, rangexy,
...
void tea_leaf_cheby_iterate(ops_dat u, ops_dat u0, ops_dat p, ops_dat r,
                            ops_dat Mi, ops_dat w, ops_dat z, ops_dat Kx,
                            ops_dat Ky, ops_dat cp, ops_dat bfp,
                            double *ch_alphas, double *ch_betas, double rx,
                            double ry, int step, int preconditioner_type) {
  int x_min = field.x_min;
  int x_max = field.x_max;
  int y_min = field.y_min;
  int y_max = field.y_max;
  ...
  if (preconditioner_type != TL_PREC_NONE) {
    ...
    ops_par_loop_tea_leaf_axpby_kernel(
        "tea_leaf_axpby_kernel", tea_grid, 2, rangexy,
...
void tea_calc_eigenvalues(double *cg_alphas, double *cg_betas, double *eigmin,
                          double *eigmax, int max_iters, int tl_ch_cg_presteps,
                          int *info) {
  ...
  for (int n = 1; n <= tl_ch_cg_presteps; n++) {
    ...
    if (n > 1)
      diag[n] = diag[n] + cg_betas[n - 1] / cg_alphas[n - 1];
    if (n < tl_ch_cg_presteps)
      offdiag[n + 1] = sqrt(cg_betas[n]) / cg_alphas[n];
  }
  ...
  *eigmax = diag[tl_ch_cg_presteps];
  ...
  if (*eigmin < 0.0 || *eigmax < 0.0)
    *info = 1;
}

void tea_calc_ch_coefs(double *ch_alphas, double *ch_betas, double eigmin,
                       double eigmax, double *theta, int max_cheby_iters) {
...
void tea_leaf_cheby_first_step(double *ch_alphas, double *ch_betas, int *fields,
                               double *error, double *theta, double cn,
                               int max_cheby_iters, int *est_itc,
  ...
  tea_leaf_cheby_init(u, u0, vector_p, vector_r, vector_Mi, vector_w, vector_z,
                      vector_Kx, vector_Ky, tri_cp, tri_bfp, rx, ry, *theta,
                      tl_preconditioner_type);

  update_halo(fields, 1);

  tea_leaf_cheby_iterate(u, u0, vector_p, vector_r, vector_Mi, vector_w,
                         vector_z, vector_Kx, vector_Ky, tri_cp, tri_bfp,
                         ch_alphas, ch_betas, rx, ry, 1,
                         tl_preconditioner_type);
