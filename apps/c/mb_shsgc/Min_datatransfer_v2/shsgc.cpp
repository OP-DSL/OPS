...
//   work array declaration not required for min datatransfer
...
//     ops_printf("iteration is %d\n",iter);
    ...
    for(int nrk=0; nrk<3; nrk++){
      ...
      for(int i=0; i<nblock; i++){
        // calculate all the convective term evaluation at one go
        int range[] = {sizes[(2*i)],sizes[(2*i)+1]};
        ops_par_loop(convective_kernel, "convective_kernel", shsgc_grid[i], 1, range,
                     ops_arg_dat(rho_new[i]   , 1, S1D_0M1M2P1P2, "double", OPS_READ ),
                     ops_arg_dat(rhou_new[i]  , 1, S1D_0M1M2P1P2, "double", OPS_READ ),
                     ops_arg_dat(rhoE_new[i]  , 1, S1D_0M1M2P1P2, "double", OPS_READ ),
        ...
        ops_par_loop(updateRK3_kernel, "updateRK3_kernel", shsgc_grid[i], 1, range,
                     ops_arg_dat(rho_new[i],  1, S1D_0, "double",OPS_WRITE),
                     ops_arg_dat(rhou_new[i], 1, S1D_0, "double",OPS_WRITE),
                     ops_arg_dat(rhoE_new[i], 1, S1D_0, "double",OPS_WRITE),
...
// findout the best way to code the riemann invariants at one go
