...
void ops_par_loop_limiter_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  ops_timing_realloc(6,"limiter_kernel");
  OPS_kernels[6].count++;
  ...
  for( n_x=start[0]; n_x<start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x+=SIMD_VEC ) {
    ...
    for ( int i=0; i<SIMD_VEC; i++ ){
      limiter_kernel(  (double *)p_a[0]+ i*1*3, (double *)p_a[1]+ i*1*3, (double *)p_a[2]+ i*1*3 );
  ...
  for ( int n_x=start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x<end[0]; n_x++ ){
    ...
    limiter_kernel(  (double *)p_a[0], (double *)p_a[1], (double *)p_a[2] );
