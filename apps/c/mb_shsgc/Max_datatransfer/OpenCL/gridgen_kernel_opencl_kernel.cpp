...
void buildOpenCLKernels_gridgen_kernel(int xdim0) {
  ...
  if (!isbuilt_gridgen_kernel) {
    ...
    printf("Compiling gridgen_kernel %d source -- start \n", OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  -Dxdim0_gridgen_kernel=%d ",
                pPath, 32, xdim0);
      else
        sprintf(buildOpts, "-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  "
                           "-Dxdim0_gridgen_kernel=%d ",
                pPath, 32, xdim0);
    else {
...
void ops_par_loop_gridgen_kernel(char const *name, ops_block block, int dim,
  ...
  buildOpenCLKernels_gridgen_kernel(xdim0);
  ...
  if (globalWorkSize[0] > 0 && globalWorkSize[1] > 0 && globalWorkSize[2] > 0) {
    ...
    clSafeCall(clSetKernelArg(OPS_instance::getOPSInstance()
                                  ->opencl_instance->OPS_opencl_core.kernel[0],
                              1, sizeof(cl_double), (void *)&dx));
    clSafeCall(clSetKernelArg(OPS_instance::getOPSInstance()
                                  ->opencl_instance->OPS_opencl_core.kernel[0],
                              2, sizeof(cl_double), (void *)&xt));
    clSafeCall(clSetKernelArg(OPS_instance::getOPSInstance()
                                  ->opencl_instance->OPS_opencl_core.kernel[0],
                              3, sizeof(cl_int), (void *)&base0));
    clSafeCall(clSetKernelArg(OPS_instance::getOPSInstance()
                                  ->opencl_instance->OPS_opencl_core.kernel[0],
                              4, sizeof(cl_int), (void *)&arg_idx[0]));
    clSafeCall(clSetKernelArg(OPS_instance::getOPSInstance()
                                  ->opencl_instance->OPS_opencl_core.kernel[0],
