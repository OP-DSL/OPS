...
void buildOpenCLKernels_gridgen_kernel(int xdim0) {
  ...
  if(!isbuilt_gridgen_kernel) {
    ...
    printf("Compiling gridgen_kernel %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_gridgen_kernel=%d ", pPath, 32,xdim0);
        else
    ...
    OPS_opencl_core.kernel[0] = clCreateKernel(OPS_opencl_core.program, "ops_gridgen_kernel", &ret);
    clSafeCall( ret );
...
void ops_par_loop_gridgen_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  buildOpenCLKernels_gridgen_kernel(
  xdim0);
  ...
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[0], 1, sizeof(cl_double), (void*) &dx ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[0], 2, sizeof(cl_double), (void*) &xt ));
