...
void buildOpenCLKernels_calvar_kernel(int xdim0, int xdim1, int xdim2, int xdim3, int xdim4) {
  ...
  if(!isbuilt_calvar_kernel) {
    ...
    printf("Compiling calvar_kernel %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_calvar_kernel=%d  -Dxdim1_calvar_kernel=%d  -Dxdim2_calvar_kernel=%d  -Dxdim3_calvar_kernel=%d  -Dxdim4_calvar_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2,xdim3,xdim4);
        else
    ...
    OPS_opencl_core.kernel[3] = clCreateKernel(OPS_opencl_core.program, "ops_calvar_kernel", &ret);
    clSafeCall( ret );
...
void ops_par_loop_calvar_kernel(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4) {
  ...
  buildOpenCLKernels_calvar_kernel(
  ...
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[3], 5, sizeof(cl_double), (void*) &gam1 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[3], 6, sizeof(cl_int), (void*) &base0 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[3], 7, sizeof(cl_int), (void*) &base1 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[3], 8, sizeof(cl_int), (void*) &base2 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[3], 9, sizeof(cl_int), (void*) &base3 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[3], 10, sizeof(cl_int), (void*) &base4 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[3], 11, sizeof(cl_int), (void*) &x_size ));
