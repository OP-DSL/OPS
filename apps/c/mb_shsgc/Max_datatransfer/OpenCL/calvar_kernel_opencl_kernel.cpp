...
void buildOpenCLKernels_calvar_kernel(int xdim0, int xdim1, int xdim2,
                                      int xdim3, int xdim4) {
  ...
  if (!isbuilt_calvar_kernel) {
    ...
    printf("Compiling calvar_kernel %d source -- start \n", OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  -Dxdim0_calvar_kernel=%d  "
                           "-Dxdim1_calvar_kernel=%d  -Dxdim2_calvar_kernel=%d "
                           " -Dxdim3_calvar_kernel=%d  "
                           "-Dxdim4_calvar_kernel=%d ",
                pPath, 32, xdim0, xdim1, xdim2, xdim3, xdim4);
      else
        sprintf(buildOpts, "-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  "
                           "-Dxdim0_calvar_kernel=%d  -Dxdim1_calvar_kernel=%d "
                           " -Dxdim2_calvar_kernel=%d  "
                           "-Dxdim3_calvar_kernel=%d  "
                           "-Dxdim4_calvar_kernel=%d ",
                pPath, 32, xdim0, xdim1, xdim2, xdim3, xdim4);
    else {
    ...
    printf("compiling calvar_kernel -- done\n");

    // Create the OpenCL kernel
    OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[3] =
        clCreateKernel(OPS_instance::getOPSInstance()
                           ->opencl_instance->OPS_opencl_core.program,
                       "ops_calvar_kernel", &ret);
    clSafeCall(ret);
...
void ops_par_loop_calvar_kernel(char const *name, ops_block block, int dim,
  ...
  buildOpenCLKernels_calvar_kernel(xdim0, xdim1, xdim2, xdim3, xdim4);
  ...
  if (globalWorkSize[0] > 0 && globalWorkSize[1] > 0 && globalWorkSize[2] > 0) {
    ...
    clSafeCall(clSetKernelArg(OPS_instance::getOPSInstance()
                                  ->opencl_instance->OPS_opencl_core.kernel[3],
