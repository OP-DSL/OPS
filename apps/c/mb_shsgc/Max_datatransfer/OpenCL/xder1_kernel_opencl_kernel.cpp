...
void buildOpenCLKernels_xder1_kernel(int xdim0, int xdim1) {
  ...
  if (!isbuilt_xder1_kernel) {
    ...
    printf("Compiling xder1_kernel %d source -- start \n", OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  -Dxdim0_xder1_kernel=%d  "
                           "-Dxdim1_xder1_kernel=%d ",
                pPath, 32, xdim0, xdim1);
      else
        sprintf(buildOpts, "-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  "
                           "-Dxdim0_xder1_kernel=%d  -Dxdim1_xder1_kernel=%d ",
                pPath, 32, xdim0, xdim1);
    else {
    ...
    printf("compiling xder1_kernel -- done\n");

    // Create the OpenCL kernel
    OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[4] =
        clCreateKernel(OPS_instance::getOPSInstance()
                           ->opencl_instance->OPS_opencl_core.program,
                       "ops_xder1_kernel", &ret);
    clSafeCall(ret);
...
void ops_par_loop_xder1_kernel(char const *name, ops_block block, int dim,
  ...
  buildOpenCLKernels_xder1_kernel(xdim0, xdim1);
  ...
  if (globalWorkSize[0] > 0 && globalWorkSize[1] > 0 && globalWorkSize[2] > 0) {
    ...
    clSafeCall(clSetKernelArg(OPS_instance::getOPSInstance()
                                  ->opencl_instance->OPS_opencl_core.kernel[4],
