...
void buildOpenCLKernels_xder1_kernel(int xdim0, int xdim1) {
  ...
  if(!isbuilt_xder1_kernel) {
    ...
    printf("Compiling xder1_kernel %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_xder1_kernel=%d  -Dxdim1_xder1_kernel=%d ", pPath, 32,xdim0,xdim1);
        else
    ...
    OPS_opencl_core.kernel[4] = clCreateKernel(OPS_opencl_core.program, "ops_xder1_kernel", &ret);
    clSafeCall( ret );
...
void ops_par_loop_xder1_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  buildOpenCLKernels_xder1_kernel(
  xdim0,xdim1);
  ...
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[4], 2, sizeof(cl_double), (void*) &dx ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[4], 3, sizeof(cl_int), (void*) &base0 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[4], 4, sizeof(cl_int), (void*) &base1 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[4], 5, sizeof(cl_int), (void*) &x_size ));
