...
// user function
#pragma acc routine
inline void calupwindeff_kernel(const ptrm_double cmp, const ptrm_double gt,
  ...
  double e1 = (OPS_ACC(cmp, 0, 0) * (OPS_ACC(gt, 0, 0) + OPS_ACC(gt, 0, 1)) -
               OPS_ACC(cf, 0, 0) * OPS_ACC(al, 0, 0)) *
              OPS_ACC(ep2, 0, 0);
  double e2 = (OPS_ACC(cmp, 1, 0) * (OPS_ACC(gt, 1, 0) + OPS_ACC(gt, 1, 1)) -
               OPS_ACC(cf, 1, 0) * OPS_ACC(al, 1, 0)) *
              OPS_ACC(ep2, 1, 0);
  double e3 = (OPS_ACC(cmp, 2, 0) * (OPS_ACC(gt, 2, 0) + OPS_ACC(gt, 2, 1)) -
               OPS_ACC(cf, 2, 0) * OPS_ACC(al, 2, 0)) *
              OPS_ACC(ep2, 2, 0);

  OPS_ACC(eff, 0, 0) =
      e1 * OPS_ACC(r, 0, 0) + e2 * OPS_ACC(r, 1, 0) + e3 * OPS_ACC(r, 2, 0);
  OPS_ACC(eff, 1, 0) =
      e1 * OPS_ACC(r, 3, 0) + e2 * OPS_ACC(r, 4, 0) + e3 * OPS_ACC(r, 5, 0);
  OPS_ACC(eff, 2, 0) =
      e1 * OPS_ACC(r, 6, 0) + e2 * OPS_ACC(r, 7, 0) + e3 * OPS_ACC(r, 8, 0);
}
