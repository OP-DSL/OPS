...
__device__
    ...
    void
    xder1_kernel_gpu(const ACC<double> &inp, ACC<double> &out) {
  double dix = 1 / (12.00 * dx);
  out(0) = (inp(-2) - inp(2) + 8.0 * (inp(1) - inp(-1))) * dix;
}

__global__ void ops_xder1_kernel(double *__restrict arg0,
...
void ops_par_loop_xder1_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_xder1_kernel_h[0][0] ||
      xdim1 != dims_xder1_kernel_h[1][0]) {
    dims_xder1_kernel_h[0][0] = xdim0;
    dims_xder1_kernel_h[1][0] = xdim1;
    cutilSafeCall(cudaMemcpyToSymbol(dims_xder1_kernel, dims_xder1_kernel_h,
                                     sizeof(dims_xder1_kernel)));
  }
