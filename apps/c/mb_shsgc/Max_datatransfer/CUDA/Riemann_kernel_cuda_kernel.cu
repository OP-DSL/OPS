...
__device__
    ...
    void
    Riemann_kernel_gpu(const ACC<double> &rho_new, const ACC<double> &rhou_new,
                       const ACC<double> &rhoE_new, ACC<double> &alam,
                       ACC<double> &r, ACC<double> &al) {
...
__global__ void ops_Riemann_kernel(double *__restrict arg0,
  ...
  if (idx_x < size0) {
    ...
    Riemann_kernel_gpu(argp0, argp1, argp2, argp3, argp4, argp5);
  }
}
...
void ops_par_loop_Riemann_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_Riemann_kernel_h[0][0] ||
      xdim1 != dims_Riemann_kernel_h[1][0] ||
      xdim2 != dims_Riemann_kernel_h[2][0] ||
      xdim3 != dims_Riemann_kernel_h[3][0] ||
      xdim4 != dims_Riemann_kernel_h[4][0] ||
      xdim5 != dims_Riemann_kernel_h[5][0]) {
    ...
    cutilSafeCall(cudaMemcpyToSymbol(dims_Riemann_kernel, dims_Riemann_kernel_h,
                                     sizeof(dims_Riemann_kernel)));
  }
