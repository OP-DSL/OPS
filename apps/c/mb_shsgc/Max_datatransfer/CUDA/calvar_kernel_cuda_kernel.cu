...
__device__
    ...
    void
    calvar_kernel_gpu(const ACC<double> &rho_new, const ACC<double> &rhou_new,
                      const ACC<double> &rhoE_new, ACC<double> &workarray2,
...
__global__ void ops_calvar_kernel(double *__restrict arg0,
  ...
  if (idx_x < size0) {
    ...
    calvar_kernel_gpu(argp0, argp1, argp2, argp3, argp4);
  }
}
...
void ops_par_loop_calvar_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_calvar_kernel_h[0][0] ||
      xdim1 != dims_calvar_kernel_h[1][0] ||
      xdim2 != dims_calvar_kernel_h[2][0] ||
      xdim3 != dims_calvar_kernel_h[3][0] ||
      xdim4 != dims_calvar_kernel_h[4][0]) {
    ...
    cutilSafeCall(cudaMemcpyToSymbol(dims_calvar_kernel, dims_calvar_kernel_h,
                                     sizeof(dims_calvar_kernel)));
  }
