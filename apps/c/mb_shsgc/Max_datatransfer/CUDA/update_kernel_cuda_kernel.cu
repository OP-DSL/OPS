...
__device__
    ...
    void
    update_kernel_gpu(ACC<double> &rho_new, ACC<double> &rhou_new,
                      ACC<double> &rhoE_new, const ACC<double> &s) {
...
void ops_par_loop_update_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_update_kernel_h[0][0] ||
      xdim1 != dims_update_kernel_h[1][0] ||
      xdim2 != dims_update_kernel_h[2][0] ||
      xdim3 != dims_update_kernel_h[3][0]) {
    dims_update_kernel_h[0][0] = xdim0;
    dims_update_kernel_h[1][0] = xdim1;
    dims_update_kernel_h[2][0] = xdim2;
    dims_update_kernel_h[3][0] = xdim3;
    cutilSafeCall(cudaMemcpyToSymbol(dims_update_kernel, dims_update_kernel_h,
                                     sizeof(dims_update_kernel)));
  }
