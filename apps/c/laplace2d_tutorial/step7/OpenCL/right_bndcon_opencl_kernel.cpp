...
void buildOpenCLKernels_right_bndcon(int xdim0) {
  ...
  if(!isbuilt_right_bndcon) {
    ...
    printf("Compiling right_bndcon %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_right_bndcon=%d ", pPath, 32,xdim0);
        else
          sprintf(buildOpts,"-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_right_bndcon=%d ", pPath, 32,xdim0);
      else {
