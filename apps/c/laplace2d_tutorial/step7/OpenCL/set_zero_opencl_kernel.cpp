...
void buildOpenCLKernels_set_zero(int xdim0) {
  ...
  if (!isbuilt_set_zero) {
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  -Dxdim0_set_zero=%d ",
                pPath, 32, xdim0);
      else
    ...
    OPS_opencl_core.kernel[1] =
        clCreateKernel(OPS_opencl_core.program, "ops_set_zero", &ret);
    clSafeCall(ret);
...
void ops_par_loop_set_zero(char const *name, ops_block block, int dim,
  ...
  buildOpenCLKernels_set_zero(xdim0);
  ...
  if (globalWorkSize[0] > 0 && globalWorkSize[1] > 0 && globalWorkSize[2] > 0) {
    ...
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[1], 0, sizeof(cl_mem),
                              (void *)&arg0.data_d));
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[1], 1, sizeof(cl_int),
                              (void *)&base0));
