...
static bool isbuilt_copy = false;

void buildOpenCLKernels_copy(int xdim0, int xdim1) {
  ...
  if(!isbuilt_copy) {
    ...
    cl_int ret;
    char* source_filename[1] = {(char*)"./OpenCL/copy.cl"};
    ...
    printf("Compiling copy %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_copy=%d  -Dxdim1_copy=%d ", pPath, 32,xdim0,xdim1);
        else
          sprintf(buildOpts,"-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_copy=%d  -Dxdim1_copy=%d ", pPath, 32,xdim0,xdim1);
      else {
      ...
      printf("compiling copy -- done\n");

    // Create the OpenCL kernel
    OPS_opencl_core.kernel[5] = clCreateKernel(OPS_opencl_core.program, "ops_copy", &ret);
    clSafeCall( ret );
...
void ops_par_loop_copy(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[5], 2, sizeof(cl_int), (void*) &base0 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[5], 3, sizeof(cl_int), (void*) &base1 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[5], 4, sizeof(cl_int), (void*) &x_size ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[5], 5, sizeof(cl_int), (void*) &y_size ));
