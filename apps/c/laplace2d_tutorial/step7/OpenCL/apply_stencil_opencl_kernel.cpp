...
static bool isbuilt_apply_stencil = false;

void buildOpenCLKernels_apply_stencil(OPS_instance *instance, int xdim0, int xdim1) {

  //int ocl_fma = OCL_FMA;
  if(!isbuilt_apply_stencil) {
    buildOpenCLKernels(instance);
    //clSafeCall( clUnloadCompiler() );
    cl_int ret;
    char* source_filename[1] = {(char*)"./OpenCL/apply_stencil.cl"};
    ...
    instance->ostream() <<"Compiling apply_stencil "<<OCL_FMA<<" source -- start \n";
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_apply_stencil=%d  -Dxdim1_apply_stencil=%d ", pPath, 32,xdim0,xdim1);
        else
          sprintf(buildOpts,"-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_apply_stencil=%d  -Dxdim1_apply_stencil=%d ", pPath, 32,xdim0,xdim1);
      else {
      ...
      instance->ostream() << "compiling apply_stencil -- done\n";

    // Create the OpenCL kernel
    instance->opencl_instance->OPS_opencl_core.kernel[4] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_apply_stencil", &ret);
    clSafeCall( ret );

    isbuilt_apply_stencil = true;
    free(source_str[0]);
  }
...
void ops_par_loop_apply_stencil(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  buildOpenCLKernels_apply_stencil(block->instance,
  xdim0,xdim1);
