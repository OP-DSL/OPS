...
void buildOpenCLKernels_apply_stencil(int xdim0, int xdim1) {
  ...
  if (!isbuilt_apply_stencil) {
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  -Dxdim0_apply_stencil=%d  "
                           "-Dxdim1_apply_stencil=%d ",
                pPath, 32, xdim0, xdim1);
      else
        sprintf(buildOpts, "-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  "
                           "-Dxdim0_apply_stencil=%d  "
                           "-Dxdim1_apply_stencil=%d ",
                pPath, 32, xdim0, xdim1);
    else {
    ...
    OPS_opencl_core.kernel[4] =
        clCreateKernel(OPS_opencl_core.program, "ops_apply_stencil", &ret);
    clSafeCall(ret);
...
void ops_par_loop_apply_stencil(char const *name, ops_block block, int dim,
  ...
  buildOpenCLKernels_apply_stencil(xdim0, xdim1);
  ...
  if (globalWorkSize[0] > 0 && globalWorkSize[1] > 0 && globalWorkSize[2] > 0) {
    ...
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[4], 2, sizeof(cl_mem),
                              (void *)&arg2.data_d));
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[4], 3,
