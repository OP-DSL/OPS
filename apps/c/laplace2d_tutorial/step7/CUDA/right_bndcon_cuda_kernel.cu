...
void right_bndcon_gpu(ACC<double> &A,
  const int *idx) {
...
int size1 ){
  ...
  if (idx_x < size0 && idx_y < size1) {
    ACC<double> argp0(dims_right_bndcon[0][0], arg0);
    right_bndcon_gpu(argp0, arg_idx);
  }
...
void ops_par_loop_right_bndcon_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_right_bndcon_h[0][0]) {
    ...
    cutilSafeCall(block->instance->ostream(), cudaMemcpyToSymbol( dims_right_bndcon, dims_right_bndcon_h, sizeof(dims_right_bndcon)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    ops_right_bndcon<<<grid, tblock >>> (  (double *)p_a[0], arg_idx[0], arg_idx[1],x_size, y_size);
