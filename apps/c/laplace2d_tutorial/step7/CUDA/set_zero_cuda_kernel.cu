...
//
__constant__ int dims_set_zero[1][1];
static int dims_set_zero_h[1][1] = {0};

#undef OPS_ACC0

#define OPS_ACC0(x, y) (x + dims_set_zero[0][0] * (y))

// user function
__device__ void set_zero_gpu(double *A) { A[OPS_ACC0(0, 0)] = 0.0; }

#undef OPS_ACC0

__global__ void ops_set_zero(double *__restrict arg0, int size0, int size1) {

  int idx_y = blockDim.y * blockIdx.y + threadIdx.y;
  int idx_x = blockDim.x * blockIdx.x + threadIdx.x;

  arg0 += idx_x * 1 * 1 + idx_y * 1 * 1 * dims_set_zero[0][0];
...
void ops_par_loop_set_zero_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_set_zero_h[0][0]) {
    dims_set_zero_h[0][0] = xdim0;
    cutilSafeCall(cudaMemcpyToSymbol(dims_set_zero, dims_set_zero_h,
                                     sizeof(dims_set_zero)));
  }
