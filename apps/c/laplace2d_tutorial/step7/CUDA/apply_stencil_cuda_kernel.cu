...
//
__constant__ int dims_apply_stencil[3][1];
static int dims_apply_stencil_h[3][1] = {0};

#undef OPS_ACC0
#undef OPS_ACC1

#define OPS_ACC0(x, y) (x + dims_apply_stencil[0][0] * (y))
#define OPS_ACC1(x, y) (x + dims_apply_stencil[1][0] * (y))

// user function
__device__

    void
    apply_stencil_gpu(const double *A, double *Anew, double *error) {
...
__global__ void ops_apply_stencil(const double *__restrict arg0,
  ...
  double arg2_l[1];
  for (int d = 0; d < 1; d++)
    arg2_l[d] = -INFINITY_double;

  int idx_y = blockDim.y * blockIdx.y + threadIdx.y;
  int idx_x = blockDim.x * blockIdx.x + threadIdx.x;

  arg0 += idx_x * 1 * 1 + idx_y * 1 * 1 * dims_apply_stencil[0][0];
  arg1 += idx_x * 1 * 1 + idx_y * 1 * 1 * dims_apply_stencil[1][0];

  if (idx_x < size0 && idx_y < size1) {
    apply_stencil_gpu(arg0, arg1, arg2_l);
  }
  for (int d = 0; d < 1; d++)
    ops_reduction_cuda<OPS_MAX>(
...
void ops_par_loop_apply_stencil_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_apply_stencil_h[0][0] ||
      xdim1 != dims_apply_stencil_h[1][0]) {
    dims_apply_stencil_h[0][0] = xdim0;
    dims_apply_stencil_h[1][0] = xdim1;
    cutilSafeCall(cudaMemcpyToSymbol(dims_apply_stencil, dims_apply_stencil_h,
                                     sizeof(dims_apply_stencil)));
  }
  ...
  for (int b = 0; b < maxblocks; b++)
    for (int d = 0; d < 1; d++)
      ((double *)arg2.data)[d + b * 1] = -INFINITY_double;
  reduct_bytes += ROUND_UP(maxblocks * 1 * sizeof(double));
  ...
  if (x_size > 0 && y_size > 0)
    ops_apply_stencil<<<grid, tblock, nshared>>>(
