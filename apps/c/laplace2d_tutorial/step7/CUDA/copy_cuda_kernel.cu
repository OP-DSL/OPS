...
//
__constant__ int dims_copy[2][1];
static int dims_copy_h[2][1] = {0};

#undef OPS_ACC0
#undef OPS_ACC1

#define OPS_ACC0(x, y) (x + dims_copy[0][0] * (y))
#define OPS_ACC1(x, y) (x + dims_copy[1][0] * (y))

// user function
__device__

    void
    copy_gpu(double *A, const double *Anew) {
...
__global__ void ops_copy(double *__restrict arg0, const double *__restrict arg1,
                         int size0, int size1) {

  int idx_y = blockDim.y * blockIdx.y + threadIdx.y;
  int idx_x = blockDim.x * blockIdx.x + threadIdx.x;

  arg0 += idx_x * 1 * 1 + idx_y * 1 * 1 * dims_copy[0][0];
  arg1 += idx_x * 1 * 1 + idx_y * 1 * 1 * dims_copy[1][0];
...
void ops_par_loop_copy_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_copy_h[0][0] || xdim1 != dims_copy_h[1][0]) {
    ...
    cutilSafeCall(
        cudaMemcpyToSymbol(dims_copy, dims_copy_h, sizeof(dims_copy)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    ops_copy<<<grid, tblock>>>((double *)p_a[0], (double *)p_a[1], x_size,
                               y_size);
