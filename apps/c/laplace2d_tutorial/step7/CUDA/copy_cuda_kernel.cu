...
//
__constant__ int xdim0_copy;
int xdim0_copy_h = -1;
__constant__ int xdim1_copy;
int xdim1_copy_h = -1;
...
void copy_gpu(double *A, const double *Anew) {
...
int size1 ){
  ...
  arg0 += idx_x * 1*1 + idx_y * 1*1 * xdim0_copy;
  arg1 += idx_x * 1*1 + idx_y * 1*1 * xdim1_copy;
...
void ops_par_loop_copy_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != xdim0_copy_h || xdim1 != xdim1_copy_h) {
    cudaMemcpyToSymbol( xdim0_copy, &xdim0, sizeof(int) );
    xdim0_copy_h = xdim0;
    cudaMemcpyToSymbol( xdim1_copy, &xdim1, sizeof(int) );
    xdim1_copy_h = xdim1;
  }
