...
void ops_par_loop_update_halo_kernel1_r2(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4, ops_arg arg5, ops_arg arg6, ops_arg arg7) {
  ...
  if (OPS_diags > 1) {
    ...
    ops_timers_core(&c2,&t2);
    OPS_kernels[63].mpi_time += t2-t1;
    OPS_kernels[63].transfer += ops_compute_transfer2(dim, start, end, &arg0);
    OPS_kernels[63].transfer += ops_compute_transfer2(dim, start, end, &arg1);
    OPS_kernels[63].transfer += ops_compute_transfer2(dim, start, end, &arg2);
    OPS_kernels[63].transfer += ops_compute_transfer2(dim, start, end, &arg3);
    OPS_kernels[63].transfer += ops_compute_transfer2(dim, start, end, &arg4);
    OPS_kernels[63].transfer += ops_compute_transfer2(dim, start, end, &arg5);
    OPS_kernels[63].transfer += ops_compute_transfer2(dim, start, end, &arg6);
  }
}
