...
void ops_par_loop_flux_calc_kernelz(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2, ops_arg arg3) {
  ...
  if (OPS_diags > 1) {
    ...
    ops_timers_core(&c2,&t2);
    OPS_kernels[44].mpi_time += t2-t1;
    OPS_kernels[44].transfer += ops_compute_transfer2(dim, start, end, &arg0);
    OPS_kernels[44].transfer += ops_compute_transfer2(dim, start, end, &arg1);
    OPS_kernels[44].transfer += ops_compute_transfer2(dim, start, end, &arg2);
    OPS_kernels[44].transfer += ops_compute_transfer2(dim, start, end, &arg3);
  }
}
