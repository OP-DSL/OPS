...
#else
#ifdef OPS_HYBRID
void ops_par_loop_calc_dt_kernel_min_execute_gpu(ops_kernel_descriptor *desc) {
#else
void ops_par_loop_calc_dt_kernel_min_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != xdim0_calc_dt_kernel_min_h || ydim0 != ydim0_calc_dt_kernel_min_h) {
    ...
    cudaMemcpyToSymbolAsync( ydim0_calc_dt_kernel_min, &ydim0, sizeof(int),0 );
...
#ifdef OPS_HYBRID
void ops_par_loop_calc_dt_kernel_min_execute_cpu(ops_kernel_descriptor *desc);
void ops_par_loop_calc_dt_kernel_min_execute(ops_kernel_descriptor *desc) {
  if (desc->device == 1) {
    ops_par_loop_calc_dt_kernel_min_execute_gpu(desc);
  }
  else {
    ops_par_loop_calc_dt_kernel_min_execute_cpu(desc);
  }
}
#endif
