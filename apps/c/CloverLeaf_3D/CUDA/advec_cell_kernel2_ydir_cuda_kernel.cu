...
void ops_par_loop_advec_cell_kernel2_ydir(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4) {
  ...
  if (OPS_diags > 1) {
    ...
    ops_timers_core(&c2,&t2);
    OPS_kernels[12].mpi_time += t2-t1;
    OPS_kernels[12].transfer += ops_compute_transfer2(dim, start, end, &arg0);
    OPS_kernels[12].transfer += ops_compute_transfer2(dim, start, end, &arg1);
    OPS_kernels[12].transfer += ops_compute_transfer2(dim, start, end, &arg2);
    OPS_kernels[12].transfer += ops_compute_transfer2(dim, start, end, &arg3);
    OPS_kernels[12].transfer += ops_compute_transfer2(dim, start, end, &arg4);
  }
}
