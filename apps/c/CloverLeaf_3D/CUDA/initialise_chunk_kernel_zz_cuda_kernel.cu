...
#define OPS_ACC0(x, y, z)                                                      \
  (x + dims_initialise_chunk_kernel_zz[0][0] * (y) +                           \
   dims_initialise_chunk_kernel_zz[0][0] *                                     \
       dims_initialise_chunk_kernel_zz[0][1] * (z))

// user function
__device__

    void
    initialise_chunk_kernel_zz_gpu(int *zz, int *idx) {
  zz[OPS_ACC0(0, 0, 0)] = idx[2] - 2;
}

#undef OPS_ACC0

__global__ void ops_initialise_chunk_kernel_zz(int *__restrict arg0,
  ...
  arg0 += idx_x * 0 * 1 +
          idx_y * 0 * 1 * dims_initialise_chunk_kernel_zz[0][0] +
          idx_z * 1 * 1 * dims_initialise_chunk_kernel_zz[0][0] *
              dims_initialise_chunk_kernel_zz[0][1];
...
void ops_par_loop_initialise_chunk_kernel_zz_execute(
  ...
  if (xdim0 != dims_initialise_chunk_kernel_zz_h[0][0] ||
      ydim0 != dims_initialise_chunk_kernel_zz_h[0][1]) {
    dims_initialise_chunk_kernel_zz_h[0][0] = xdim0;
    dims_initialise_chunk_kernel_zz_h[0][1] = ydim0;
    cutilSafeCall(cudaMemcpyToSymbol(dims_initialise_chunk_kernel_zz,
                                     dims_initialise_chunk_kernel_zz_h,
                                     sizeof(dims_initialise_chunk_kernel_zz)));
  }
  ...
  if (x_size > 0 && y_size > 0 && z_size > 0)
    ops_initialise_chunk_kernel_zz<<<grid, tblock>>>((int *)p_a[0], arg_idx[0],
                                                     arg_idx[1], arg_idx[2],
