...
void ops_par_loop_flux_calc_kernelx_execute(ops_kernel_descriptor *desc) {
  ...
  cl::sycl::buffer<double,1> xarea_p = static_cast<cl::sycl::buffer<char,1> *>((void*)args[1].data_d)->reinterpret<double,1>(cl::sycl::range<1>(args[1].dat->mem/sizeof(double)));

  int base2 = args[2].dat->base_offset/sizeof(double);
  cl::sycl::buffer<double,1> xvel0_p = static_cast<cl::sycl::buffer<char,1> *>((void*)args[2].data_d)->reinterpret<double,1>(cl::sycl::range<1>(args[2].dat->mem/sizeof(double)));

  int base3 = args[3].dat->base_offset/sizeof(double);
  cl::sycl::buffer<double,1> xvel1_p = static_cast<cl::sycl::buffer<char,1> *>((void*)args[3].data_d)->reinterpret<double,1>(cl::sycl::range<1>(args[3].dat->mem/sizeof(double)));
  ...
  block->instance->sycl_instance->queue->submit([&](cl::sycl::handler &cgh) {
    ...
    cgh.parallel_for<class flux_calc_kernelx_kernel>(cl::sycl::nd_range<3>(cl::sycl::range<3>(
      ...
      ACC<double> vol_flux_x(xdim0_flux_calc_kernelx, ydim0_flux_calc_kernelx, &Accessor_vol_flux_x[0] + base0 + n_x*1 + n_y * xdim0_flux_calc_kernelx*1 + n_z * xdim0_flux_calc_kernelx * ydim0_flux_calc_kernelx*1);
      const ACC<double> xarea(xdim1_flux_calc_kernelx, ydim1_flux_calc_kernelx, &Accessor_xarea[0] + base1 + n_x*1 + n_y * xdim1_flux_calc_kernelx*1 + n_z * xdim1_flux_calc_kernelx * ydim1_flux_calc_kernelx*1);
      const ACC<double> xvel0(xdim2_flux_calc_kernelx, ydim2_flux_calc_kernelx, &Accessor_xvel0[0] + base2 + n_x*1 + n_y * xdim2_flux_calc_kernelx*1 + n_z * xdim2_flux_calc_kernelx * ydim2_flux_calc_kernelx*1);
      const ACC<double> xvel1(xdim3_flux_calc_kernelx, ydim3_flux_calc_kernelx, &Accessor_xvel1[0] + base3 + n_x*1 + n_y * xdim3_flux_calc_kernelx*1 + n_z * xdim3_flux_calc_kernelx * ydim3_flux_calc_kernelx*1);
      //USER CODE
      if (n_x < end_0 && n_y < end_1 && n_z < end_2) {
        

  vol_flux_x(0,0,0) = 0.125 * dt_sycl[0] * (xarea(0,0,0)) *
