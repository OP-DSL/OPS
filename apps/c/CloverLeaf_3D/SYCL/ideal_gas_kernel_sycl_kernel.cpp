...
void ops_par_loop_ideal_gas_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  cl::sycl::buffer<double,1> density_p = static_cast<cl::sycl::buffer<char,1> *>((void*)args[0].data_d)->reinterpret<double,1>(cl::sycl::range<1>(args[0].dat->mem/sizeof(double)));

  int base1 = args[1].dat->base_offset/sizeof(double);
  cl::sycl::buffer<double,1> energy_p = static_cast<cl::sycl::buffer<char,1> *>((void*)args[1].data_d)->reinterpret<double,1>(cl::sycl::range<1>(args[1].dat->mem/sizeof(double)));

  int base2 = args[2].dat->base_offset/sizeof(double);
  cl::sycl::buffer<double,1> pressure_p = static_cast<cl::sycl::buffer<char,1> *>((void*)args[2].data_d)->reinterpret<double,1>(cl::sycl::range<1>(args[2].dat->mem/sizeof(double)));

  int base3 = args[3].dat->base_offset/sizeof(double);
  cl::sycl::buffer<double,1> soundspeed_p = static_cast<cl::sycl::buffer<char,1> *>((void*)args[3].data_d)->reinterpret<double,1>(cl::sycl::range<1>(args[3].dat->mem/sizeof(double)));
  ...
  block->instance->sycl_instance->queue->submit([&](cl::sycl::handler &cgh) {
    //accessors
    auto Accessor_density = density_p.get_access<cl::sycl::access::mode::read_write>(cgh);
    auto Accessor_energy = energy_p.get_access<cl::sycl::access::mode::read_write>(cgh);
    ...
    cgh.parallel_for<class ideal_gas_kernel_kernel>(cl::sycl::nd_range<3>(cl::sycl::range<3>(
      ...
      const ACC<double> density(xdim0_ideal_gas_kernel, ydim0_ideal_gas_kernel, &Accessor_density[0] + base0 + n_x*1 + n_y * xdim0_ideal_gas_kernel*1 + n_z * xdim0_ideal_gas_kernel * ydim0_ideal_gas_kernel*1);
      const ACC<double> energy(xdim1_ideal_gas_kernel, ydim1_ideal_gas_kernel, &Accessor_energy[0] + base1 + n_x*1 + n_y * xdim1_ideal_gas_kernel*1 + n_z * xdim1_ideal_gas_kernel * ydim1_ideal_gas_kernel*1);
      ACC<double> pressure(xdim2_ideal_gas_kernel, ydim2_ideal_gas_kernel, &Accessor_pressure[0] + base2 + n_x*1 + n_y * xdim2_ideal_gas_kernel*1 + n_z * xdim2_ideal_gas_kernel * ydim2_ideal_gas_kernel*1);
      ACC<double> soundspeed(xdim3_ideal_gas_kernel, ydim3_ideal_gas_kernel, &Accessor_soundspeed[0] + base3 + n_x*1 + n_y * xdim3_ideal_gas_kernel*1 + n_z * xdim3_ideal_gas_kernel * ydim3_ideal_gas_kernel*1);
      //USER CODE
  ...
  soundspeed(0,0,0) = cl::sycl::sqrt(sound_speed_squared);
