...
void ops_par_loop_flux_calc_kernely_execute(ops_kernel_descriptor *desc) {
  ...
  cl::sycl::buffer<double,1> yarea_p = static_cast<cl::sycl::buffer<char,1> *>((void*)args[1].data_d)->reinterpret<double,1>(cl::sycl::range<1>(args[1].dat->mem/sizeof(double)));
  ...
  block->instance->sycl_instance->queue->submit([&](cl::sycl::handler &cgh) {
    ...
    cgh.parallel_for<class flux_calc_kernely_kernel>(cl::sycl::nd_range<3>(cl::sycl::range<3>(
      ...
      ACC<double> vol_flux_y(xdim0_flux_calc_kernely, ydim0_flux_calc_kernely, &Accessor_vol_flux_y[0] + base0 + n_x*1 + n_y * xdim0_flux_calc_kernely*1 + n_z * xdim0_flux_calc_kernely * ydim0_flux_calc_kernely*1);
      const ACC<double> yarea(xdim1_flux_calc_kernely, ydim1_flux_calc_kernely, &Accessor_yarea[0] + base1 + n_x*1 + n_y * xdim1_flux_calc_kernely*1 + n_z * xdim1_flux_calc_kernely * ydim1_flux_calc_kernely*1);
      const ACC<double> yvel0(xdim2_flux_calc_kernely, ydim2_flux_calc_kernely, &Accessor_yvel0[0] + base2 + n_x*1 + n_y * xdim2_flux_calc_kernely*1 + n_z * xdim2_flux_calc_kernely * ydim2_flux_calc_kernely*1);
      const ACC<double> yvel1(xdim3_flux_calc_kernely, ydim3_flux_calc_kernely, &Accessor_yvel1[0] + base3 + n_x*1 + n_y * xdim3_flux_calc_kernely*1 + n_z * xdim3_flux_calc_kernely * ydim3_flux_calc_kernely*1);
      //USER CODE
      if (n_x < end_0 && n_y < end_1 && n_z < end_2) {
        

  vol_flux_y(0,0,0) = 0.125 * dt_sycl[0] * (yarea(0,0,0)) *
