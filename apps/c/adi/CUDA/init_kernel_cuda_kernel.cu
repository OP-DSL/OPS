...
#define OPS_ACC0(x, y, z)                                                      \
  (x + xdim0_init_kernel * (y) + xdim0_init_kernel * ydim0_init_kernel * (z))

// user function
__device__

    void
    init_kernel(double *val, int *idx) {
...
__global__ void ops_init_kernel(double *__restrict arg0, int arg_idx0,
  ...
  arg0 += idx_x * 1 * 1 + idx_y * 1 * 1 * xdim0_init_kernel +
          idx_z * 1 * 1 * xdim0_init_kernel * ydim0_init_kernel;
...
void ops_par_loop_init_kernel(char const *name, ops_block block, int dim,
  ...
  ops_init_kernel<<<grid, tblock>>>((double *)p_a[0], arg_idx[0], arg_idx[1],
