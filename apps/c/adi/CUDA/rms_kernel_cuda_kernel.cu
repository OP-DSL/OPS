...
//
__constant__ int xdim0_rms_kernel;
int xdim0_rms_kernel_h = -1;
__constant__ int ydim0_rms_kernel;
int ydim0_rms_kernel_h = -1;

#undef OPS_ACC0


#define OPS_ACC0(x,y,z) (x+xdim0_rms_kernel*(y)+xdim0_rms_kernel*ydim0_rms_kernel*(z))
...
__global__ void ops_rms_kernel(
...
int size2 ){
  ...
  arg0 += idx_x * 1*1 + idx_y * 1*1 * xdim0_rms_kernel + idx_z * 1*1 * xdim0_rms_kernel * ydim0_rms_kernel;

  if (idx_x < size0 && idx_y < size1 && idx_z < size2) {
    rms_kernel(arg0, arg1_l);
  }
  for (int d=0; d<1; d++)
...
// host stub function
void ops_par_loop_rms_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  if (xdim0 != xdim0_rms_kernel_h || ydim0 != ydim0_rms_kernel_h) {
    cudaMemcpyToSymbol( xdim0_rms_kernel, &xdim0, sizeof(int) );
    xdim0_rms_kernel_h = xdim0;
    cudaMemcpyToSymbol( ydim0_rms_kernel, &ydim0, sizeof(int) );
    ydim0_rms_kernel_h = ydim0;
  }
  ...
  ops_rms_kernel<<<grid, tblock, nshared >>> (  (double *)p_a[0], (double *)arg1.data_d,x_size, y_size, z_size);
