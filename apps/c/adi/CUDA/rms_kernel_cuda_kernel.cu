...
#define OPS_ACC0(x, y, z)                                                      \
  (x + xdim0_rms_kernel * (y) + xdim0_rms_kernel * ydim0_rms_kernel * (z))

// user function
__device__

    void
    rms_kernel(const double *array, double *rms) {
...
__global__ void ops_rms_kernel(const double *__restrict arg0,
                               double *__restrict arg1, int size0, int size1,
                               int size2) {
  ...
  arg0 += idx_x * 1 * 1 + idx_y * 1 * 1 * xdim0_rms_kernel +
          idx_z * 1 * 1 * xdim0_rms_kernel * ydim0_rms_kernel;
...
void ops_par_loop_rms_kernel(char const *name, ops_block block, int dim,
  ...
  ops_rms_kernel<<<grid, tblock, nshared>>>(
