...
//user function
inline void drhoEpudx_kernel(const ptr_double rhou_new,
                             const ptr_double rho_new,
                             const ptr_double rhoE_new, ptr_double rhoE_res) {
  ...
  double fnim1 =
      OPS_ACC(rhou_new, -1) * OPS_ACC(rhou_new, -1) / OPS_ACC(rho_new, -1);
  p = gam1 * (OPS_ACC(rhoE_new, -1) - 0.5 * fnim1);
  fnim1 = (OPS_ACC(rhoE_new, -1) + p) * OPS_ACC(rhou_new, -1) /
          OPS_ACC(rho_new, -1);

  double fnim2 =
      OPS_ACC(rhou_new, -2) * OPS_ACC(rhou_new, -2) / OPS_ACC(rho_new, -2);
  p = gam1 * (OPS_ACC(rhoE_new, -2) - 0.5 * fnim2);
  fnim2 = (OPS_ACC(rhoE_new, -2) + p) * OPS_ACC(rhou_new, -2) /
          OPS_ACC(rho_new, -2);
  ...
  fnip1 =
      (OPS_ACC(rhoE_new, 1) + p) * OPS_ACC(rhou_new, 1) / OPS_ACC(rho_new, 1);

  double fnip2 =
      OPS_ACC(rhou_new, 2) * OPS_ACC(rhou_new, 2) / OPS_ACC(rho_new, 2);
  p = gam1 * (OPS_ACC(rhoE_new, 2) - 0.5 * fnip2);
  fnip2 =
      (OPS_ACC(rhoE_new, 2) + p) * OPS_ACC(rhou_new, 2) / OPS_ACC(rho_new, 2);
...
void drhoEpudx_kernel_c_wrapper(
  ...
  for ( int n_x=0; n_x<x_size; n_x++ ){
    ...
    drhoEpudx_kernel(ptr0, ptr1, ptr2, ptr3);
  }
}
