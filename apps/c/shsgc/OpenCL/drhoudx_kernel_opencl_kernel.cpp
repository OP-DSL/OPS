...
void buildOpenCLKernels_drhoudx_kernel(int xdim0, int xdim1) {
  ...
  if (!isbuilt_drhoudx_kernel) {
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  -Dxdim0_drhoudx_kernel=%d  "
                           "-Dxdim1_drhoudx_kernel=%d ",
                pPath, 32, xdim0, xdim1);
      else
        sprintf(buildOpts, "-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  "
                           "-Dxdim0_drhoudx_kernel=%d  "
                           "-Dxdim1_drhoudx_kernel=%d ",
                pPath, 32, xdim0, xdim1);
    else {
    ...
    OPS_opencl_core.kernel[3] =
        clCreateKernel(OPS_opencl_core.program, "ops_drhoudx_kernel", &ret);
    clSafeCall(ret);
...
void ops_par_loop_drhoudx_kernel(char const *name, ops_block block, int dim,
  ...
  buildOpenCLKernels_drhoudx_kernel(xdim0, xdim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[3], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[3], 2, sizeof(cl_double),
                            (void *)&dx));
