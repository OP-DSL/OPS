...
void buildOpenCLKernels_fact_kernel(int xdim0, int xdim1) {
  ...
  if (!isbuilt_fact_kernel) {
    ...
    OPS_opencl_core.kernel[12] =
        clCreateKernel(OPS_opencl_core.program, "ops_fact_kernel", &ret);
    clSafeCall(ret);
...
void ops_par_loop_fact_kernel(char const *name, ops_block block, int dim,
  ...
  if (globalWorkSize[0] > 0 && globalWorkSize[1] > 0 && globalWorkSize[2] > 0) {
    ...
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[12], 1, sizeof(cl_mem),
                              (void *)&arg1.data_d));
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[12], 2, sizeof(cl_double),
                              (void *)&dx));
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[12], 3, sizeof(cl_double),
                              (void *)&dt));
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[12], 4, sizeof(cl_int),
                              (void *)&base0));
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[12], 5, sizeof(cl_int),
                              (void *)&base1));
    clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[12], 6, sizeof(cl_int),
                              (void *)&x_size));
