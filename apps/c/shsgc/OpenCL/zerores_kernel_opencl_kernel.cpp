...
void buildOpenCLKernels_zerores_kernel(int xdim0, int xdim1, int xdim2) {
  ...
  if (!isbuilt_zerores_kernel) {
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  -Dxdim0_zerores_kernel=%d  "
                           "-Dxdim1_zerores_kernel=%d  "
                           "-Dxdim2_zerores_kernel=%d ",
                pPath, 32, xdim0, xdim1, xdim2);
      else
        sprintf(buildOpts, "-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  "
                           "-Dxdim0_zerores_kernel=%d  "
                           "-Dxdim1_zerores_kernel=%d  "
                           "-Dxdim2_zerores_kernel=%d ",
                pPath, 32, xdim0, xdim1, xdim2);
    else {
    ...
    OPS_opencl_core.kernel[2] =
        clCreateKernel(OPS_opencl_core.program, "ops_zerores_kernel", &ret);
    clSafeCall(ret);
...
void ops_par_loop_zerores_kernel(char const *name, ops_block block, int dim,
  ...
  buildOpenCLKernels_zerores_kernel(xdim0, xdim1, xdim2);
