...
// user function
void Riemann_kernel(const __global double *restrict rho_new,
                    const __global double *restrict rhou_new,
                    const __global double *restrict rhoE_new,
                    __global double *restrict alam, __global double *restrict r,
                    __global double *restrict al, const double gam1)
...
__kernel void ops_Riemann_kernel(
    ...
    const double gam1, const int base0, const int base1, const int base2,
    const int base3, const int base4, const int base5, const int size0) {

  int idx_x = get_global_id(0);

  if (idx_x < size0) {
    Riemann_kernel(&arg0[base0 + idx_x * 1 * 1], &arg1[base1 + idx_x * 1 * 1],
                   &arg2[base2 + idx_x * 1 * 1], &arg3[base3 + idx_x * 1 * 3],
                   &arg4[base4 + idx_x * 1 * 9], &arg5[base5 + idx_x * 1 * 3],
                   gam1);
  }
}
