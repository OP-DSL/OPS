  ...
  for ( int thr=0; thr<nthreads; thr++ ){
    ...
    for ( int n_x=start_i; n_x<start_i+(finish_i-start_i)/SIMD_VEC; n_x++ ){
      ...
      for ( int i=0; i<SIMD_VEC; i++ ){
        drhoEpudx_kernel(  (const double * )p_a[0]+ i*1*1, (const double * )p_a[1]+ i*1*1, (const double * )p_a[2]+ i*1*1,
           (double * )p_a[3]+ i*1*1 );
    ...
    for ( int n_x=start_i+((finish_i-start_i)/SIMD_VEC)*SIMD_VEC; n_x<finish_i; n_x++ ){
      ...
      drhoEpudx_kernel(  (const double * )p_a[0], (const double * )p_a[1], (const double * )p_a[2],
           (double * )p_a[3] );
