...
void ops_par_loop_poisson_populate_kernel(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4, ops_arg arg5) {
  ...
  int arg_gbl0[MAX(1 , 64) * MAX_REDUCT_THREADS];
  int arg_gbl1[MAX(1 , 64) * MAX_REDUCT_THREADS];
  for ( int thr=0; thr<nthreads; thr++ ){
    for ( int d=0; d<1; d++ ){
      arg_gbl0[d+64*thr] = arg0h[d];
    }
    for ( int d=0; d<1; d++ ){
      arg_gbl1[d+64*thr] = arg1h[d];
    }
  }
    ...
    for ( int n_y=start_i; n_y<finish_i; n_y++ ){
      for ( int n_x=start[0]; n_x<start[0]+(end[0]-start[0])/SIMD_VEC; n_x++ ){
        ...
        for ( int i=0; i<SIMD_VEC; i++ ){
          poisson_populate_kernel(  &arg_gbl0[64*thr], &arg_gbl1[64*thr], arg_idx,
