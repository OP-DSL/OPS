...
void ops_par_loop_advec_cell_kernel4_ydir(char const *name, ops_block Block, int dim, int* range,
 ...
 ops_arg arg9, ops_arg arg10) {
  ...
  //Timing
  double t1,t2,c1,c2;
  ops_timing_realloc(19,"advec_cell_kernel4_ydir");
  ...
  OPS_kernels[19].transfer += ops_compute_transfer(dim, range, &arg6);
  OPS_kernels[19].transfer += ops_compute_transfer(dim, range, &arg7);
  OPS_kernels[19].transfer += ops_compute_transfer(dim, range, &arg8);
  OPS_kernels[19].transfer += ops_compute_transfer(dim, range, &arg9);
  OPS_kernels[19].transfer += ops_compute_transfer(dim, range, &arg10);
}
