...
void ops_par_loop_PdV_kernel_nopredict(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg12, ops_arg arg13) {
  ...
  int max4[ndim]; int min4[ndim];
  for ( int n=0; n<ndim; n++ ){
    max4[n] = 0;min4[n] = 0;
  }
  for ( int p=0; p<args[4].stencil->points; p++ ){
    for ( int n=0; n<ndim; n++ ){
      max4[n] = MAX(max4[n],args[4].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
      min4[n] = MIN(min4[n],args[4].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
    }
  }
  //set up initial pointers and exchange halos if nessasary
  int base4 = dat4 * 1 * 
  ...
  int max5[ndim]; int min5[ndim];
  for ( int n=0; n<ndim; n++ ){
    max5[n] = 0;min5[n] = 0;
  }
  for ( int p=0; p<args[5].stencil->points; p++ ){
    for ( int n=0; n<ndim; n++ ){
      max5[n] = MAX(max5[n],args[5].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
      min5[n] = MIN(min5[n],args[5].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
    }
  }
  //set up initial pointers and exchange halos if nessasary
  int base5 = dat5 * 1 * 
  ...
  int max6[ndim]; int min6[ndim];
  for ( int n=0; n<ndim; n++ ){
    max6[n] = 0;min6[n] = 0;
  }
  for ( int p=0; p<args[6].stencil->points; p++ ){
    for ( int n=0; n<ndim; n++ ){
      max6[n] = MAX(max6[n],args[6].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
      min6[n] = MIN(min6[n],args[6].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
    }
  }
  //set up initial pointers and exchange halos if nessasary
  int base6 = dat6 * 1 * 
  ...
  int max7[ndim]; int min7[ndim];
  for ( int n=0; n<ndim; n++ ){
    max7[n] = 0;min7[n] = 0;
  }
  for ( int p=0; p<args[7].stencil->points; p++ ){
    for ( int n=0; n<ndim; n++ ){
      max7[n] = MAX(max7[n],args[7].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
      min7[n] = MIN(min7[n],args[7].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
    }
  }
  //set up initial pointers and exchange halos if nessasary
  int base7 = dat7 * 1 * 
  ...
  int max8[ndim]; int min8[ndim];
  for ( int n=0; n<ndim; n++ ){
    max8[n] = 0;min8[n] = 0;
  }
  for ( int p=0; p<args[8].stencil->points; p++ ){
    for ( int n=0; n<ndim; n++ ){
      max8[n] = MAX(max8[n],args[8].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
      min8[n] = MIN(min8[n],args[8].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
    }
  }
  //set up initial pointers and exchange halos if nessasary
  int base8 = dat8 * 1 * 
  ...
  int max9[ndim]; int min9[ndim];
  for ( int n=0; n<ndim; n++ ){
    max9[n] = 0;min9[n] = 0;
  }
  for ( int p=0; p<args[9].stencil->points; p++ ){
    for ( int n=0; n<ndim; n++ ){
      max9[n] = MAX(max9[n],args[9].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
      min9[n] = MIN(min9[n],args[9].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
    }
  }
  //set up initial pointers and exchange halos if nessasary
  int base9 = dat9 * 1 * 
  ...
  int max10[ndim]; int min10[ndim];
  for ( int n=0; n<ndim; n++ ){
    max10[n] = 0;min10[n] = 0;
  }
  for ( int p=0; p<args[10].stencil->points; p++ ){
    for ( int n=0; n<ndim; n++ ){
      max10[n] = MAX(max10[n],args[10].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
      min10[n] = MIN(min10[n],args[10].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
    }
  }
  //set up initial pointers and exchange halos if nessasary
  int base10 = dat10 * 1 * 
  ...
  int max11[ndim]; int min11[ndim];
  for ( int n=0; n<ndim; n++ ){
    max11[n] = 0;min11[n] = 0;
  }
  for ( int p=0; p<args[11].stencil->points; p++ ){
    for ( int n=0; n<ndim; n++ ){
      max11[n] = MAX(max11[n],args[11].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
      min11[n] = MIN(min11[n],args[11].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
    }
  }
  //set up initial pointers and exchange halos if nessasary
  int base11 = dat11 * 1 * 
  ...
  int max12[ndim]; int min12[ndim];
  for ( int n=0; n<ndim; n++ ){
    max12[n] = 0;min12[n] = 0;
  }
  for ( int p=0; p<args[12].stencil->points; p++ ){
    for ( int n=0; n<ndim; n++ ){
      max12[n] = MAX(max12[n],args[12].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
      min12[n] = MIN(min12[n],args[12].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
    }
  }
  //set up initial pointers and exchange halos if nessasary
  int base12 = dat12 * 1 * 
  ...
  int max13[ndim]; int min13[ndim];
  for ( int n=0; n<ndim; n++ ){
    max13[n] = 0;min13[n] = 0;
  }
  for ( int p=0; p<args[13].stencil->points; p++ ){
    for ( int n=0; n<ndim; n++ ){
      max13[n] = MAX(max13[n],args[13].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
      min13[n] = MIN(min13[n],args[13].stencil->stencil[ndim*p + n]) * ((range[2*n+1]-range[2*n]) == 1 ? 0 : 1);
    }
  }
  //set up initial pointers and exchange halos if nessasary
  int base13 = dat13 * 1 * 
