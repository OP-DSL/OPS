...
inline void advec_cell_kernel3_xdir( const double *vol_flux_x, const double *pre_vol, const int *xx,
  ...
  else if (xx[OPS_ACC2(1,0)] < x_max+2-2) { //the extra -2 is due to the extra boundary in the OPS version
    upwind   = 1; //j+1
    donor    = 0; //j
    downwind = -1; //j-1
    dif      = upwind;
...
inline void advec_cell_kernel3_ydir( const double *vol_flux_y, const double *pre_vol, const int *yy,
  ...
  else if (yy[OPS_ACC2(0,1)] < y_max+2-2) {  //the extra -2 is due to the extra boundary in the OPS version
    upwind   = 1; //j+1
    donor    = 0; //j
    downwind = -1; //j-1
    dif      = upwind;
