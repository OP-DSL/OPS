...
void ops_par_loop_generate_chunk_kernel(char const *name, int dim, int* range,
 ...
 ops_arg arg4, ops_arg arg5, ops_arg arg6, ops_arg arg7) {
  ...
  //Timing
  double t1,t2,c1,c2;
  ops_timing_realloc(87,"generate_chunk_kernel");
  ...
  for ( int thr=0; thr<nthreads; thr++ ){
    ...
    p_a[6] = &args[6].data[
    + args[6].dat->size * args[6].dat->block_size[0] * ( start * 0 - args[6].dat->offset[1] )
    ...
    for ( int n_y=start; n_y<finish; n_y++ ){
      for ( int n_x=range[0]; n_x<range[0]+(range[1]-range[0])/4; n_x++ ){
        ...
        #pragma simd
        for ( int i=0; i<4; i++ ){
          generate_chunk_kernel(  (double *)p_a[0]+ i*1, (double *)p_a[1]+ i*0,
           (double *)p_a[2]+ i*1, (double *)p_a[3]+ i*1,
           (double *)p_a[4]+ i*1, (double *)p_a[5]+ i*1,
           (double *)p_a[6]+ i*1, (double *)p_a[7]+ i*0 );
  ...
  OPS_kernels[87].transfer += ops_compute_transfer(dim, range, &arg2);
  OPS_kernels[87].transfer += ops_compute_transfer(dim, range, &arg3);
  OPS_kernels[87].transfer += ops_compute_transfer(dim, range, &arg4);
  OPS_kernels[87].transfer += ops_compute_transfer(dim, range, &arg5);
  OPS_kernels[87].transfer += ops_compute_transfer(dim, range, &arg6);
  OPS_kernels[87].transfer += ops_compute_transfer(dim, range, &arg7);
}
