...
void ops_par_loop_initialise_chunk_kernel_volume(char const *name, int dim, int* range,
 ...
 ops_arg arg4) {
  ...
  //Timing
  double t1,t2,c1,c2;
  ops_timing_realloc(86,"initialise_chunk_kernel_volume");
  ...
  for ( int thr=0; thr<nthreads; thr++ ){
    ...
    for ( int n_y=start; n_y<finish; n_y++ ){
      for ( int n_x=range[0]; n_x<range[0]+(range[1]-range[0])/4; n_x++ ){
        ...
        #pragma simd
        for ( int i=0; i<4; i++ ){
          initialise_chunk_kernel_volume(  (double *)p_a[0]+ i*1, (double *)p_a[1]+ i*0,
      ...
      for ( int n_x=range[0]+((range[1]-range[0])/4)*4; n_x<range[1]; n_x++ ){
        ...
        initialise_chunk_kernel_volume(  (double *)p_a[0], (double *)p_a[1],
  ...
  OPS_kernels[86].transfer += ops_compute_transfer(dim, range, &arg2);
  OPS_kernels[86].transfer += ops_compute_transfer(dim, range, &arg3);
  OPS_kernels[86].transfer += ops_compute_transfer(dim, range, &arg4);
}
