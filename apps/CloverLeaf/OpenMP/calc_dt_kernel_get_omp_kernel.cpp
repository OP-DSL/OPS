...
void ops_par_loop_calc_dt_kernel_get(char const *name, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2, ops_arg arg3) {
  ...
  double *arg_gbl2[nthreads];
  double *arg_gbl3[nthreads];
  for ( int thr=0; thr<nthreads; thr++ ){
    arg_gbl2[thr] = (double *)malloc(1 * sizeof(double ));
    arg_gbl3[thr] = (double *)malloc(1 * sizeof(double ));
  }
  ...
  for ( int thr=0; thr<nthreads; thr++ ){
    ...
    ops_args_set(range[0], start, 4, args,p_a); //set up the initial possition
    ...
    for ( int n_y=start; n_y<finish; n_y++ ){
      for ( int n_x=range[0]; n_x<range[1]; n_x++ ){
        ...
        calc_dt_kernel_get(  (double **)p_a[0], (double **)p_a[1],
           (double **) &arg_gbl2[thr], (double **) &arg_gbl3[thr] );
  ...
  for ( int thr=0; thr<nthreads; thr++ ){
    *((double*)(args[2].data)) += *arg_gbl2[thr];
    *((double*)(args[3].data)) += *arg_gbl3[thr];
  }
  for ( int thr=0; thr<nthreads; thr++ ){
    free(arg_gbl2[thr]);
    free(arg_gbl3[thr]);
