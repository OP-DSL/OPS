...
void ops_par_loop_initialise_chunk_kernel_cellx(char const *name, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  //Timing
  double t1,t2,c1,c2;
  ops_timing_realloc(84,"initialise_chunk_kernel_cellx");
  ...
  for ( int thr=0; thr<nthreads; thr++ ){
    ...
    for ( int n_y=start; n_y<finish; n_y++ ){
      for ( int n_x=range[0]; n_x<range[0]+(range[1]-range[0])/4; n_x++ ){
        ...
        #pragma simd
        for ( int i=0; i<4; i++ ){
          initialise_chunk_kernel_cellx(  (double *)p_a[0]+ i*1, (double *)p_a[1]+ i*1,
           (double *)p_a[2]+ i*1 );
      ...
      for ( int n_x=range[0]+((range[1]-range[0])/4)*4; n_x<range[1]; n_x++ ){
        ...
        initialise_chunk_kernel_cellx(  (double *)p_a[0], (double *)p_a[1],
           (double *)p_a[2] );
  ...
  OPS_kernels[84].transfer += ops_compute_transfer(dim, range, &arg2);
}
