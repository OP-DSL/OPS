...
void ops_par_loop_PdV_kernel_predict(char const *name, int dim, int* range,
 ...
 ops_arg arg8, ops_arg arg9, ops_arg arg10, ops_arg arg11) {
  ...
  //Timing
  double t1,t2,c1,c2;
  ops_timing_realloc(80,"PdV_kernel_predict");
  ops_timers_core(&c1,&t1);

  ops_halo_exchanges(args, 12);
  ...
  for ( int thr=0; thr<nthreads; thr++ ){
    ...
    for ( int n_y=start; n_y<finish; n_y++ ){
      for ( int n_x=range[0]; n_x<range[0]+(range[1]-range[0])/4; n_x++ ){
        ...
        #pragma simd
        for ( int i=0; i<4; i++ ){
          PdV_kernel_predict(  (double *)p_a[0]+ i*1, (double *)p_a[1]+ i*1,
  ...
  OPS_kernels[80].transfer += ops_compute_transfer(dim, range, &arg8);
  OPS_kernels[80].transfer += ops_compute_transfer(dim, range, &arg9);
  OPS_kernels[80].transfer += ops_compute_transfer(dim, range, &arg10);
  OPS_kernels[80].transfer += ops_compute_transfer(dim, range, &arg11);
}
