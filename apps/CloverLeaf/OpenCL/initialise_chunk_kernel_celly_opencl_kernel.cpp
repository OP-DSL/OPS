...
static bool isbuilt_initialise_chunk_kernel_celly = false;

void buildOpenCLKernels_initialise_chunk_kernel_celly(int xdim0,
 int xdim1,
 int xdim2) {
  
  if(!isbuilt_initialise_chunk_kernel_celly) {
    buildOpenCLKernels();
    //clSafeCall( clUnloadCompiler() );       
    cl_int ret;
    char* source_filename[1] = {"./OpenCL/initialise_chunk_kernel_celly.cl"};
    ...
    printf("Compiling initialise_chunk_kernel_celly source -- start \n");
      ...
      if (pPath!=NULL)
        sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_initialise_chunk_kernel_celly=%d -Dxdim1_initialise_chunk_kernel_celly=%d -Dxdim2_initialise_chunk_kernel_celly=%d", pPath, 32,xdim0,xdim1,xdim2);
      else {
...
void ops_par_loop_initialise_chunk_kernel_celly(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  buildOpenCLKernels_initialise_chunk_kernel_celly(
  xdim0,
  xdim1,
  xdim2);
  ...
  //Timing
  double t1,t2,c1,c2;
  ops_timing_realloc(75,"initialise_chunk_kernel_celly");
  ...
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[75], 3, sizeof(cl_mem), (void*) &OPS_opencl_core.constant[6]) );
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[75], 4, sizeof(cl_mem), (void*) &OPS_opencl_core.constant[7]) );
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[75], 5, sizeof(cl_int), (void*) &base0 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[75], 6, sizeof(cl_int), (void*) &base1 ));
  clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[75], 7, sizeof(cl_int), (void*) &base2 ));
