...
void ops_par_loop_generate_chunk_kernel(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4, ops_arg arg5, ops_arg arg6, ops_arg arg7) {
  ...
  //Timing
  double t1,t2,c1,c2;
  ops_timing_realloc(75,"generate_chunk_kernel");
    ...
    for ( int n_y=start_i; n_y<finish_i; n_y++ ){
      for ( int n_x=start[0]; n_x<start[0]+(end[0]-start[0])/SIMD_VEC; n_x++ ){
        ...
        for ( int i=0; i<SIMD_VEC; i++ ){
          generate_chunk_kernel(  (const double * restrict)p_a[0]+ i*1, (const double * restrict)p_a[1]+ i*0, (double * restrict)p_a[2]+ i*1,
           (double * restrict)p_a[3]+ i*1, (double * restrict)p_a[4]+ i*1, (double * restrict)p_a[5]+ i*1, (const double * restrict)p_a[6]+ i*1,
           (const double * restrict)p_a[7]+ i*0 );
      ...
      for ( int n_x=start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x<end[0]; n_x++ ){
        ...
        generate_chunk_kernel(  (const double * restrict)p_a[0], (const double * restrict)p_a[1], (double * restrict)p_a[2],
           (double * restrict)p_a[3], (double * restrict)p_a[4], (double * restrict)p_a[5], (const double * restrict)p_a[6],
           (const double * restrict)p_a[7] );
  ...
  OPS_kernels[75].transfer += ops_compute_transfer(dim, range, &arg7);
}
