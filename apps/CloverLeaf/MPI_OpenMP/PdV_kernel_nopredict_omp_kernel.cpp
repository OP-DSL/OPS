...
void ops_par_loop_PdV_kernel_nopredict(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg12, ops_arg arg13) {
  ...
  for ( int n=1; n<ndim; n++ ){
    offs[12][n] = off2(ndim, n, &start_add[12*ndim],
    &end_add[12*ndim],args[12].dat->block_size, args[12].stencil->stride);
  }
  offs[13][0] = args[13].stencil->stride[0]*1;  //unit step in x dimension
  for ( int n=1; n<ndim; n++ ){
    offs[13][n] = off2(ndim, n, &start_add[13*ndim],
    &end_add[13*ndim],args[13].dat->block_size, args[13].stencil->stride);
  }
