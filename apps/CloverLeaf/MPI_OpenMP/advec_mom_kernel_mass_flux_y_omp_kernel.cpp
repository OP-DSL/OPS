    ...
    for ( int n_y=start_i; n_y<finish_i; n_y++ ){
      for ( int n_x=start[0]; n_x<start[0]+(end[0]-start[0])/SIMD_VEC; n_x++ ){
        ...
        for ( int i=0; i<SIMD_VEC; i++ ){
          advec_mom_kernel_mass_flux_y(  (double * restrict)p_a[0]+ i*1, (const double * restrict)p_a[1]+ i*1 );
      ...
      for ( int n_x=start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x<end[0]; n_x++ ){
        ...
        advec_mom_kernel_mass_flux_y(  (double * restrict)p_a[0], (const double * restrict)p_a[1] );
