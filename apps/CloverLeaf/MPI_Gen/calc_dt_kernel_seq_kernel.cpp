...
void ops_par_loop_calc_dt_kernel(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg8, ops_arg arg9, ops_arg arg10) {
  ...
  //Timing
  double t1,t2,c1,c2;
  ops_timing_realloc(74,"calc_dt_kernel");
  ...
  for ( int n_y=s[1]; n_y<e[1]; n_y++ ){
    for ( int n_x=s[0]; n_x<s[0]+(e[0]-s[0])/SIMD_VEC; n_x++ ){
      ...
      OPS_kernels[74].transfer += ops_compute_transfer(dim, range, &arg6);
      OPS_kernels[74].transfer += ops_compute_transfer(dim, range, &arg7);
      OPS_kernels[74].transfer += ops_compute_transfer(dim, range, &arg8);
      OPS_kernels[74].transfer += ops_compute_transfer(dim, range, &arg9);
      OPS_kernels[74].transfer += ops_compute_transfer(dim, range, &arg10);
    }
