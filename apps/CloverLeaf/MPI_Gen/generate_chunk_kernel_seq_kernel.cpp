...
void ops_par_loop_generate_chunk_kernel(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4, ops_arg arg5, ops_arg arg6, ops_arg arg7) {
  ...
  //Timing
  double t1,t2,c1,c2;
  ops_timing_realloc(76,"generate_chunk_kernel");
  ...
  for ( int n_y=s[1]; n_y<e[1]; n_y++ ){
    for ( int n_x=s[0]; n_x<s[0]+(e[0]-s[0])/SIMD_VEC; n_x++ ){
      ...
      OPS_kernels[76].transfer += ops_compute_transfer(dim, range, &arg5);
      OPS_kernels[76].transfer += ops_compute_transfer(dim, range, &arg6);
      OPS_kernels[76].transfer += ops_compute_transfer(dim, range, &arg7);
    }
