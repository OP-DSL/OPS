 ...
 ops_arg arg4, ops_arg arg5, ops_arg arg6, ops_arg arg7) {
  ...
  for ( int n_y=0; n_y<count[1]; n_y++ ){
    ...
    for ( int n_x=0; n_x<count[0]; n_x++ ){
      ...
      for (int i=0; i<8; i++) {
        if (args[i].argtype == OPS_ARG_DAT)
          p_a[i] = p_a[i] + (args[i].dat->size * offs[i][0]);
      }
    }
    ...
    for (int i=0; i<8; i++) {
      if (args[i].argtype == OPS_ARG_DAT)
        p_a[i] = p_a[i] + (args[i].dat->size * offs[i][1]);
    }
  }
  ...
  for (int nt=0; nt<total_range; nt++) {
    ...
    generate_chunk_kernel(  (double *)p_a[0], (double *)p_a[1], (double *)p_a[2], (double*)p_a[3],
