//
// auto-generated by ops.py on 2014-02-24 13:59
//
...
// host stub function
void ops_par_loop_calc_dt_kernel_gety(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  #ifdef OPS_DEBUG
  ops_register_args(args, "calc_dt_kernel_gety");
  #endif
  ...
  //Timing
  double t1,t2,c1,c2;
  ops_timing_realloc(77,"calc_dt_kernel_gety");
  ...
  for ( int n_y=s[1]; n_y<e[1]; n_y++ ){
    for ( int n_x=s[0]; n_x<s[0]+(e[0]-s[0])/SIMD_VEC; n_x++ ){
        ...
        for ( int i=0; i<SIMD_VEC; i++ ){
          calc_dt_kernel_gety(  (double *)p_a[0]+ i*0, (double *)p_a[1] );
      ...
      for ( int n_x=s[0]+((e[0]-s[0])/SIMD_VEC)*SIMD_VEC; n_x<e[0]; n_x++ ){
          ...
          calc_dt_kernel_gety(  (double *)p_a[0], (double *)p_a[1] );
