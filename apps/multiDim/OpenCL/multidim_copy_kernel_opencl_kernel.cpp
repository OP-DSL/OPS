...
void buildOpenCLKernels_multidim_copy_kernel(int xdim0, int xdim1) {
  ...
  if(!isbuilt_multidim_copy_kernel) {
    ...
    printf("Compiling multidim_copy_kernel %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_multidim_copy_kernel=%d  -Dxdim1_multidim_copy_kernel=%d ", pPath, 32,xdim0,xdim1);
        else
          sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_multidim_copy_kernel=%d  -Dxdim1_multidim_copy_kernel=%d ", pPath, 32,xdim0,xdim1);
      else {
...
void ops_par_loop_multidim_copy_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  base1 = base1 + args[1].dat->size[0] *2*
