...
//
int mdim0_multidim_print_kernel;
#define OPS_ACC_MD0(d,x,y) ((x)*mdim0_multidim_print_kernel+(d)+(xdim0*(y)*mdim0_multidim_print_kernel))
...
void ops_par_loop_multidim_print_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0) {
  ...
  mdim0_multidim_print_kernel = args[0].dat->dim;
  ...
  for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
    ...
    for( n_x=start[0]; n_x<start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x+=SIMD_VEC ) {
      ...
      for ( int i=0; i<SIMD_VEC; i++ ){
        multidim_print_kernel(  (double *)p_a[0]+ i*1*mdim0_multidim_print_kernel );
