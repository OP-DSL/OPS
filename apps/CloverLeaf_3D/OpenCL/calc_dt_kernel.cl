...
const double dtdiv_safe)
  ...
  if(dsx <= 0) printf("celldx[OPS_ACC0(0,0,0)] = %lf\n",celldx[OPS_ACC0(0,0,0)]);
  if(dsy <= 0) printf("celldy[OPS_ACC1(0,0,0)] = %lf\n",celldy[OPS_ACC1(0,0,0)]);
  if(dsz <= 0) printf("celldz[OPS_ACC11(0,0,0)] = %lf\n",celldz[OPS_ACC11(0,0,0)]);
    
  cc = soundspeed[OPS_ACC2(0,0,0)] * soundspeed[OPS_ACC2(0,0,0)];
  cc = cc + 2.0 * viscosity[OPS_ACC3(0,0,0)]/density0[OPS_ACC4(0,0,0)];
  printf("cc = %lf g_small = %lf, soundspeed[OPS_ACC2(0,0,0)] = %lf, viscosity[OPS_ACC3(0,0,0)] = %lf, density0[OPS_ACC4(0,0,0)] = %lf\n",
         cc, g_small, soundspeed[OPS_ACC2(0,0,0)], viscosity[OPS_ACC3(0,0,0)],density0[OPS_ACC4(0,0,0)]);
  cc = MAX(sqrt(cc),g_small);

  dtct = dtc_safe * MIN(dsx,MIN(dsy,dsz))/cc;
  
  //if(dtc_safe <= 0)printf("dtc_safe = %lf\n",dtc_safe);
  //if(cc <= 0)printf("cc = %lf\n",cc);
  if(dtct <=0) printf("dtct = %lf, cc = %lf, dtc_safe = %lf\n",dtct,cc,dtc_safe);
  div=0.0;
  ...
  if(dt_min[OPS_ACC10(0,0,0)] <= 0)
    printf("dt_min[OPS_ACC10(0,0,0)] = %lf , dtct = %lf ,dtut = %lf ,dtvt = %lf , dtdivt = %lf , dtwt = %lf \n",
           dt_min[OPS_ACC10(0,0,0)],dtct,dtut,dtvt, dtdivt,dtwt);
 ...
 __global const double * restrict dtw_safe,
