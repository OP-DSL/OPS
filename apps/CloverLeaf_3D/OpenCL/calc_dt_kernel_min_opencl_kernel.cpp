...
void buildOpenCLKernels_calc_dt_kernel_min(int xdim0, int ydim0) {
  ...
  if(!isbuilt_calc_dt_kernel_min) {
    ...
    printf("Compiling calc_dt_kernel_min %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA=1 -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_calc_dt_kernel_min=%d -Dydim0_calc_dt_kernel_min=%d", pPath, 32,xdim0, ydim0);
        else
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA=0 -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_calc_dt_kernel_min=%d -Dydim0_calc_dt_kernel_min=%d", pPath, 32,xdim0, ydim0);
      else {
