...
void ops_par_loop_calc_dt_kernel_get(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4, ops_arg arg5) {
      ...
      for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
        for ( int n_x=start[0]; n_x<start[0]+(end[0]-start[0])/SIMD_VEC; n_x++ ){
          ...
          for ( int i=0; i<SIMD_VEC; i++ ){
           ...
           &arg_gbl3[64*thr], (const double * restrict)p_a[4]+ i*0, &arg_gbl5[64*thr] );
        ...
        for ( int n_x=start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x<end[0]; n_x++ ){
          ...
          calc_dt_kernel_get(  (const double * restrict)p_a[0], (const double * restrict)p_a[1], &arg_gbl2[64*thr],
           &arg_gbl3[64*thr], (const double * restrict)p_a[4], &arg_gbl5[64*thr] );
  ...
  ops_mpi_reduce(&arg5,(double *)arg5h);
