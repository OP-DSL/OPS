      ...
      for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
        for ( int n_x=start[0]; n_x<start[0]+(end[0]-start[0])/SIMD_VEC; n_x++ ){
          ...
          for ( int i=0; i<SIMD_VEC; i++ ){
            update_halo_kernel2_xvel_plus_2_front(  (double * restrict)p_a[0]+ i*1, (double * restrict)p_a[1]+ i*1, &arg_gbl2[64*thr] );
        ...
        for ( int n_x=start[0]+((end[0]-start[0])/SIMD_VEC)*SIMD_VEC; n_x<end[0]; n_x++ ){
          ...
          update_halo_kernel2_xvel_plus_2_front(  (double * restrict)p_a[0], (double * restrict)p_a[1], &arg_gbl2[64*thr] );
