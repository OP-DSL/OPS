...
void PdV(int predict)
{
  ...
  if(predict == TRUE) {
  ops_par_loop(PdV_kernel_predict, "PdV_kernel_predict", clover_grid, 3, rangexyz_inner,
  ...
  ops_par_loop(PdV_kernel_nopredict, "PdV_kernel_nopredict", clover_grid, 3, rangexyz_inner,
    ops_arg_dat(xarea, 1, S3D_000_P100, "double", OPS_READ),
    ops_arg_dat(xvel0, 1, S3D_000_fP1P1P1, "double", OPS_READ),
