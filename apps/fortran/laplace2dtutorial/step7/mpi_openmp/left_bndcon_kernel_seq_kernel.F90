! Auto-generated at 2024-12-18 22:20:10.671567 by ops-translator

MODULE LEFT_BNDCON_KERNEL_MODULE
...
!DEC$ ATTRIBUTES FORCEINLINE :: left_bndcon_kernel
#endif
...
!   For testing on Hopper, where without SIN function result in same output
!    A(OPS_ACC1(0,0)) = (pi * idx(2) / (jmax+2))

END SUBROUTINE

#undef OPS_ACC1

SUBROUTINE left_bndcon_kernel_wrap( &
    opsDat1Local, &
    idx, &
    dat1_base, &
    start_indx, &
...
END SUBROUTINE left_bndcon_kernel_wrap

!   ===============
!   Host subroutine
!   ===============
#ifndef OPS_LAZY
SUBROUTINE left_bndcon_kernel_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2 &
    )
...
#else
SUBROUTINE left_bndcon_kernel_host_execute( descPtr )
...
SUBROUTINE left_bndcon_kernel_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    2, 2, dim, 0, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(left_bndcon_kernel_host_execute))

END SUBROUTINE
#endif

END MODULE LEFT_BNDCON_KERNEL_MODULE
