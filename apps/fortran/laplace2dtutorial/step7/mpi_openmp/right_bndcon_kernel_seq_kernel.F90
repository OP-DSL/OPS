! Auto-generated at 2024-12-18 22:20:10.722598 by ops-translator
...
!DEC$ ATTRIBUTES FORCEINLINE :: right_bndcon_kernel
#endif
SUBROUTINE right_bndcon_kernel(A, idx)

    REAL(KIND=8), DIMENSION(1) :: A
    INTEGER(KIND=4), DIMENSION(2), INTENT(IN) :: idx

    A(OPS_ACC1(0,0)) = SIN(pi * idx(2) / (jmax+2)) * EXP(-pi)

END SUBROUTINE

#undef OPS_ACC1

SUBROUTINE right_bndcon_kernel_wrap( &
    opsDat1Local, &
    idx, &
    dat1_base, &
    start_indx, &
...
END SUBROUTINE right_bndcon_kernel_wrap
...
#else
SUBROUTINE right_bndcon_kernel_host_execute( descPtr )
    ...
    kernelName = "right_bndcon_kernel"
...
SUBROUTINE right_bndcon_kernel_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    2, 3, dim, 0, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(right_bndcon_kernel_host_execute))
