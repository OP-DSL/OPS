// Auto-generated at 2024-12-18 11:34:42.930921 by ops-translator

extern "C"
void apply_stencil_kernel_host_c(
    const char *name,
    ops_arg *args,
    int nargs,
    int index,
...
//  ======
    ...
    ops_arg arg0, arg1, arg2;
    arg0 = args[0];
    arg1 = args[1];
    arg2 = args[2];

#if defined(CHECKPOINTING) && !defined(OPS_LAZY)
    if (!ops_checkpointing_before(args, 3, range, index)) return;
#endif

    if (block->instance->OPS_diags > 1)
    {
        ops_timing_realloc(block->instance, index, "apply_stencil");
        block->instance->OPS_kernels[index].count++;
...
//  ==============
    ...
    if (block->instance->OPS_diags > 1)
    {
        ...
        block->instance->OPS_kernels[index].mpi_time += __t2 - __t1;
    }
  ...
  *error = fmax( *error, fabs(Anew(0,0)-A(0,0)));
    ...
    {
        ops_timers_core(&__c1, &__t1);
        block->instance->OPS_kernels[index].time += __t1 - __t2;
    }
...
//      ====================
        ops_timers_core(&__c2, &__t2);
        block->instance->OPS_kernels[index].mpi_time += __t2 -__t1;
        block->instance->OPS_kernels[index].transfer += ops_compute_transfer(dim, start_indx, end_indx, &arg0);
        block->instance->OPS_kernels[index].transfer += ops_compute_transfer(dim, start_indx, end_indx, &arg1);
    }
}
