...
Reductions in loops are done using the `ops_arg_reduce` argument, which takes a reduction handle as an argument. The result of the reduction can then be acquired using a separate call to `ops_reduction_result`. The semantics are the following: a reduction handle after it is declared is in an "uninitialized" state. The first time it is used as an argument to a loop, its type is determined (increment/min/max), and it is initialized appropriately $(0,\infty,-\infty)$, and subsequent uses of the handle in parallel loops are combined together, up until the point where the result is acquired using `ops_reduction_result`, which then sets it back to an uninitialized state. This also implies that different parallel loops, which all use the same reduction handle but are otherwise independent, are independent and their partial reduction results can be combined together associatively and commutatively.

OPS takes responsibility for all data, its movement, and the execution of parallel loops. With different execution hardware and optimizations, this means OPS will **re-organize** data as well as execution (potentially across different loops), and therefore **data accesses or manipulation should only be done through the OPS API**. There is an external data access API that allows access to the data stored by OPS, which in turn allows interfacing with external libraries.

This restriction is exploited by a lazy execution mechanism in OPS. The idea is that OPS API calls that do not return a result need not be executed immediately, but rather queued, and once an API call requires returning some data, operations in the queue are executed, and the result is returned. This allows OPS to analyze and optimize operations in the queue together. This mechanism is fully automated by OPS and is used with the various `_tiled` executables. For more information on how to use this mechanism for improving CPU performance, see the section on Tiling. Some API calls triggering the execution of queued operations include `ops_reduction_result` and the functions in the data access API.
...
* To implement periodic boundary conditions using dummy "halo" points, we sometimes have to copy one plane of boundary data to another. For example, if the first dimension has size $I$, then we might copy the plane $i=I-2$ to plane $i=0$, and plane $i=1$ to plane $i=I-1$.
...
OPS handles all of these different requirements through stencil definitions.

## OPS C and C++ API

Both C and C++ style APIs are provided for utilizing the capabilities provided by the OPS library. They are essentially the same, although there are minor differences in syntax. The C++ API is mainly designed for data abstraction, which therefore provides better data encapsulation and the support of multiple instances and threading (OpenMP currently). In the following, both C style routines and C++ class and methods will be introduced according to their functionality with a notice (C) or (C++). If there is no such notice, the routine either applies to both or might not be provided by the C++ API.
...
Currently, higher diags_levels do the following checks:
...
where the meaning of arguments is the same as in the C API, while the extra argument (i.e., ss) is for accepting the messages.
...
__ops_dat ops_decl_dat(ops_block block, int dim, int *size, int *base, int *d_m, int *d_p, T *data, char *type, char *name)__
...
The `size` allows you to declare different sized data arrays on a given
`block`. `d_m` and `d_p` are depths of the "block halos" that are used to
...
so that they can be correctly handled for different parallelizations. For example, CUDA on GPUs. Once defined,
...
goes to y in the second block, and y in the first block goes to x in the second
...
from_dir = \[0,1,2\], to_dir = \[0,1,2\]. In more complex cases, this
...
__void ops_reduction_result(ops_reduction handle, T *result)__
...
__void ops_partition(char *method)__
...
and ops_decl_dat statements have been declared.

| Arguments      | Description |
| ----------- | ----------- |
| method| string describing the partitioning method. Currently this string is not used internally, but is simply a placeholder to indicate different partitioning methods in the future. |
...
Same as the C counterpart.

#### ops_printf

__void ops_printf(const char * format, ...)__

This routine simply prints a variable number of arguments; it is created in place of the standard C
...
Write the details of an ops_stencil to a named HDF5 file. Can be used over MPI (puts the data in an ops_dat into an HDF5 file using MPI I/O).
...
Write the details of an ops_dat to a named HDF5 file. Can be used over MPI (puts the data in an ops_dat into an
HDF5 file using MPI I/O).
...
Gives you an array of integers (in the user kernel) that have the index of
...
Initializes the checkpointing system; has to be called after *ops_partition*. Returns true if the application launches in restore
...
| options| a combination of flags, listed in *ops_checkpointing.h*, also see below|
...
what datasets to be saved for full recovery. This routine should be called frequently (compared to checkpointing frequency) and it will trigger the creation of the checkpoint the first time it is called after the timeout occurs.

| Arguments      | Description |
| ----------- | ----------- |
| ndats| number of datasets to be saved|
| datlist| array of *ops_dat* handles to be saved|

#### ops_checkpointing_fastfw

__bool ops_checkpointing_fastfw(int nbytes, char *payload)__

A user can call this routine at a point in the code to mark the location of a checkpoint. At this point, the
...
and will trigger the creation of the checkpoint the first time it is called after the timeout occurs. In restore mode,
it will restore all datasets the first time it is called, and returns true indicating that the saved payload is returned
...
__bool ops_checkpointing_manual_datlist_fastfw(int ndats, ops_dat *datlist, int nbytes, char *payload)__
...
__bool ops_checkpointing_manual_datlist_fastfw_trigger(int ndats, ops_dat *datlist, int nbytes, char *payload)__
...
This section describes APIs that give the user access to internal data structures in OPS and return data to user-space. These should be used cautiously and sparingly, as they can affect performance significantly.
...
The C++ version of ``ops_dat_get_extents`` where the arguments are the same except no need for the ops_dat argument.
...
| stride|      an array populated with strides in spatial dimensions needed for column-major indexing|
...
The C++ version of ``ops_dat_get_raw_metadata`` where the arguments are the same except no need for the ops_dat argument.
...
#### ops_dat_core::get_raw_pointer (C++)
The C++ version of ``ops_dat_get_raw_pointer`` where the arguments are the same except no need for the ops_dat argument.
...
The C++ version of ``ops_dat_release_raw_data`` where the arguments are the same except no need for the ops_dat argument.
...
The C++ version of ``ops_dat_fetch_data_memspace`` where the arguments are the same except no need for the ops_dat argument.
...
Please refer to the `OPS/apps/c/random/random.cpp` example application.
...
Then parameters specific to different solving strategies can be set using setter methods. For applications using MPI, it is beneficial to reuse `ops_tridsolver_params` objects between solves as much as possible due to setup times involved with creating Tridsolver's MPI communicators.
...
__void ops_tridMultiDimBatch_Inc(int ndim, int solvedim, int* range, ops_dat a, ops_dat b, ops_dat c, ops_dat d, ops_dat u, ops_tridsolver_params *tridsolver_ctx)__
...
* `OPS_FORCE_DECOMP_X=`, `OPS_FORCE_DECOMP_Y=`, and `OPS_FORCE_DECOMP_Z=` : Forcing the number of processors in the X, Y, and Z directions for domain decomposition in MPI. By default, OPS selects the most suitable configuration unless the user explicitly specifies it using these options.

* `OPS_BLOCK_SIZE_X=`, `OPS_BLOCK_SIZE_Y=`, and `OPS_BLOCK_SIZE_Z=` : The CUDA (and HIP) thread block sizes in X, Y, and Z dimensions. The sizes should be an integer between 1 - 1024, and currently they should be selected such that `OPS_BLOCK_SIZE_X`*`OPS_BLOCK_SIZE_Y`*`OPS_BLOCK_SIZE_Z` < 1024
