...
Degenerate datasets are datasets where in at least one of the dimensions the dataset's size is 1. For example, in a 2D problem, if we have a degenerate dataset called xcoords, size (N,1), then we will need a stencil with stride (1,0) to access it in a regular 2D loop. Such datasets can also be written to, or used for a reduction with the same stencils, but OPS_WRITE, OPS_INC, OPS_MAX, OPS_MIN used allowed. The semantics of reductions are similart to `ops_arg_reduce`, but the reduction is done on a dataset, not just a grid-invariant scalar (or small array).Multiple reductions of the same type can be done on the same dataset, and the results are combined together associatively and commutatively. Data written to (or reduced into) a degenerate dataset in a parallel loop with any range will be visible to other parallel loops even with different ranges - over MPI, this data is broadcasted to all processes. (Known issue: when different parts of a lower dimensional dataset are written to in subsequent different loops, on different processes, not all updates may be visible to other processes).
