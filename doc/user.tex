...
handled for different parallelizations. For e.g CUDA on GPUs. Once defined they remain unchanged throughout the 
...
\addcontentsline{toc}{subsubsection}{ops\_timing\_output}
...
\addcontentsline{toc}{subsubsection}{ops\_arg\_idx}
...
\addcontentsline{toc}{subsubsection}{ops\_decl\_stencil\_hdf5}
...
loops at the very beginning of the simulations which should be excluded from the checkpointing
process; mainly because they initialise datasets that do not change during the main body of the
execution. During backup mode, and datasets written during this initial phase will be regarded
“never modified” by the first step of the algorithm in Section 3, during restore mode these
loops are executed as usual. An example would be the computation of the mesh geometry,
...
this initial phase, beyond which the normal checkpointing algorithms apply.

OPS\_CHECKPOINT\_MANUAL\_DATLIST - Indicates that the user manually control the location of the checkpoint 
specifying the \texttt{ops\_dat}s to be saved. It also enables fast-forwarding, by skipping the execution of the 
application (even though none of the parallel loops would actually execute, there may be significant work outside of 
those) up to the checkpoint

OPS\_CHECKPOINT\_FASTFW - Combines the above two options but also defines the list of datasets to be saved and a 
payload, and enables fast-forwarding. 

OPS\_CHECKPOINT\_MANUAL -
\end{routine}


\addcontentsline{toc}{subsubsection}{ops\_checkpointing\_manual\_datlist}
...
\addcontentsline{toc}{subsubsection}{ops\_checkpointing\_fastfw}
...
\addcontentsline{toc}{subsubsection}{ops\_checkpointing\_manual\_datlist\_fastfw}
...
\addcontentsline{toc}{subsubsection}{ops\_checkpointing\_manual\_datlist\_fastfw\_trigger}
