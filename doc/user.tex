...
\begin{routine} {ops\_dat ops\_decl\_dat(ops\_block block, int dim, int* size, int *d\_m, int *d\_p, \\T *data, char
*type, char *name)}
...
\begin{routine} {ops\_dat ops\_decl\_dat\_hdf5(ops\_block block, int dim, int *d\_m, int *d\_p, char *file, char *type, char *name)}
...
\item[from\_dir]          direction of incrementing for “from” for each dimension of iter\_size
\item[to\_dir]           direction of incrementing for “to” for each dimension of iter\_size
\end{routine}

(Simple example: a transfer from (1:2,0:99,0:99) to (-1:0,0:99,0:99) would use iter\_size = [2,100,100], from base = [1,0,0], to base = [-1,0,0], from dir = [0,1,2], to dir = [0,1,2]. In more complex case this allows for transfers between blocks with different orientations.)

\begin{routine} {ops\_halo\_group ops\_decl\_halo\_group(int nhalos, ops\_halo *halos)}
{This routine defines a collection of halos. Semantically, When an exchange is triggered for all halos in a group, there is no ordering defined in which they are carried out.}
...
\begin{routine} {ops\_halo\_group ops\_decl\_halo\_group(int nhalos, ops\_halo *halos)}
{This routine defines a collection of halos. When an exchange is triggered for all halos in a group, there is no ordering defined in which they are carried out.}

\item[nhalos]         number of halos in {\tt halos}
\item[halos]           array of halos
\end{routine}

Halo groups (\texttt{ops\_halo\_group}) are intended to be sets, so for their manipulation a number of functions ar introduced:

\begin{routine} {ops\_halo\_group ops\_decl\_halo\_group\_empty()}
{This routine defines an empty halo group.}
\item[returns]    an empty halo group
\end{routine}
...
\begin{routine} {ops\_halo\_group ops\_halo\_group\_union(ops\_halo\_group group1, ops\_halo\_group group2)}
{Takes the union of two halo groups and returns a new halo group.}

\item[group1]         the first halo group
\item[group2]         the second halo group
\end{routine}
