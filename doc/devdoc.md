...
└── .python/                 # Python virtual environment (generated by `make python`, not in version control)
```
...
CMake automatically discovers:  MPI (`find_package(MPI)`), HDF5 (`find_package(HDF5)`), CUDA (`find_package(CUDAToolkit)`), OpenMP (`find_package(OpenMP)`), HIP (when `OPS_HIP=ON`), and Python 3.8+. The translator's Python virtual environment (`ops_translator/.python/`) is set up automatically during the CMake install step via `ops_translator/setup_venv_cmake.sh`, which internally calls `make python`.
