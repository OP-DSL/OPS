...
template <typename ParamT> struct param_handler {
  ...
  #endif //OPS_MPI
    } else if (arg.argtype == OPS_ARG_SCL) {
        return ((ops_scalar)arg.data)->data;
...
/**
 * Performs a passive parallel loop in adjoint codes, executing the user-defined
 * function kernel, passing data as specified by the list arguments. The
 * iteration space is dim dimensional, and the bounds are specified by range
 *
 * Arguments to kernel are passed through as ACC<datatype>& references
 * for ops_dats, and need to be accessed with the overloaded () operator.
 * For other types of arguments, pointers are passed that can be
 ...
 */
template <typename... ParamType, typename... OPSARG>
void ops_par_loop_passive(void (*kernel)(ParamType...), char const *name,
                          ops_block block, int dim, int *range,
                          OPSARG... arguments) {
  ops_par_loop(kernel, name, block, dim, range, arguments...);
}
