...
public:
    ...
    void run(ops::hls::AccessRange& range, ops::hls::Grid<float>& arg0, ops::hls::Grid<float>& arg1)
    {
        ...
        OCL_CHECK(err, err = m_fpga->getCommandQueue().enqueueTask(m_datamover_apply_stencil, &activeEvents, &event_kernel));
		OCL_CHECK(err, err = m_fpga->getCommandQueue().enqueueTask(m_kernel_apply_stencil, &activeEvents, &event_datamover));
