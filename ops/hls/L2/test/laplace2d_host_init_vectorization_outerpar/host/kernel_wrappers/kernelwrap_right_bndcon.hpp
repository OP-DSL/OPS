...
void kernel_right_bndcon(float& arg0_out_0, const int *idx)
{
	arg0_out_0 = sin(pi * (idx[1] + 1) / (jmax + 1)) * exp(-pi);
}
...
void ops_par_loop_right_bndcon(int dim , int* ops_range, ops::hls::Grid<float>& arg0)
{
	...
	for (unsigned short j = range.start[1]; j < range.end[1]; j++)
	{
		for (unsigned short i = range.start[0]; i < range.end[0]; i++)
		{
			...
			kernel_right_bndcon(arg0.hostBuffer[getOffset(arg0_0_stencil_offset, arg0.originalProperty, i, j)], idx);
		}
	}
