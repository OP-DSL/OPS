...
inline stencil_type kernel_apply_stencil_core(stencil_type& point_0, 
    stencil_type& point_1, 
    stencil_type& point_2, 
    stencil_type& point_3, 
    stencil_type& point_4)
{
#ifdef DEBUG_LOG
    printf("[KERNEL_INTERNAL]|%s| starting kernel core\n",__func__);
#endif

    stencil_type r0 = point_0;
    stencil_type r1 = point_1;
    stencil_type r2 = point_2;
    stencil_type r3 = point_3;
    stencil_type r4 = point_4;

    stencil_type r5 = point_0 + point_1;
    stencil_type r6 = point_3 + point_4;
    stencil_type r7 = r5 + r6;
    stencil_type r = 0.25f * r7;

#ifdef DEBUG_LOG
			printf("[KERNEL_INTERNAL]|%s| read_val: (%f, %f, %f, %f, %f), write_val: (%f)\n",
					 __func__, r0, r1, r2, r3, r4, r);
#endif
...
class Stencil_apply_stencil : public ops::hls::StencilCoreV2<stencil_type, s2d_5pt_num_points, vector_factor, ops::hls::CoefTypes::CONST_COEF,
        s2d_5pt_stencil_size, s2d_5pt_stencil_dim>
{
    public:
        ...
        void stencilRun(::hls::stream<ap_uint<axis_data_width>>& rd_buffer,
        		::hls::stream<ap_uint<axis_data_width>>& wr_buffer)
        {
#ifdef DEBUG_LOG
...
#pragma HLS UNROLL factor=vector_factor
					...
					bool neg_cond = register_it((index < stencilConfig.lower_limit[0])
					...
					// }
                    stencil_type result =  kernel_apply_stencil_core(rowArr_0[k + s_stencil_half_span_x], 
                            rowArr_1[k], 
                            rowArr_1[k+1], 
                            rowArr_1[k+2], 
                            rowArr_2[k + s_stencil_half_span_x]);
                    
                    ops::hls::DataConv tmpConvWrite;

                    if (not neg_cond)
                    {
                        tmpConvWrite.f = result;
                    }
                    else
                    {
                        tmpConvWrite.f = rowArr_1[k+1];
                    }

                    update_val.range(s_datatype_size * (k + 1) - 1, k * s_datatype_size) = tmpConvWrite.i;
                }
                ...
                write:
                {
                    ...
                    if (cond_write)
                    {
                        ...
                        #pragma HLS UNROLL factor=vector_factor
                			...
                			tmp.i = update_val.range(s_datatype_size * (k + 1) - 1, k * s_datatype_size);
                			printf("%f,", tmp.f);
                		}
                		printf(")\n");
#endif
                        wr_buffer << update_val;
                    }
                }
            }
#ifdef DEBUG_LOG
